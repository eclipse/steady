...
public class SvnClient implements IVCSClient {
    ...
    public Path checkout(String _rev) throws Exception {
        ...
        try {
            ...
            SvnClient.log.error(
                    "Error while checking out revision " + _rev + ": " + e.getMessage());
        } finally {
        ...
        try {
            Collection<SVNDirEntry> entries =
                    this.rootRepo.getDir(
                            ...
                            -1,
                            null,
                            (Collection<SVNDirEntry>) null); // this.getDirEntries(path);
            ...
            while (iterator.hasNext()) {
                ...
                path =
                        entry.getURL()
                                .toString()
                                .substring(entry.getRepositoryRoot().toString().length());
    ...
    public void cleanup() {
        try {
            // Does not work as the directory must be empty. Solution to be implemented: Create a
            // file walker deleting every other file or directory.
    ...
    public Set<FileChange> getFileChanges(String _rev) {
        ...
        try {
            // Update revision log (using this.asOf as argument will result in an actual SVN call
            ...
            for (Map.Entry<String, SVNLogEntryPath> p : changedPaths.entrySet()) {
                ...
                if ((oldf != null && !oldf.isDirectory()) || (newf != null && !newf.isDirectory()))
                    changes.add(
                            new FileChange(
                                    this.rootRepo.getLocation().toString(), rel_path, oldf, newf));
            }
        } catch (Exception e) {
    ...
    public long getRevisionTimeStamp(String revision) {
        ...
        try {
            // An "svn:date" revision property that is a date & time stamp representing the time
            // when the revision was created.
            SVNPropertyValue propertyValue =
                    rootRepo.getRevisionPropertyValue(
                            Long.parseLong(revision), SVNRevisionProperty.DATE);
            ...
            // e.printStackTrace();
            SvnClient.log.error(
                    "Error when retrieving time stamp for revision : "
                            + revision
                            + " "
                            + e.getMessage());
        }
