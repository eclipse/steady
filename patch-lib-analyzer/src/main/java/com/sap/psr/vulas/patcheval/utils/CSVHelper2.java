...
public class CSVHelper2 {
    ...
    public static String appendResultsToFile(String bugId, File f, List<ConstructPathLibResult2> newResults) {
         ...
         try (BufferedReader br = new BufferedReader( new InputStreamReader(new FileInputStream(f.getAbsolutePath()), FileUtil.getCharset()) )){
            ...
            fileWriter.close();
            CSVHelper2.log.info("CSV " + f + " successfully created/updated with [" + newResults.size()+"] (new) results.");
     ...
     * @throws IOException
     */
    private static StringBuilder createCSVString(String bugId, List<ConstructPathLibResult2> results) throws IOException {
    	...
    	StringBuilder sb = new StringBuilder();
    	 for (int r =0;r<results.size();r++ ){
             ...
             if(cpr.getLidResult().isSourcesAvailable()!=null) sb.append(cpr.getLidResult().isSourcesAvailable().toString());

             sb.append(COMMA_DELIMITER);
             if(cpr.getLidResult().getChangesToV()!=null) sb.append(String.valueOf(cpr.getLidResult().getChangesToV()));
             sb.append(COMMA_DELIMITER);
             if(cpr.getLidResult().getChangesToF()!=null) sb.append(String.valueOf(cpr.getLidResult().getChangesToF()));
             sb.append(COMMA_DELIMITER);
             if(cpr.getVulnAst()!=null){
             	String path = File.separator+"asts" + File.separator  
             			+bugId+"_vulnAst_"+cpr.getQname() + "_" + getShortRepoPathName(cpr.getPath()).replace('/', '-') + ".txt";
             	if(path.length()>260){
             		path = (File.separator+"asts" + File.separator  
                 			+bugId+"_vulnAst_"+cpr.getQname() + "_" + getShortRepoPathName(cpr.getPath()).replace('/', '-')).substring(0, 200) + ".txt";
             	}
             ...
             if(cpr.getFixedAst()!=null) {
             	String path =  File.separator+"asts"+File.separator  +
             			bugId+"_fixedAst_"+cpr.getQname() + "_" + getShortRepoPathName(cpr.getPath()).replace('/', '-')+".txt";
             	if(path.length()>260){
             		path = ( File.separator+"asts"+File.separator  
                 			+bugId+"_fixedAst_"+cpr.getQname() + "_" + getShortRepoPathName(cpr.getPath()).replace('/', '-')).substring(0, 200) + ".txt";
             	}
	...
	public static String writeResultsToFile(String bugId, List<ConstructPathLibResult2> allResults) {
        ...
        String FILE_HEADER = 
        		"Qname"+COMMA_DELIMITER+
        		"Path"+COMMA_DELIMITER+
        		"Group"+COMMA_DELIMITER + 
        		"Artifact"+COMMA_DELIMITER+
        		"Version"+COMMA_DELIMITER+
        		"Overall Change Type"+COMMA_DELIMITER+
        		"JA qnameInJar"+COMMA_DELIMITER+
        		"CIA Artifact sources available"+COMMA_DELIMITER+
        		"dTV"+COMMA_DELIMITER+
        		"dTF"+COMMA_DELIMITER+
        		"Vuln AST"+COMMA_DELIMITER+
        		"Fixed AST"+COMMA_DELIMITER+
        		"Tested AST"+COMMA_DELIMITER+
        		"lib timestamp";
	...
	public static synchronized void rewriteLineInCSV(String bugId, ArtifactResult2 r) {
        ...
        if (containingFolder.exists() ) {
        	...
        	try{ 
	        	...
	        	if ( f.exists() ){
	        		CSVHelper2.log.info("Rewrite result for artifact [" +r.toString()+"] in file [" + f.getName()+"]");
				...
				try (BufferedReader br = new BufferedReader( new InputStreamReader(new FileInputStream(f.getAbsolutePath()), FileUtil.getCharset()) )){
					...
					while ((line = br.readLine()) != null ) {
						...
						if(splitLine[0].compareTo("") != 0){
							if(splitLine[0].equals("Qname")){
								if(splitLine.length==14)
									FILE_HEADER =line+COMMA_DELIMITER+"SameBytecode"+COMMA_DELIMITER+"DoneComparisons";
								...
								if(r.equalGAV(splitLine[2], splitLine[3], splitLine[4])){
									for(ConstructPathAssessment2 cpa:r.getConstructPathAssessments()){
										if(cpa.getConstruct().equals(splitLine[0])&&cpa.getPath().equals(splitLine[1])){
								            ...
								            if(cpa.getdToV()!=null)
								            	sb.append(cpa.getdToV()).append(COMMA_DELIMITER);
								            else
								            	sb.append(splitLine[8]).append(COMMA_DELIMITER);
								            if(cpa.getdToF()!=null)
								            	sb.append(cpa.getdToF()).append(COMMA_DELIMITER);
								            else
								            	sb.append(splitLine[9]).append(COMMA_DELIMITER);
											...
											if(cpa.getLibsSameBytecode()!=null){
												...
												for(LibraryId l:cpa.getLibsSameBytecode()){
													if(i==0){
														libs=l.getMvnGroup().concat(":").concat(l.getArtifact()).concat(":").concat(l.getVersion());i++;
													}
													else libs+="|"+l.getMvnGroup().concat(":").concat(l.getArtifact()).concat(":").concat(l.getVersion());
												}
											...
											sb.append(COMMA_DELIMITER);
											//TODO: this break should not be necessary, as for each line in the CSV only 1 constructPathAssessment should exist
											// however we currently have CSV where the same construct paths for the same lib appear multiple times (to understand why)
											// so the break ensures that we do not add multiple times the same construct path in the same line!
											break;
										}
									}
								}
				        ...
				        log.info("Completed rewriting results for artifact [" + r.toString()+"] to include results from bytecode comparison.");
					}
	        	}		
