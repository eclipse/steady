...
public class CSVHelper2 {
                    ...
                    new BufferedWriter(
                            new OutputStreamWriter(
                                    new FileOutputStream(
                                            f.getAbsolutePath(), true), // true to append
     ...
     * @throws IOException
     */
    private static StringBuilder createCSVString(
            String bugId, List<ConstructPathLibResult2> results) throws IOException {

        File methDir =
                new File(PEConfiguration.getBaseFolder().toString() + File.separator + "asts");
    ...
    public static String writeResultsToFile(
            String bugId, List<ConstructPathLibResult2> allResults) {
        File f =
                new File(
                        PEConfiguration.getBaseFolder().toString()
                                + File.separator
        ...
        try {
            fileWriter =
                    new BufferedWriter(
                            new OutputStreamWriter(
                                    new FileOutputStream(
                                            f.getAbsolutePath(), false), // false to append
                                    FileUtil.getCharset() // Set encoding
                                    ));
                            ...
                            if (splitLine[0].compareTo("") != 0) {
                                if (splitLine[0].equals("Qname")) {
                                    ...
                                    if (r.equalGAV(splitLine[2], splitLine[3], splitLine[4])) {
                                        for (ConstructPathAssessment2 cpa :
                                                r.getConstructPathAssessments()) {
                                            if (cpa.getConstruct().equals(splitLine[0])
                                                ...
                                                sb.append(COMMA_DELIMITER);
                                                // TODO: this break should not be necessary, as for
                                                // each line in the CSV only 1
                                                // constructPathAssessment should exist
                                                // however we currently have CSV where the same
                                                // construct paths for the same lib appear multiple
                                                // times (to understand why)
                                                // so the break ensures that we do not add multiple
                                                // times the same construct path in the same line!
                                                break;
                                            }
                                        }
                                    } else sb.append(line);
