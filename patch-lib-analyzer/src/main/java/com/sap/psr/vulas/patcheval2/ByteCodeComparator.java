...
public class ByteCodeComparator implements Runnable {
    ...
    public void run() {
        ...
        for (ConstructPathAssessment2 cpa2 : ar.getConstructPathAssessments()) {
            done_comparisons = 0;
            if (cpa2.getQnameInJar()
                    && BugLibManager.bytecodes.containsKey(
                            cpa2.getConstruct().concat(cpa2.getPath()))) {
                // first check that the construct is not in a nested classes otherwise the bytecode
                // comparison does not work
                // because the decompiler  in use (Procyon) does not keep the outer class in the
                // decompiled inner class
                // (i.e., the decompiled ClassA$ClassB.class contains public static class ClassB
                // instead of public static class ClassA$ClassB)
                // right now we check if a construct is in a nested class in the most dumb way: if
                // the string contains $
                if (!cpa2.getConstruct().contains("$")) {

                    ConstructBytecodeASTManager mgr =
                            BugLibManager.bytecodes.get(cpa2.getConstruct().concat(cpa2.getPath()));

                    // only compare if the number of comparisons tobedone is > than the one
                    // performed in the previous run (which was stored in the csv)
                    // this is done to avoid that comparisons are done for every run even if no
                    // equalities in the bytecode is found
                    // the check on the cardinality is an approx, however the number of sources
                    // equalities should never decreases thus this check seems adeguate
                    Integer comparisonsToBeDone = mgr.getLidsSize();

                    if (cpa2.getConstructType() != null
                            && (cpa2.getDoneComparisons() == null
                                    || !cpa2.getDoneComparisons().equals(comparisonsToBeDone))) {

                        log.info(
                                "["
                                        + comparisonsToBeDone
                                        + "] comparisons to be done for construct path "
                                        + cpa2.getConstruct()
                                        + " : "
                                        + cpa2.getPath()
                                        + " in archive ["
                                ...
                                BackendConnector.getInstance()
                                        .getAstForQnameInLib(
                                                ...
                                                ar.getGroup()
                                                        ...
                                                        + ar.getVersion()
                                                        + "/"
                                                        + cpa2.getConstructType().toString()
                                                        + "/"
                                                        + cpa2.getConstruct(),
                                                false,
                        ...
                        if (ast_current != null) {
                            ...
                            for (LibraryId l : mgr.getVulnLids()) {
                                ...
                                log.info(
                                        "Artifact ["
                                                + ar.toString()
                                                + "] vuln comparison number ["
                                                + done_comparisons
                                                + "/"
                                                + comparisonsToBeDone
                                                + "]");
                                // check if it is equal to bytecode of each c,p in lid found equal
                                // to vuln
                                ...
                                if (ast_lid != null) {
                                    ...
                                    String editV =
                                            BackendConnector.getInstance().getAstDiff(null, body);
                                    ...
                                    if (scV.getModifications().size() == 0) {
                                        ...
                                        log.info(
                                                "LID ["
                                                        + ar.toString()
                                                        + "] equal to vuln based on AST bytecode"
                                                        + " comparison with  ["
                                                        + l.toString()
                                                        + "]");
                                ...
                                if (ast_lid != null) {
                                    ...
                                    if (scV.getModifications().size() == 0) {
                                        ...
                                        log.info(
                                                "LID ["
                                                        + ar.toString()
                                                        + "] equal to fix based on AST bytecode"
                                                        + " comparison with  ["
                                                        + l.toString()
                                                        + "]");
                                        //	cpa2.addLibsSameBytecode(l);
