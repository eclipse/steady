            ...
            if (!processedPath.contains(path) && cpa.getType().equals("MOD")) {
                ...
                for (int j = i + 1; j < this.getConstructPathAssessments().size(); j++) {
                    ...
                    if (cpa2.getType().equals("MOD") && cpa2.getPath().equals(path)) {
                        ...
                        if (cpa2.dToV == 0) totV++;
                        //		    			else
                        //		    				ArtifactResult2.log.debug("Construct [" + cpa2.getConstruct() +
                        // "] not equal to Vulnerable");
                    }
                }
                ArtifactResult2.log.debug(
                        "Archive ["
        ...
        for (ConstructPathAssessment2 cpa : this.getConstructPathAssessments()) {
            if (cpa.getType().equals("MOD")) {
                ...
                if (cpa.dToF == 0) totF++;
                    ...
                    ArtifactResult2.log.debug(
                            "Construct [" + cpa.getConstruct() + "] not equal to Fixed");
            }
        }
        ArtifactResult2.log.debug(
                "Archive ["
    ...
    private boolean constructResultsConsistent() {
        ...
        for (ConstructPathAssessment2 cpa : this.constructPathAssessments) {
            if (cpa.getType().equals("MOD") && cpa.getdToV() == 0 && cpa.getdToF() != 0)
                ...
                // ArtifactResult2.log.warn("Found a construct equal both to VULNERABLE and to FIXED
                // for the same BUG, LIB [" + this.toString()+"]");
                return false;
            }
        }
        if (foundV && foundF) {
            //	ArtifactResult2.log.warn("Found a construct equal to VULNERABLE and another equal to
            // FIXED for the same BUG, LIB [" + this.toString()+"]");
            return false;
        } else return true;
    }
            ...
            for (ConstructPathAssessment2 cpa : this.getConstructPathAssessments()) {
                if (cpa.getPath().equals(path)) {
                    if (cpa.getType().equals("ADD") && cpa.getQnameInJar().equals(Boolean.FALSE)) {
                        fixed = false;
                        break;
                    } else if (cpa.getType().equals("ADD")
                            && cpa.getQnameInJar().equals(Boolean.TRUE)) fixed = true;
                    else if (cpa.getType().equals("DEL")
                            && cpa.getQnameInJar().equals(Boolean.TRUE)) {
                        fixed = false;
                        break;
                    } else if (cpa.getType().equals("DEL")
                            && cpa.getQnameInJar().equals(Boolean.FALSE)) fixed = true;
                }
            }
