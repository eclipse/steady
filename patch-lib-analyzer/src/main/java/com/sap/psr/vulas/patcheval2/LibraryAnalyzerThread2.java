...
public class LibraryAnalyzerThread2 implements Callable<List<ConstructPathLibResult2>> {
  ...
  /**
   * Constructor for LibraryAnalyzerThread2.
   *
   * @param _id a int.
   * @param methsConsMOD list of MOD constructs in the overall change list (i.e., consolidating
   *     construct,path changed over multiple commits) for current bug
  ...
  public List<ConstructPathLibResult2> call() throws Exception {
    ...
    if (!binAvailable) {
      ...
      if (c.size() > 0) {
        ...
        if (cids_array == null) {
          log.warn(
              "The intersection returned null (thus something went wrong in cia); the Jar for library Id ["
                  + l.toString()
            ...
            if (qnameInBin && sourcesAvailable) {
              ...
              if (mcCC.getChangeType().equals(ConstructChangeType.MOD)
                ...
                PythonConstructDigest pythonConstructDigest =
                    gson.fromJson(ast_lid, PythonConstructDigest.class);
                if (pythonConstructDigest != null) {
                  PythonConstructDigest vulnConstructDigest =
                      gson.fromJson(mcCC.getBuggyBody(), PythonConstructDigest.class);
                  PythonConstructDigest fixedConstructDigest =
                      gson.fromJson(mcCC.getFixedBody(), PythonConstructDigest.class);
      ...
      if (ad.size() > 0) {
        ...
        if (adcids_array == null) {
          log.warn(
              "The intersection returned null (thus something went wrong in cia); the artifact for library Id ["
                  + l.toString()
