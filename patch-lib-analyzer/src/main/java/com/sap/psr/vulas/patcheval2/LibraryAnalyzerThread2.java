...
public class LibraryAnalyzerThread2 implements Callable<List<ConstructPathLibResult2>> {
    ...
    public List<ConstructPathLibResult2> call() throws Exception {
        ...
        // doesArtifactExist works for both Java and Python as the called api (/artifacts/g/a/v)
        // does not use the package information for Python
        // doesArtifactExist sets the package to Jar; as a result it checks that a Jar exists for
        // java and that an artifact for the given gav exists for python (no matter which packaging)
            ...
            if (c.size() > 0) {
                ...
                if (cids_array == null) {
                    ...
                    for (OverallConstructChange mcCC : singleMethsConsCC) {
                        ...
                        // if sourcesAvailable is true, it also implies that
                        ...
                        if (qnameInBin && sourcesAvailable) {
                            ...
                            // In particular,
                            // for (a):  it should always be MOD at this point of the execution,
                            // since add and del are pre filtered out
                            // for (b) even if the overall change is a mod, we may not have the
                            // buggy and fixed
                            // asts: e.g., the fix includes 2 commit and the constructs is first
                            // DELETED and the
                            // ADDED, this means that overall is a MOD but we do not have asts (see
                            // CVE-2015-5254
                            // as an example)

                            if (mcCC.getChangeType().equals(ConstructChangeType.MOD)
                                    && (mcCC.getBuggyBody() != null
                                            || mcCC.getFixedBody() != null)) {
                                ...
                                if (sourcesQnameLib) {
                                    // GET
                                    // /constructs/{mvnGroup}/{artifact}/{version}/{type}/{qname}/
                                    // : AST_LID
                                    ast_lid =
                                            BackendConnector.getInstance()
                                                    .getAstForQnameInLib(
                                    ...
                                    if (ast_lid != null) {
                                        if (ast_lid.compareTo("") == 0) {
                                            // throw exception as this should never happen as we get
                                            // the ast only if the sources contain the qname!
                                            throw new Exception(
                                                    "Ast lid for qname ["
                                                            + mcCC.getConstructId().getQname()
                                                            + "]is empty");
                                            ...
                                            if (mcCC.getFixedBody() != null) {
                                                ...
                                                String editF =
                                                        BackendConnector.getInstance()
                                                                .getAstDiff(null, lidToFixedBody);
                                        ...
                                        // it should never happen that ast not available if we ask
                                        // for it
                                        log.error(
                                                "Got null for ast "
                                                        + qString
                                                        ...
                                                        + sourcesAvailable);
                                        throw new Exception(
                                                "AST_LID not available"); // happens when 500
                                    }
                                }
                            }
                            ...
                            if (mcCC.getChangeType().equals(ConstructChangeType.MOD)
                                ...
                                PythonConstructDigest pythonConstructDigest =
                                        (PythonConstructDigest)
                                                JacksonUtil.asObject(
                                                        ast_lid, PythonConstructDigest.class);
                        ...
                        //  LidResult2 lr = new LidResult2(l, timestamp, qnameInBin,
                        // sourcesAvailable, changesToV, changesToF, ast_lid);
            ...
            if (ad.size() > 0) {
                ...
                if (adcids_array == null) {
                    ...
                    for (OverallConstructChange adCc : addedDelMethsConsCC) {
                        ...
                        if (adCc != null
                                && adcids != null
                                && adcids.contains(adCc.getConstructId())) {
