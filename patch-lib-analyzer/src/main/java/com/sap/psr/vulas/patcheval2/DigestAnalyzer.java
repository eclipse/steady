...
public class DigestAnalyzer {
  ...
  /**
   * Constructor for DigestAnalyzer.
   *
      ...
      if (canAnalyze) {
        ...
        for (Bug b : bugsToAnalyze) {
          ...
          for (AffectedVersionSource s : AffectedVersionSource.values()) {
            AffectedLibrary[] al =
                BackendConnector.getInstance().getBugAffectedLibraries(b.getBugId(), s.toString());
          ...
          // consistently to VULN/FIXED, propagate it to SHA1 (meant to be used for tomcat >6 right
          // now)
            ...
            for (AffectedLibrary a : array) {
              ...
              (a.getLibraryId() != null
                  ...
                  (a.getLibraryId() != null
                      ...
                      && (l.getLibraryId()
                              .getArtifact()
                              .substring(0, l.getLibraryId().getArtifact().lastIndexOf("."))
                              .equals("org.apache")
                          || l.getLibraryId()
                              .getArtifact()
                              .substring(0, l.getLibraryId().getArtifact().lastIndexOf("."))
                              .equals("org.apache.tomcat")
                          || l.getLibraryId()
                              .getArtifact()
                              .substring(0, l.getLibraryId().getArtifact().lastIndexOf("."))
                              .equals("org.apache.catalina")
                          || l.getLibraryId()
                              .getArtifact()
                              .substring(0, l.getLibraryId().getArtifact().lastIndexOf("."))
                              .equals("com.springsource.org.apache")
                          || l.getLibraryId()
                              .getArtifact()
                              .substring(0, l.getLibraryId().getArtifact().lastIndexOf("."))
                              .equals("com.springsource.org.apache.tomcat"))
                      && l.getLibraryId()
                          .getArtifact()
                          .substring(
                              l.getLibraryId().getArtifact().lastIndexOf(".") + 1,
                              l.getLibraryId().getArtifact().length())
                          .equals(
                              a.getLibraryId()
                                  .getArtifact()
                                  .substring(
                                      a.getLibraryId().getArtifact().lastIndexOf("-") + 1,
                                      a.getLibraryId().getArtifact().length()))
                      && a.getLibraryId().getVersion().equals(version))
                  ||
          ...
          if (toUpload) {
            log.info(
                "Creating Json for PROPAGATE_MANUAL for digest ["
                    + digest
                    + "] and bug ["
                    + b.getBugId()
                    + "]");
            ...
            BackendConnector.getInstance()
                .uploadPatchEvalResults(b.getBugId(), sourceResult.toString(), "PROPAGATE_MANUAL");
          }
        }
      }
  ...
  private JsonObject createJsonResult(Library _lib, Boolean _affected) {
    ...
    result.addProperty(
        "explanation",
        "Generated automatically by DigestAnalyzer on "
            + new SimpleDateFormat("yyyy/MM/dd HH:mm:ss").format(new Date()));

    if (_affected != null) result.addProperty("affected", _affected);
  ...
  public static void main(String[] _args) {
    ...
    try {
      ...
      if (cmd.hasOption("digest")) {
        ...
        log.info(
            "Running patcheval to assess all bugs of digest["
                + digest
