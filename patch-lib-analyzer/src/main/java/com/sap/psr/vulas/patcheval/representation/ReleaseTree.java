...
public class ReleaseTree {
        ...
        if ((lv == null || a.compareTo(lv) >= 0) && (ff == null || b.compareTo(ff) <= 0)) {
            ...
            if (tot == 0)
                ...
                ReleaseTree.log.debug(
                        "["
                                + tot
                                + "] constructs analyzed but no intersection found between"
                                + " archives ["
                                + a.toString()
                                + "] and ["
                                + b.toString()
    ...
    private HashMap<ArtifactResult2, Intersection2> getPreIs(Intersection2 i) {
        ...
        // add node if it's a minor maintenance release or in the same maintenance (which means all
        // its build releases are affected as they do not contain an intersection)
        ...
        if (this.node.compareVersion(i.getFrom()) <= 0
                && ((i.getFrom().getMaintenanceRelease().equals(i.getTo().getMaintenanceRelease())
                                && this.node
                                        ...
                                        .equals(i.getTo().getMaintenanceRelease()))
                        || !(i.getFrom()
                                .getMaintenanceRelease()
                                .equals(i.getTo().getMaintenanceRelease())))) a.put(this.node, i);
    ...
    private void getBeforeIntersection(HashMap<ArtifactResult2, Intersection2> arbeforeIS) {
        ...
        // no need to navigate the build as it's not branching on its own and the intersection(if
        // any) was already found in this.getIS()
        //		if(this.build!=null)
