...
/**
 * This class holds all artifact results for the same minor release, e.g., possible roots are 1.0.0,
 * 1.1.0,.. ,2.3.0,.. Each node contains its next build and maintenance release (if any), e.g., for
 * node 2.3.0 - build release 2.3.0.1 / - maintenance release 2.3.1
 */
public class ReleaseTree {
  ...
  public String toString() {
    ...
    if (this.build != null) sb.append("right children : ").append(this.build.toString());
    if (this.maintenance != null) sb.append("left children : ").append(this.maintenance.toString());
    return sb.toString();
  }

  /**
   * This method adds an ArtifactResult2 r to a release tree. If the version major.minor.maintenance
   * of r is the same than the current node, it navigates the build ReleaseTree and inserts r as
   * parent of the subtree(if any) having a greater major.minor.maintenance.build; If the version
   * major.minor.maintenance of r is greater then the current node, it navigates the maintenance
   * ReleaseTree and inserts r as parent of the subtree (if any) having a greater
   * major.minor.maintenance;
   *
   * <p>The version comparison is done using the method compareVersion of class ArtifactResult2
   * which compares ArtifactResult2 based on the following priorities 1) compare version numbers of
   * the form major.minor[.maintenance[.build]], if only digits are contained 2) compare timestap
   * (of publication in maven central) 3) alphanumerical version comparison
   *
  ...
  /**
   * Getter for the field <code>build</code>.
   *
   * @return a {@link com.sap.psr.vulas.patcheval.representation.ReleaseTree} object.
   */
  ...
  /**
   * Setter for the field <code>build</code>.
   *
  ...
  /**
   * This method returns the last vulnerable artifact. If both build and maintenance release exist,
   * then LV is searched first among the build releases within the same maintenance (that means in
   * root.build); then among the maintenance releases.
   *
  ...
  private ArtifactResult2 getLV() {
    ...
    if (this.build != null) {
      ...
      if (lastVuln != null) return lastVuln;
    }
  ...
  private ArtifactResult2 getLVInMaintainance() {
    ...
    if (lastVuln == null) lastVuln = this.isLV();
    return lastVuln;
  }

  private ArtifactResult2 isLV() {
    if (this.node.getSourceAvailable() && this.node.isEqualToV()) {
      ReleaseTree.log.debug(
          "LID ["
              + this.node.toString()
              + "] equal to vuln with confidence ["
              + this.node.getVConfidence()
              + "]");
      return this.node;
    }
    return null;
  ...
  /**
   * This method returns the first fixed artifact. It starts from the root, and navigates the
   * maintenance releases until it finds the build releases if it finds a before ff, then it returns
   * null (a is not fixed)
   *
  ...
  private ArtifactResult2 getFFinBuild() {
    ...
    if (firstFixed != null) return firstFixed;
  ...
  //		if(!this.node.getMaintenanceRelease().equals(a.node.getMaintenanceRelease()) &&
  // a.maintenance!=null){
  //			return this.getFF(a.maintenance,firstFixed);
  //		}
  //		else if(this.node.getMaintenanceRelease().equals(a.node.getMaintenanceRelease()) &&
  // a.build!=null){
  ...
  private ArtifactResult2 isFF() {
    if (this.node.getSourceAvailable() && this.node.isEqualToF()) { // && firstFixed==null){
      ReleaseTree.log.debug(
          "LID ["
              + this.node.toString()
              + "] equal to fixed with confidence ["
              + this.node.getFConfidence()
              + "]");
      return this.node;
    } else return null;
  }
  ...
  private Intersection2 getIS() {
    ...
    if (this.build != null) {
      ...
      if (firstF == null) firstF = this.getFFInMaintainance();
  ...
  private Intersection2 getBuildIS(ArtifactResult2 lv, ArtifactResult2 ff) {
    ...
    if (this.build != null) {
      ...
      if (is != null && this.build.getBuildIS(lv, ff) == null) return is;
  ...
  private Intersection2 getMaintenanceIS(ArtifactResult2 lv, ArtifactResult2 ff) {
    ...
    if (this.maintenance != null) {
      ...
      if (is != null && this.maintenance.getMaintenanceIS(lv, ff) == null) return is;
      else return this.maintenance.getMaintenanceIS(lv, ff);
    }
    return is;
  }
  ...
  //	 *
  //	 * @return the Intersection if it's unique per build or per maintenance; null if it doesn't
  // exist
  //	 */
  ...
  private Intersection2 getIS(
      ArtifactResult2 a, ArtifactResult2 b, ArtifactResult2 lv, ArtifactResult2 ff) {
    ...
    if ((lv == null || a.compareTo(lv) >= 0) && (ff == null || b.compareTo(ff) <= 0)) {
        ...
        if (cpa.getQnameInJar() && cpa.getType().equals("MOD") && cpa.getdToF() > -1) {
          ...
          for (ConstructPathAssessment2 cpa2 : b.getConstructPathAssessments()) {
            if (cpa.getConstruct().equals(cpa2.getConstruct())
                ...
                && cpa.getPath().equals(cpa2.getPath())
                && cpa2.getQnameInJar()
                && cpa2.getdToF() > -1) {
      ...
      if (tot == 0)
        ReleaseTree.log.debug(
            "None of the MOD constructs are part of archive [" + a.toString() + "]");
      else if (ixc == 0)
        ReleaseTree.log.debug(
            "["
                + tot
                + "] constructs analyzed but no intersection found between archives ["
                + a.toString()
                + "] and ["
                + b.toString()
                + "]");
        ...
        ReleaseTree.log.debug(
            "Found intersection between archives ["
                + a.toString()
                + "] and ["
                + b.toString()
                + "] with confidence ["
  ...
  /**
   * This method computes all artifactResults2 of a releaseTree which are before an intersection
   * (considering both maintenance or build branches)
   *
  ...
  public HashMap<ArtifactResult2, Intersection2> getBeforeIntersection() {
    HashMap<ArtifactResult2, Intersection2> arbeforeIS =
        new HashMap<ArtifactResult2, Intersection2>();
  ...
  private HashMap<ArtifactResult2, Intersection2> getPreIs(Intersection2 i) {
    ...
    // add node if it's a minor maintenance release or in the same maintenance (which means all its
    // build releases are affected as they do not contain an intersection)
    //	if(this.node.compareVersion(i.getFrom())<=0 ||
    // (this.node.getMaintenanceRelease().equals(i.getFrom().getMaintenanceRelease())&&
    // !this.node.getMaintenanceRelease().equals(i.getTo().getMaintenanceRelease())))
    ...
    if (this.node.compareVersion(i.getFrom()) <= 0
        && ((i.getFrom().getMaintenanceRelease().equals(i.getTo().getMaintenanceRelease())
            ...
            || !(i.getFrom().getMaintenanceRelease().equals(i.getTo().getMaintenanceRelease()))))
      a.put(this.node, i);
    if (this.build != null) a.putAll(this.build.getPreIs(i));
    if (this.maintenance != null) a.putAll(this.maintenance.getPreIs(i));
    return a;
  }

  /**
   * This method populates the hashmap passed as argument with all the pairs artifactresult and
   * intersection they preceed. The method is called recoursively, at each iteration it looks for
   * the intersection for the root node either in the maintenance or build branch and adds it to the
   * hashmap (if any)
   *
   * @param arbeforeIS: the hashmap to be populated with all the artifactresult and intersection
   *     they preceed found
   */
  private void getBeforeIntersection(HashMap<ArtifactResult2, Intersection2> arbeforeIS) {
    ...
    // no need to navigate the build as it's not branching on its own and the intersection(if any)
    // was already found in this.getIS()
    //		if(this.build!=null)
    //			this.build.getBeforeIntersection(arbeforeIS);

    if (this.maintenance != null) this.maintenance.getBeforeIntersection(arbeforeIS);
  }
  ...
  public HashMap<ArtifactResult2, Intersection2> getPostIntersection() {
    HashMap<ArtifactResult2, Intersection2> arpostIS =
        new HashMap<ArtifactResult2, Intersection2>();
  ...
  private HashMap<ArtifactResult2, Intersection2> getPostIs(Intersection2 i) {
    ...
    if (this.node.compareVersion(i.getTo()) >= 0
        && ((i.getFrom().getMaintenanceRelease().equals(i.getTo().getMaintenanceRelease())
            ...
            || !(i.getFrom().getMaintenanceRelease().equals(i.getTo().getMaintenanceRelease()))))
      a.put(this.node, i);
    if (this.build != null) a.putAll(this.build.getPostIs(i));
    if (this.maintenance != null) a.putAll(this.maintenance.getPostIs(i));
    return a;
  }
  ...
  private void getPostIntersection(HashMap<ArtifactResult2, Intersection2> arpostIS) {
    ...
    if (this.build != null) this.build.getPostIntersection(arpostIS);
    if (this.maintenance != null) this.maintenance.getPostIntersection(arpostIS);
  }
  //	/**
  //	 * This method looks for an intersection among the build releases of a or maintenance
  //	 *
  ...
  public ArtifactResult2 getLastFirstFixed() {
    if (this.isFF() != null
        && (this.build == null || this.build.getLV() == null)
        && (this.maintenance == null || this.maintenance.getLVInMaintainance() == null))
      return this.node;
    ...
    if (this.maintenance != null) return this.maintenance.getLastFirstFixed();
    else {
      return null;
    }
    //	if()
  ...
  private void getEqualsV(List<ArtifactResult2> listOfEqualsV) {
    ...
    if (this.build != null) this.build.getEqualsV(listOfEqualsV);
    if (this.maintenance != null) this.maintenance.getEqualsV(listOfEqualsV);
  }
  ...
  private void getEqualsF(List<ArtifactResult2> listOfEqualsF) {
    ...
    if (this.build != null) this.build.getEqualsF(listOfEqualsF);
    if (this.maintenance != null) this.maintenance.getEqualsF(listOfEqualsF);
  }
  ...
  public HashMap<ArtifactResult2, ArtifactResult2> getMinorV() {
    HashMap<ArtifactResult2, ArtifactResult2> arwithLV =
        new HashMap<ArtifactResult2, ArtifactResult2>();
    ...
    if (a != null && this.node != a && !this.node.isEqualToV() && this.node.compareTo(a) < 0) {
      ReleaseTree.log.debug(
          "LID ["
              ...
              + "] is minor than ["
              + a.toString()
              + "] which is vuln with confidence ["
              + a.getVConfidence()
              + "]");
      arwithLV.put(this.node, a);
    }
    if (this.build != null) this.build.getMinorV(arwithLV);
    if (this.maintenance != null) this.maintenance.getMinorV(arwithLV);
  }
  ...
  public HashMap<ArtifactResult2, ArtifactResult2> getMajorF() {
    HashMap<ArtifactResult2, ArtifactResult2> arwithFF =
        new HashMap<ArtifactResult2, ArtifactResult2>();
    ...
    if (a != null && this.node != a && !this.node.isEqualToF() && this.node.compareTo(a) > 0) {
      ReleaseTree.log.debug(
          "LID ["
              ...
              + "] is major than ["
              + a.toString()
              + "] which is fixed in build with confidence ["
              + a.getFConfidence()
              + "]");
      arwithFF.put(this.node, a);
    } else if (b != null
        && this.node != b
        && !this.node.isEqualToF()
        && this.node.compareTo(b) > 0) {
      ReleaseTree.log.debug(
          "LID ["
              + this.node.toString()
              + "] is major than ["
              + b.toString()
              + "which is fixed in maintenance with confidence"
              + b.getFConfidence()
              + "]");
      arwithFF.put(this.node, b);
    }
    if (this.build != null) {
      if (a != null) this.build.getMajorF(arwithFF, a);
      else this.build.getMajorF(arwithFF, b);
    }
    if (this.maintenance != null) this.maintenance.getMajorF(arwithFF, b);
  }
  ...
  public List<ArtifactResult2> getNodes() {
    ...
    list.add(this.node);
    if (this.build != null) list.addAll(this.build.getNodes());
    if (this.maintenance != null) list.addAll(this.maintenance.getNodes());
    return list;
  }
}
