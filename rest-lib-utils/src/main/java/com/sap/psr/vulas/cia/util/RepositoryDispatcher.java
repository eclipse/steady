...
public class RepositoryDispatcher implements RepositoryWrapper {
    ...
    public Set<Artifact> getAllArtifactVersions(
        ...
        for (RepositoryWrapper dv : loader) {
            if (dv.isConfigured()) {
                try {
                    partial_result =
                            dv.getAllArtifactVersions(group, artifact, classifier, packaging);
    ...
    public Artifact getArtifactVersion(
        ...
        for (RepositoryWrapper dv : loader) {
            if (dv.isConfigured()
                ...
                try {
                    result =
                            dv.getArtifactVersion(
                                    group, artifact, version, classifier, packaging, null);
                    if (result != null) break;
                } catch (NotImplementedException e) {
    ...
    public Path downloadArtifact(Artifact a) throws Exception {
        ...
        if (a.isCached()) {
            ...
            try {
                ...
                e.printStackTrace();
                log.error(
                        "Exception when opening archive file ["
                                + p.toFile()
                                + "]: "
                                + e.getMessage());
    ...
    private Path downloadArtifactFile(Artifact a) throws Exception {
        ...
        for (RepositoryWrapper dv : loader) {
            if (dv.isConfigured()
                    && (dv.getSupportedLanguages().contains(a.getProgrammingLanguage()))) {
                try {
