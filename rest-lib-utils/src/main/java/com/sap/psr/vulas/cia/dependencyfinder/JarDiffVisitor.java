...
public class JarDiffVisitor extends VisitorBase {
        ...
        if (_classfile.isInterface()) {
            ...
            while (i.hasNext()) {
                c.addRelates(
                        "extends",
                        new ConstructId(
                                ProgrammingLanguage.JAVA, ConstructType.INTF, i.next().toString()));
            }
        } else {
    ...
    public void visitClassDifferences(ClassDifferences differences) {
        ...
        if (differences.isModified()) {
            ClassDiffVisitor visitor =
                    new ClassDiffVisitor(this.getClass(differences.getNewClass()));
