...
public class MavenCentralWrapper implements RepositoryWrapper {
    ...
    private MavenVersionsSearch getFromMavenCentral(Map<String, String> _params)
        ...
        for (Integer i = 1; i < this.mavenCentralRetryCount + 1; i++) {
            ...
            try {
                responseEntity =
                        rest_template.getForEntity(baseUrl, MavenVersionsSearch.class, _params);
                break;
    ...
    private Set<Artifact> getArtifactVersions(
        ...
        try {
            ...
            // NOTE: we do not use the packaging in the query to maven as e.g. jackson-databind
            // 2.9.5 has packaging bundle instead of jar
            // same for classifier
            ...
            if (search != null) {
                ...
                if (!search.getResponse().getDocs().isEmpty() && latest) {
                    final TreeSet<ResponseDoc> filtered =
                            this.filter(
                                    search.getResponse().getSortedDocs(), classifier, packaging);
                ...
                // Client only wants versions GT x
                    ...
                    for (ResponseDoc d : search.getResponse().getSortedDocs()) {
                        if (d.getTimestamp() > timestamp
                                && d.availableWith(classifier, packaging)) {
    ...
    public Artifact getArtifactVersion(
        ...
        Set<Artifact> a =
                this.getArtifactVersions(
                        group, artifact, version, false, null, classifier, packaging);
        List<Artifact> list = new ArrayList<Artifact>(a);
        if (list.size() > 1) {
            throw new Exception(
    ...
    public Set<Artifact> getArtifactForClass(
        ...
        if (search != null) {
            for (ResponseDoc d :
                    this.filter(
                            search.getResponse().getSortedDocs(),
                            classifierFilter,
                            packagingFilter)) all.add(d.toArtifact());
        }
        return all;
    }
