...
/**
 * This is a standard assertion builder implementation for the https token as specified by the ws
 * security policy 1.2 specification. In order for this builder to be used it is required that the
 * security policy namespace uri is {@link SP12Constants#SP_NS} The builder will handle
 *
 * <ul>
 *   <li><code>HttpBasicAuthentication</code>
 *   <li><code>HttpDigestAuthentication</code>
 *   <li><code>RequireClientCertificate</code>
 * </ul>
 *
    ...
    if (consts.getVersion() == SPConstants.Version.SP_V11) {
      ...
      if (polEl == null) {
        ...
        if (child != null) {
          ...
          } else if (SP12Constants.HTTP_DIGEST_AUTHENTICATION.equals(
              DOMUtils.getElementQName(child))) {
            httpsToken.setHttpDigestAuthentication(true);
          } else if (SP12Constants.REQUIRE_CLIENT_CERTIFICATE.equals(
              DOMUtils.getElementQName(child))) {
