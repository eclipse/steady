...
public abstract class AbstractBindingPolicyValidator implements BindingPolicyValidator {
  ...
  protected boolean validateLayout(
      boolean laxTimestampFirst, boolean laxTimestampLast, List<WSSecurityEngineResult> results) {
  ...
  /**
   * Check to see if a signature was applied before encryption. Note that results are stored in the
   * reverse order.
   */
  ...
  /**
   * Check to see if encryption was applied before signature. Note that results are stored in the
   * reverse order.
   */
  ...
  protected boolean checkDerivedKeys(
    ...
    if (tokenWrapper instanceof EncryptionToken && !hasDerivedKeys && !encryptedResults.isEmpty()) {
      ...
      return false;
        ...
        && !hasDerivedKeys
        && !(signedResults.isEmpty() || encryptedResults.isEmpty())) {
      return false;
    }
    return true;
  }
