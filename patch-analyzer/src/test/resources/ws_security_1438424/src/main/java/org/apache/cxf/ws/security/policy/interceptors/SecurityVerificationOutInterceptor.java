...
public class SecurityVerificationOutInterceptor extends AbstractPhaseInterceptor<SoapMessage> {
  ...
  /**
   * Checks if some security assertions are specified without binding assertion and cannot be
   * fulfilled. Throw PolicyException in this case
   *
   * @param message
  ...
  private void checkAssertion(Collection<AssertionInfo> ais, QName assertion) {
    if ((ais != null) && (!ais.isEmpty())) {
      String error =
          String.format(
              "%s assertion cannot be fulfilled without binding. "
