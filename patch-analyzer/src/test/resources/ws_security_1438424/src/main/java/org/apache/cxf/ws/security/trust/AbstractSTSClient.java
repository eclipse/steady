...
public abstract class AbstractSTSClient implements Configurable, InterceptorProvider {
  ...
  /**
   * Sets the WS-P policy that is applied to communications between this client and the remote
   * server if no value is supplied for {@link #setWsdlLocation(String)}.
   *
   * <p>Accepts {@link Policy} or {@link Element} as input.
   *
  ...
  public void setPolicy(Object newPolicy) {
    if (newPolicy instanceof Policy) {
      ...
      throw new IllegalArgumentException(
          "Unsupported policy object.  Type must be "
  ...
  /**
   * Indicate whether to use the signer's public X509 certificate for the subject confirmation key
   * info when creating a RequestsSecurityToken message. If the property is set to 'false', only the
   * public key value will be provided in the request. If the property is set to 'true' the complete
   * certificate will be sent in the request.
   *
   * <p>Note: this setting is only applicable for assertions that use an asymmetric proof key
   */
  ...
  protected void createClient() throws BusException, EndpointException {
    ...
    if (wsdlLocation != null) {
      ...
      Endpoint endpoint =
          STSUtils.createSTSEndpoint(
              bus, namespace, null, location, soapVersion, policy, endpointName);
      ...
      Object o =
          boi.getOperationInfo()
              ...
              .getExtensionAttribute(
                  new QName("http://www.w3.org/2007/05/addressing/metadata", "Action"));
  ...
  protected STSResponse issue(
      String appliesTo, String action, String requestType, String binaryExchange) throws Exception {
    createClient();
  ...
  protected Element getDelegationSecurityToken(Object delegationObject) throws Exception {
    if (delegationObject != null) {
      ...
      if (isString || isElement || isCallbackHandler) {
        if (isString) {
          final Document doc = DOMUtils.readXml(new StringReader((String) delegationObject));
  ...
  public STSResponse renew(SecurityToken tok) throws Exception {
    ...
    if (isSecureConv) {
      client
          .getRequestContext()
          .put(SoapBindingConstants.SOAP_ACTION, namespace + "/RST/SCT/Renew");
    } else {
  ...
  protected STSResponse cancel(SecurityToken token) throws Exception {
    ...
    if (isSecureConv) {
      client
          .getRequestContext()
          .put(SoapBindingConstants.SOAP_ACTION, namespace + "/RST/SCT/Cancel");
    } else {
  ...
  protected byte[] decryptKey(Element child) throws TrustException, WSSecurityException {
    ...
    if (encryptionAlgorithm != null && encryptionAlgorithm.endsWith("spnego#GSS_Wrap")) {
      // Get the CipherValue
      Element tmpE = WSSecurityUtil.getDirectChildElement(child, "CipherData", WSConstants.ENC_NS);
      byte[] cipherValue = null;
      if (tmpE != null) {
        tmpE = WSSecurityUtil.getDirectChildElement(tmpE, "CipherValue", WSConstants.ENC_NS);
        if (tmpE != null) {
      ...
      try {
        ...
        List<WSSecurityEngineResult> result = proc.handleToken(child, data, docInfo);
  ...
  protected Crypto createCrypto(boolean decrypt) throws IOException, WSSecurityException {
    Crypto crypto =
        (Crypto) getProperty(SecurityConstants.STS_TOKEN_CRYPTO + (decrypt ? ".decrypt" : ""));
    if (crypto != null) {
      return crypto;
    }
