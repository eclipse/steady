...
/**
 * Base class providing an extensibility point for populating javax.security.auth.Subject from a
 * current UsernameToken.
 *
 * <p>WSS4J requires a password for validating digests which may not be available when external
 * security systems provide for the authentication. This class implements WSS4J Processor interface
 * so that it can delegate a UsernameToken validation to an external system.
 *
 * <p>In order to handle digests, this class currently creates a new WSS4J Security Engine for every
 * request. If clear text passwords are expected then a supportDigestPasswords boolean property with
 * a false value can be used to disable creating security engines.
 *
 * <p>Note that if a UsernameToken containing a clear text password has been encrypted then an
 * application is expected to provide a password callback handler for decrypting the token only.
 */
public abstract class AbstractUsernameTokenAuthenticatingInterceptor extends WSS4JInInterceptor {
  ...
  /**
   * Create a Subject representing a current user and its roles. This Subject is expected to contain
   * at least one Principal representing a user and optionally followed by one or more principal
   * Groups this user is a member of. It will also be available in doCreateSecurityContext.
   *
   * @param name username
   ...
   */
  protected abstract Subject createSubject(
      String name, String password, boolean isDigest, String nonce, String created)
      throws SecurityException;
  ...
  protected class CustomValidator extends UsernameTokenValidator {
    ...
    protected void verifyCustomPassword(
        org.apache.ws.security.message.token.UsernameToken usernameToken, RequestData data)
        throws WSSecurityException {
