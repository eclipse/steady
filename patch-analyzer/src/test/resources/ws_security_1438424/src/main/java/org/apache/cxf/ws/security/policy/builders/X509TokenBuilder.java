...
public class X509TokenBuilder implements AssertionBuilder<Element> {
  ...
  public Assertion build(Element element, AssertionBuilderFactory factory)
    ...
    if (policyElement != null) {
      ...
      } else if (DOMUtils.getFirstChildWithName(
              policyElement, SP12Constants.REQUIRE_IMPLIED_DERIVED_KEYS)
          != null) {
        x509Token.setImpliedDerivedKeys(true);
      } else if (DOMUtils.getFirstChildWithName(
              policyElement, SP12Constants.REQUIRE_EXPLICIT_DERIVED_KEYS)
          != null) {
  ...
  private void processAlternative(
      List<Assertion> assertions, X509Token parent, SPConstants consts) {
    QName name;
