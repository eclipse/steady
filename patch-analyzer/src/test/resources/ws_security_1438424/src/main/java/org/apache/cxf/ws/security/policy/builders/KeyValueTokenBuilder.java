...
public class KeyValueTokenBuilder implements AssertionBuilder<Element> {
  ...
  public Assertion build(Element element, AssertionBuilderFactory factory) {
    ...
    SPConstants consts =
        MS_NS.equals(element.getNamespaceURI()) ? SP11Constants.INSTANCE : SP12Constants.INSTANCE;
    ...
    String attribute =
        element.getAttributeNS(element.getNamespaceURI(), SPConstants.ATTR_INCLUDE_TOKEN);
    ...
    if (StringUtils.isEmpty(attribute)) {
      attribute =
          element.getAttributeNS(
              SP11Constants.INSTANCE.getNamespace(), SPConstants.ATTR_INCLUDE_TOKEN);
    }
    if (!StringUtils.isEmpty(attribute)) {
    ...
    if (polEl == null) {
      throw new IllegalArgumentException("sp:KeyValueToken/wsp:Policy must have a value");
    }
