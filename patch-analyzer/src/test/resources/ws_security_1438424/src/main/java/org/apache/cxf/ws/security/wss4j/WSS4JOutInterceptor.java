...
public class WSS4JOutInterceptor extends AbstractWSS4JInterceptor {
  ...
  /**
   * Property name for a map of action IDs ({@link Integer}) to action class names. Values can be
   * either {@link Class}) or Objects - * implementing {@link Action}.
   */
  ...
  private static final Logger TIME_LOG =
      LogUtils.getL7dLogger(
          WSS4JOutInterceptor.class, null, WSS4JOutInterceptor.class.getName() + "-Time");
  ...
  /**
   * Enable or disable mtom with WS-Security. By default MTOM is disabled as attachments would not
   * get encrypted or be part of the signature.
   *
   * @param mtomEnabled
   */
  ...
  final class WSS4JOutInterceptorInternal implements PhaseInterceptor<SoapMessage> {
    ...
    public void handleMessage(SoapMessage mc) throws Fault {
      ...
      /**
       * There is nothing to send...Usually happens when the provider needs to send a HTTP 202
      ...
      try {
        ...
        doSenderAction(
            ...
            reqData,
            actions,
            Boolean.TRUE.equals(getProperty(mc, org.apache.cxf.message.Message.REQUESTOR_ROLE)));
    ...
    private void configureActions(
        SoapMessage mc, boolean doDebug, SoapVersion version, WSSConfig config) {

      final Map<Integer, Object> actionMap =
          CastUtils.cast((Map<?, ?>) getProperty(mc, WSS4J_ACTION_MAP));
      if (actionMap != null) {
        for (Map.Entry<Integer, Object> entry : actionMap.entrySet()) {
          ...
          try {
            if (entry.getValue() instanceof Class<?>) {
              removedAction =
                  config.setAction(entry.getKey().intValue(), (Class<?>) entry.getValue());
            } else if (entry.getValue() instanceof Action) {
              removedAction =
                  config.setAction(entry.getKey().intValue(), (Action) entry.getValue());
            } else {
              throw new SoapFault(new Message("BAD_ACTION", LOG), version.getReceiver());
            }
