...
/**
 * Utility to enable the checking of WS-Security signature / WS-Security encryption coverage based
 * on the results of the WSS4J signature/encryption processor.
 */
public final class CryptoCoverageUtil {
  ...
  /**
   * Inspects the signed and encrypted content in the message and accurately resolves encrypted and
   * then signed elements in {@code signedRefs}. Entries in {@code signedRefs} that correspond to an
   * encrypted element are resolved to the decrypted element and added to {@code signedRefs}. The
   * original reference to the encrypted content remains unaltered in the list to allow for matching
   * against a requirement that xenc:EncryptedData and xenc:EncryptedKey elements be signed.
   *
   ...
   */
  public static void reconcileEncryptedSignedRefs(
      final Collection<WSDataRef> signedRefs, final Collection<WSDataRef> encryptedRefs) {
   ...
   *
   * @param soapBody the SOAP body element
   * @param refs the refs to the data extracted from the signature/encryption
   * @param type the type of cryptographic coverage to check for
   * @param scope the scope of the cryptographic coverage to check for, defaults to element
   * @throws WSSecurityException if there is an error evaluating the coverage or the body is not
   *     covered by the signature/encryption.
   */
  public static void checkBodyCoverage(
      Element soapBody, final Collection<WSDataRef> refs, CoverageType type, CoverageScope scope)
      throws WSSecurityException {
    if (!CryptoCoverageUtil.matchElement(refs, type, scope, soapBody)) {
      throw new WSSecurityException(
          "The "
              + getCoverageTypeString(type)
              + " does not cover the required elements (soap:Body).");
    }
  }

  /**
   * Checks that the references provided refer to the required signed/encrypted SOAP header
   * element(s) matching the provided name and namespace. If {@code name} is null, all headers from
   * {@code namespace} are inspected for coverage.
   *
   * @param soapHeader the SOAP header element
   * @param refs the refs to the data extracted from the signature/encryption
   * @param namespaces the namespace of the header(s) to check for coverage
   * @param name the local part of the header name to check for coverage, may be null
   * @param type the type of cryptographic coverage to check for
   * @param scope the scope of the cryptographic coverage to check for, defaults to element
   * @throws WSSecurityException if there is an error evaluating the coverage or a header is not
   *     covered by the signature/encryption.
   */
  ...
  /**
   * Checks that the references provided refer to the required signed/encrypted elements as defined
   * by the XPath expression in {@code xPath}.
   *
   * @param soapEnvelope the SOAP Envelope element
   * @param refs the refs to the data extracted from the signature/encryption
   * @param namespaces the prefix to namespace mapping, may be {@code null}
   * @param xPath the XPath expression
   * @param type the type of cryptographic coverage to check for
   * @param scope the scope of the cryptographic coverage to check for, defaults to element
   * @throws WSSecurityException if there is an error evaluating an XPath or an element is not
   *     covered by the signature/encryption.
   */
  public static void checkCoverage(
    ...
    CryptoCoverageUtil.checkCoverage(
        soapEnvelope, refs, namespaces, Arrays.asList(xPath), type, scope);
  }

  /**
   * Checks that the references provided refer to the required signed/encrypted elements as defined
   * by the XPath expressions in {@code xPaths}.
   *
   * @param soapEnvelope the SOAP Envelope element
   * @param refs the refs to the data extracted from the signature/encryption
   * @param namespaces the prefix to namespace mapping, may be {@code null}
   * @param xPaths the collection of XPath expressions
   * @param type the type of cryptographic coverage to check for
   * @param scope the scope of the cryptographic coverage to check for, defaults to element
   * @throws WSSecurityException if there is an error evaluating an XPath or an element is not
   *     covered by the signature/encryption.
  ...
  /**
   * Determines if {@code signedRef} points to the encrypted content represented by {@code
   * encryptedRef} using the following algorithm.
   *
   * <ol>
   *   <li>Check that the signed content is an XML Encryption element.
   *   <li>Check that the reference Ids of the signed content and encrypted content (not the
   *       decrypted version of the encrypted content) match. Check that the reference Id of the
   *       signed content matches the reference Id of the encrypted content prepended with a #.
   *   <li>Check for other Id attributes on the signed element that may match the referenced
   *       identifier for the encrypted content. This is a workaround for WSS-242.
   * </ol>
   *
  ...
  private static boolean isSignedEncryptionRef(WSDataRef encryptedRef, WSDataRef signedRef) {
    ...
    if (!WSConstants.ENC_NS.equals(signedRef.getProtectedElement().getNamespaceURI())) {
      return false;
    }
    ...
    String wsuId = signedRef.getProtectedElement().getAttributeNS(WSConstants.WSU_NS, "Id");

    if (signedRef.getWsuId().equals(wsuId) || signedRef.getWsuId().equals("#" + wsuId)) {
      return true;
    }
  ...
  private static boolean matchElement(
      Collection<WSDataRef> refs, CoverageType type, CoverageScope scope, Element el) {
