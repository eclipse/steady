...
/**
 * This utility extends the CryptoCoverageChecker to provide an easy way to check to see if the SOAP
 * (1.1 + 1.2) Body was signed and/or encrypted, if the Timestamp was signed, and if the
 * WS-Addressing ReplyTo and FaultTo headers were signed.
 *
 * <p>The default configuration is that the SOAP Body, Timestamp must be signed, and WS-Addressing
...
public class DefaultCryptoCoverageChecker extends CryptoCoverageChecker {
  ...
  /**
   * Creates a new instance. Enforces that the SOAP Body, Timestamp, and WS-Addressing ReplyTo and
   * FaultTo headers must be signed (if they exist in the message payload).
   */
  ...
  public final void setSignTimestamp(boolean signTimestamp) {
    ...
    XPathExpression soap11Expression =
        new XPathExpression(
            "/soapenv:Envelope/soapenv:Header/wsse:Security/wsu:Timestamp", CoverageType.SIGNED);
    XPathExpression soap12Expression =
        new XPathExpression(
  ...
  public final void setEncryptBody(boolean encryptBody) {
    ...
    XPathExpression soap11Expression =
        new XPathExpression(
            "/soapenv:Envelope/soapenv:Body", CoverageType.ENCRYPTED, CoverageScope.CONTENT);
    XPathExpression soap12Expression =
        new XPathExpression(
            "/soapenv12:Envelope/soapenv12:Body", CoverageType.ENCRYPTED, CoverageScope.CONTENT);
  ...
  public final void setSignAddressingHeaders(boolean signAddressingHeaders) {
    ...
    XPathExpression soap11Expression =
        new XPathExpression("/soapenv:Envelope/soapenv:Header/wsa:ReplyTo", CoverageType.SIGNED);
    XPathExpression soap11Expression2 =
        new XPathExpression("/soapenv:Envelope/soapenv:Header/wsa:FaultTo", CoverageType.SIGNED);
    XPathExpression soap12Expression =
        new XPathExpression(
            "/soapenv12:Envelope/soapenv12:Header/wsa:ReplyTo", CoverageType.SIGNED);
