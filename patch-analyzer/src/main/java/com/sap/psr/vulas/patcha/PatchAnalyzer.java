...
public class PatchAnalyzer {
   ...
   * @throws com.sap.psr.vulas.vcs.NoRepoClientException if no VCS client can be created for the
   *     given URL (previous results are kept)
   */
  ...
  /**
   * Sets the bug ID, which is associated to the changes when storing the results. In other words,
   * the changes identified are supposed to fix the bug with the given identifier.
   *
   ...
   * @throws com.sap.psr.vulas.vcs.NoRepoClientException if client creation and initialization
   *     failed
  ...
  /**
   * Identifies all constructs that have been changed in the given revision. To that end, all files
   * touched are downloaded from the remote repository, and afterwards compared according to the
   * syntax of the respective programming language.
   *
  ...
  public static void main(String[] _args) {
    ...
    options.addOption(
        ...
        "search string",
        true,
        "Search string (optional, if not provided, the bugid will be used for searching the commit log)");
    options.addOption("b", "bug", true, "Bug identifier");
    options.addOption(
        "e",
        "revision",
        true,
        "One or multiple revisions (optional, multiple ones must be comma-separated w/o blanks). In the case of Git repositories, the revision can be optionally concatened with ':' with the branch information.");
    options.addOption(
        "links",
        "links",
        true,
        "Comma-separated list of links to comprehensive bug information (optional, only required for non-NVD vulnerabilities)");
    options.addOption(
        "descr",
        "description",
        true,
