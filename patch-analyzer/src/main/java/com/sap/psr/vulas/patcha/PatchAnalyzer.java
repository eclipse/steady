...
public class PatchAnalyzer {
    ...
    public static IVCSClient createVCSClient(URL _url) throws NoRepoClientException {
        ...
        for (int i = 0; i < clients.length; i++) {
            try {
                // Set the proxy default before every instantiation (SvnKit seems to alter the
                // default and later calls to JGit fail)
    ...
    public Map<ConstructId, Construct> identifySourceConstructs(String _rev) throws Exception {
        if (!(this.sourceRev.equals(_rev)) || this.sourceConstructs == null) {
            ...
            // Instantiate SrcAnalyzer (using the dir to which the checkout happened) to crawl all
            // files
    ...
    public static void main(String[] _args) {
        ...
        options.addOption(
                ...
                "revision",
                true,
                "One or multiple revisions (optional, multiple ones must be comma-separated w/o"
                    + " blanks). In the case of Git repositories, the revision can be optionally"
                    + " concatened with ':' with the branch information.");
        options.addOption(
                "links",
                "links",
                true,
                ...
                try {
                    ...
                    if (skip_if_existing && BackendConnector.getInstance().isBugExisting(bugid)) {
                        PatchAnalyzer.log.info(
                                "Bug ["
                                        + bugid
                                        + "] already exists in backend, analysis will be skipped");
                        return;
                    }
                    ...
                    // Search for revisions
                        ...
                        if (revisions.size() == 0) {
                            PatchAnalyzer.log.info(
                                    "No revision found for search string '" + search_string + "'");
                            return;
                    ...
                    final StopWatch sw =
                            new StopWatch("Analysis of [" + revisions.size() + "] revision(s)");
