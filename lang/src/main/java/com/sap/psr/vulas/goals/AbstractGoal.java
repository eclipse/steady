...
/**
 * Represents the execution of a goal, which is triggered by client components such as the CLI and
 * the Maven plugin. There exist different types of goals, e.g., {@link GoalType#CLEAN} or {@link
 * GoalType#A2C}.
 *
 * <p>Goal executions can be started and stopped manually using the methods {@link
 * AbstractGoal#start()}, {@link AbstractGoal#stop()}, {@link AbstractGoal#stop(Exception)} and
 * {@link AbstractGoal#upload(boolean)}.
 *
 * <p>Goal executions can also be executed automatically using the methods {@link
 * AbstractGoal#executeSync()} or {@link AbstractGoal#executeAsync()}, which results in the
 * sequential execution of the above-mentioned methods {@link AbstractGoal#start()}, etc.
 *
 * <p>Subclasses typically override the methods prepareExecution, executeTasks and
 * cleanAfterExecution.
 *
 * <p>See VULAS-204 in case we run into problems related to special characters in paths.
 */
public abstract class AbstractGoal implements Runnable {
  ...
  /**
   * Setter for the field <code>observer</code>.
   *
  ...
  /**
   * Getter for the field <code>goalType</code>.
   *
  ...
  /**
   * Returns true if this {@link AbstractGoal} is executed in the given {@link GoalClient}, false
   * otherwise.
   *
   * @param _client a {@link com.sap.psr.vulas.shared.enums.GoalClient} object.
   * @return a boolean.
  ...
  /**
   * Returns the configuration of this goal execution. If the configuration has not been set before,
   * a new instance of {@link VulasConfiguration} is created and returned. As such, the
   * configuration settings of different goal executions can be isolated.
   *
  ...
  /**
   * Returns the context of this goal execution. If the context has not been set before, it is
   * constructed by reading tenant, space and app information from the configuration obtained from
   * {@link AbstractGoal#getConfiguration()}.
   *
  ...
  /**
   * Setter for the field <code>goalContext</code>.
   *
  ...
  /**
   * Cleans the cache of the {@link BackendConnector}. CAN be overridden in subclasses to perform
   * additional, goal-specific checks and preparations.
   *
   * @throws com.sap.psr.vulas.goals.GoalConfigurationException if any.
   */
   ...
   *
   * <p>CAN be overridden in subclasses in order to perform goal-specific clean-up.
   */
  ...
  public String toJson() throws IllegalStateException {
    ...
    final Iterator<String> iter =
        this.getConfiguration().getConfiguration().subset("vulas").getKeys();
    while (iter.hasNext()) {
      final String key = iter.next();
      final String[] value =
          this.getConfiguration().getConfiguration().getStringArray("vulas." + key);
      if (c++ > 0) b.append(",");
  ...
  /**
   * Uploads the JSON presentation of this goal execution to the Vulas backend. Returns true of
   * everything went fine (upload succeeded or is not necessary), false otherwise.
   *
