            ...
            this.addAppPaths(
                    FileUtil.getPaths(
                            this.getConfiguration()
                                    .getStringArray(CoreConfiguration.APP_DIRS, null)));

            // Warn if there's no app path
            if (!this.hasAppPaths()) log.warn("No search path(s) provided");

            // Extract single WAR file
            // if(this.appPaths.size()==1 && FileUtil.hasFileExtension(this.appPaths.get(0),
            // WAR_EXT)) {
            // this.handleAppWars();
            // }
    ...
    public void addAppPath(Path _p) throws IllegalArgumentException {
        if (!FileUtil.isAccessibleDirectory(_p) && !FileUtil.isAccessibleFile(_p))
            ...
            log.debug(
                    "["
                            + _p
                            + "] is already part of application paths, and will not be added"
                            + " another time");
