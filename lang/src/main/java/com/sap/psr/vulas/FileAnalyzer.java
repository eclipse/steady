 ...
 * Instances of {@link FileAnalyzer} are built by {@link FileAnalyzerFactory}. {@link FileAnalyzer}s
 * can be nested and form a tree structure, which is addressed by methods {@link #hasChilds()} and
 * {@link #getChilds(boolean)}.
 */
public interface FileAnalyzer {
  ...
  /**
   * Searches for programming {@link Construct}s contained in a given {@link File} and returns them
   * to the caller. The {@link File} to be analyzed is not specified using the methods of the
   * interface. Typically, it is passed to the constructor of the respective implementation.
   *
  ...
  /**
   * Returns the {@link Construct} with the given {@link ConstructId} or null if no such construct
   * is present in the analyzed file.
   *
   * @param _id a {@link com.sap.psr.vulas.ConstructId} object.
  ...
  /**
   * Returns true if there are any child {@link FileAnalyzer}s, false otherwise. Some analyzers will
   * never have any childs, e.g., those that parse single source code files. In case of other
   * analyzes, e.g., the {@link DirAnalyzer}, it depends on whether there are any "analyzable"
   * files.
   *
   * @return a boolean.
   */
  ...
  /**
   * Returns nested {@link FileAnalyzer}s or null if no child analyzers exist. Depending on the
   * argument, only direct childs or also indirect childs will be returned.
   *
