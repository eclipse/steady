...
/** Abstract AbstractVulasMojo class. */
public abstract class AbstractVulasMojo extends AbstractMojo {
  ...
  /**
   * All plugin configuration settings of the element &lt;layeredConfiguration&gt; are put in this
   * {@link Map}.
   */
  ...
  /**
   * Puts the plugin configuration element &lt;layeredConfiguration&gt; as a new layer into {@link
   * VulasConfiguration}. If no such element exists, e.g., because the POM file does not contain a
   * plugin section for Vulas, default settings are established using {@link MavenProject} and
   * {@link VulasConfiguration#setPropertyIfEmpty(String, Object)}.
   *
  ...
  /**
   * This method, called by Maven, first invokes {@link AbstractVulasMojo#createGoal()} and then
   * {@link AbstractVulasMojo#executeGoal()}.
   *
  ...
  /**
   * Evaluates the configuration settings {@link AbstractVulasMojo#INCLUDES}, {@link
   * AbstractVulasMojo#EXCLUDES} and {@link #IGNORE_POMS} to determine whether the given {@link
   * MavenProject} shall be processed.
   *
      ...
      if (do_process)
        ...
        this.getLog()
            .warn(
                "Artifact ["
                    + _prj.getArtifactId()
                    + "] will NOT be processed, it is not among those explicitly included for processing via configuration parameter ["
                    + INCLUDES
                    + "]");
    }
   ...
   * Note that such subclasses should be annotated with "requiresDependencyResolution =
  ...
  /**
   * Parses one element of the {@link Artifact}'s dependency trail, which is a {@link String}
   * comprising groupId, artifactId, type and version.
   *
   * @param _string
   * @return a {@link LibraryId} created from groupId, artifactId and version (or null if the given
