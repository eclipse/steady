...
public abstract class AbstractVulasMojo extends AbstractMojo {
	...
	private final void setKnownDependencies() throws DependencyResolutionRequiredException {
		if(this.goal!=null && this.goal instanceof AbstractAppGoal) {
			...
			final Set<Artifact> direct_artifacts = project.getDependencyArtifacts(); // The artifact class does not seem to tell whether it is a direct or transitive dependency (hence we keep the call and suppress the warning)
