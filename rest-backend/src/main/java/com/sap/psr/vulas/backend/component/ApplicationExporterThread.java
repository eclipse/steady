...
public class ApplicationExporterThread implements Runnable {
     ...
     * @return a {@link com.sap.psr.vulas.backend.component.ApplicationExporterThread} object.
     */
    public ApplicationExporterThread setIncludeGoalConfiguration(
    ...
    @Transactional(
            ...
            propagation =
                    Propagation
                            .REQUIRED) // Needed in order to lazy load properties when called async
    public void run() {
        // Show progress
        final StopWatch sw =
                new StopWatch(
                        "Worker thread: Produce ["
                                + this.format
                                + "] for ["
                                + apps.size()
                                + "] apps");
        ...
        for (Application a : this.apps) {
            ...
            try {
                ...
                if (a.getSpace().getSpaceOwners() != null
                        && a.getSpace().getSpaceOwners().size() > 0) {
                    int i = 0;
                ...
                if (!StringUtil.isEmptyOrContainsEmptyString(this.bugs)
                ...
                if (ExportFormat.JSON.equals(this.getFormat()) && this.includeAllBugs) {
                    ...
                    final TreeSet<VulnerableDependency> vd_all =
                            this.appRepository.findAppVulnerableDependencies(
                                    a, this.includeExemptions, false);
                    for (VulnerableDependency vd : vd_all) {
                        json.appendJsonToArray(
                                JacksonUtil.asJsonString(vd, null, Views.Default.class));
                    }
                    json.endArray();
                }

                // Stuff from goal execution
                final GoalExecution latest_goal_exe =
                        gexeRepository.findLatestGoalExecution(a, null);
                if (latest_goal_exe != null) {
                    csv.append(separator)
                            .append(
                                    this.dateFormat.format(
                                            latest_goal_exe.getCreatedAt().getTime()))
                            .append(separator)
