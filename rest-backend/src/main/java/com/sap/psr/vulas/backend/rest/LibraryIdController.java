...
public class LibraryIdController {
  ...
  /**
   * Returns a list of all {@link Bug}s affecting the given Maven artifact. This list can be
   * filtered by proving a CVSS threshold (geCvss) or bug identifiers (selecteBugs).
   *
  ...
  public ResponseEntity<List<Bug>> getLibIdBugs(
    ...
    try {
      ...
      final LibraryId libids =
          LibraryIdRepository.FILTER.findOne(
              this.libIdRepository.findBySecondaryKey(mvnGroup, artifact, version));
      ...
      if (selectedBugs == null || selectedBugs.length == 0)
        ...
        bugsId =
            this.afflibRepository.findBugByLibraryId(mvnGroup, artifact, version, selectedBugs);
      ...
      for (Bug s : bugsId) {
        ...
        if (geCvss > 0F) {
          ...
          if (s.getCvssScore() == null
              || s.getCvssScore()
                  < geCvss) // Bugs having cvssScore==null will not be added to the list
          continue;
        }
   ...
   * packaging "JAR". The boolean flags secureOnly and vulnerableOnly can be used to filter the
   * result set. The method can be used, for instance, to inform developers over all none-vulnerable
   * library versions as a replacement for a vulnerable one.
   *
   * @param mvnGroup a {@link java.lang.String} object.
   * @param artifact a {@link java.lang.String} object.
   * @param latest if true, only the latest version will be returned
   * @param greaterThanVersion if specified, only versions greater than the provided value are
   *     returned
  ...
  @RequestMapping(
      ...
      produces = {"application/json;charset=UTF-8"})
  @JsonView(Views.LibraryIdDetails.class)
  public ResponseEntity<List<com.sap.psr.vulas.shared.json.model.LibraryId>> getArtifactVersions(
      @PathVariable String mvnGroup,
      @PathVariable String artifact,
      @RequestParam(value = "latest", required = false, defaultValue = "false") Boolean latest,
      @RequestParam(value = "greaterThanVersion", required = false, defaultValue = "false")
          String greaterThanVersion,
      @RequestParam(value = "secureOnly", required = false, defaultValue = "false")
          Boolean secureOnly,
      @RequestParam(value = "vulnerableOnly", required = false, defaultValue = "false")
          Boolean vulnerableOnly) {
    ...
    if (secureOnly && vulnerableOnly) {
      ...
      return new ResponseEntity<List<com.sap.psr.vulas.shared.json.model.LibraryId>>(
    ...
    try {
      ...
      List<com.sap.psr.vulas.shared.json.model.LibraryId> vuln_libids =
          new ArrayList<com.sap.psr.vulas.shared.json.model.LibraryId>();
      List<com.sap.psr.vulas.shared.json.model.LibraryId> greater_libids =
          new ArrayList<com.sap.psr.vulas.shared.json.model.LibraryId>();
      for (LibraryId l : known_libids) {
        if (greaterThanVersion != null
            && (new Version(l.getVersion()).compareTo(new Version(greaterThanVersion)) > 0))
          greater_libids.add(l.toSharedType());
        for (AffectedLibrary afflib : l.getAffLibraries()) {
          Boolean affected =
              this.afflibRepository.isBugLibIdAffected(
                  afflib.getBugId().getBugId(), afflib.getLibraryId());
      ...
      List<Object[]> libids_w_rebundles =
          this.libIdRepository.findBundledLibIdByGA(mvnGroup, artifact);
      ...
      if (vulnerableOnly && greaterThanVersion == null)
        return new ResponseEntity<List<com.sap.psr.vulas.shared.json.model.LibraryId>>(
            vuln_libids, HttpStatus.OK);

      // Else we get all versions from external services
      final TreeSet<com.sap.psr.vulas.shared.json.model.LibraryId> all_libids =
          new TreeSet<com.sap.psr.vulas.shared.json.model.LibraryId>(
              ServiceWrapper.getInstance()
                  .getAllArtifactVersions(mvnGroup, artifact, latest, greaterThanVersion));
      // final Collection<com.sap.psr.vulas.shared.json.model.LibraryId> all_libids =
      // ServiceWrapper.getInstance().getAllArtifactVersions(mvnGroup,  artifact, latest,
      // greaterThanVersion);

      // and merge both collections
      if (greaterThanVersion == null && !latest) {
        for (LibraryId l : known_libids) all_libids.add(l.toSharedType());
      } else
        all_libids.addAll(
            (Collection<com.sap.psr.vulas.shared.json.model.LibraryId>) greater_libids);

      final List<com.sap.psr.vulas.shared.json.model.LibraryId> result =
          new ArrayList<com.sap.psr.vulas.shared.json.model.LibraryId>();
      ...
      if (all_libids != null) {
        ...
        for (LibraryId syn : synonym_libids) {
          all_libids.addAll(
              ServiceWrapper.getInstance()
                  .getAllArtifactVersions(syn.getMvnGroup(), syn.getArtifact(), latest, null));
        }

        Boolean found = false;
        for (com.sap.psr.vulas.shared.json.model.LibraryId a : all_libids) {
          found = false;
          // If vulnerable should be included, take the one known locally, as it is has
          // vulnerability info
          for (com.sap.psr.vulas.shared.json.model.LibraryId b : vuln_libids) {
            if (b.getMvnGroup().equals(a.getMvnGroup())
                && b.getArtifact().equals(a.getArtifact())
          ...
          if (!found && !vulnerableOnly) result.add(a);
        }
      }
      return new ResponseEntity<List<com.sap.psr.vulas.shared.json.model.LibraryId>>(
