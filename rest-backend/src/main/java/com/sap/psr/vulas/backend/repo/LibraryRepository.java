...
public interface LibraryRepository extends CrudRepository<Library, Long>, LibraryRepositoryCustom {
  ...
  @Query(
      value =
          " SELECT d.lib.digest,COUNT(distinct d.app)  FROM "
              + "   Dependency d  "
  ...
  /**
   * Returns all {@link Bug}s relevant for the {@link Library} with the given digest. A {@link Bug}
   * is considered relevant if one or more of the {@link ConstructId}s changed as part of the bug
   * fix is contained in the {@link Library}.
   *
   * @param digest a {@link java.lang.String} object.
   * @return a {@link java.util.List} object.
   */
  ...
  /**
   * Same as {@link LibraryRepository#findBugs(String)}, but offering an additional filter for
   * selected bug ID(s).
   *
   * @param digest a {@link java.lang.String} object.
  ...
  /**
   * Returns all {@link ConstructId}s contained in the {@link Library} with the given digest and of
   * the given type.
   *
   * @param digest a {@link java.lang.String} object.
  ...
  /**
   * findJPQLVulnerableLibrariesByBug.
   *
   * @param bugId a {@link java.lang.String} object.
   * @return a {@link java.util.List} object.
   */
  ...
  @Query(
      value =
          "select distinct s.digest "
              ...
              + " where (s.candidate_pack_count >= unknown_count.unknown_pack_count*90/100) AND (s.candidate_pack_count <= unknown_count.unknown_pack_count*100/100) ",
      nativeQuery = true)
  ...
  @Query(
      value =
          "select distinct d.id as dep_id, l2.id as bundled_lib_id"
              ...
              + "   where d.app=:app and not l1.id = l2.id and l2.wellknown_digest='true'  ",
      nativeQuery = true)
