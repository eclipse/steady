...
/**
 * TODO: Read with token by adding a where-clause AND (a.token = :token OR true = :ignoreToken),
 * whereby ignoreToken is a boolean argument set according to TokenUtil.isIgnoreToken
 */
@Repository
public interface ApplicationRepository
    extends CrudRepository<Application, Long>, ApplicationRepositoryCustom {
   ...
   */
  @Query(
      "SELECT app FROM Application AS app JOIN FETCH app.space s WHERE app.mvnGroup = :mvnGroup AND app.artifact = :artifact AND app.space = :space")
   ...
   */
  @Query(
      "SELECT app FROM Application AS app JOIN FETCH app.space s WHERE app.mvnGroup = :mvnGroup AND app.artifact = :artifact AND app.version = :version AND app.space = :space")
  ...
  // @Query("SELECT DISTINCT app FROM Application AS app JOIN FETCH app.space s WHERE
  // s.spaceToken=:spaceToken AND app.lastVulnChange >  :timestamp ORDER BY app.mvnGroup,
  // app.artifact, app.version")
  /**
   * findAllApps.
   *
  ...
  @Query(
      value =
          "select distinct a.id, a.artifact, a.created_at, a.modified_at, a.last_vuln_change, a.last_scan, "
              + " a.mvn_group, a.space, a.version "
              + " from app a  inner join space s on a.space=s.id "
              + " where s.space_token= :spaceToken and (extract(epoch from last_vuln_change) > :timestamp OR extract(epoch from last_scan) > :timestamp)",
      nativeQuery = true)
  List<Application> findAllApps(
      @Param("spaceToken") String spaceToken, @Param("timestamp") long timestamp);

  /**
   * Returns all {@link Application}s of the given {@link Tenant}, no matter the {@link Space}. Note
   * that the JPQL query cannot use distinct, as the applications of different spaces can have the
   * same group, artifact and version identifier.
   *
   * @param tenant a {@link com.sap.psr.vulas.backend.model.Tenant} object.
   * @return a {@link java.util.ArrayList} object.
   */
  // @Query("SELECT DISTINCT app FROM Application AS app JOIN FETCH app.space s WHERE s.tenant =
  // :tenant ORDER BY app.mvnGroup, app.artifact, app.version")
  @Query(
      "SELECT app FROM Application AS app JOIN FETCH app.space s WHERE s.tenant = :tenant ORDER BY app.mvnGroup, app.artifact, app.version")
  ArrayList<Application> findAllApps(@Param("tenant") Tenant tenant);

  /**
   * Returns all {@link Application}s of the given {@link Tenant} and {@link Space}. Note that the
   * JPQL query cannot use distinct, as the applications of different spaces can have the same
   * group, artifact and version identifier.
   *
   * @param tenant a {@link com.sap.psr.vulas.backend.model.Tenant} object.
   * @param spaceToken a {@link java.lang.String} object.
   * @return a {@link java.util.ArrayList} object.
   */
  @Query(
      "SELECT app FROM Application AS app JOIN FETCH app.space s WHERE s.tenant = :tenant AND s.spaceToken = :spaceToken ORDER BY app.mvnGroup, app.artifact, app.version")
   ...
   */
  @Query(
      "SELECT app FROM Application AS app JOIN FETCH app.space s WHERE app.mvnGroup LIKE :mvnGroup AND app.artifact LIKE :artifact AND app.version LIKE :version")
  Collection<Application> searchByGAV(
      @Param("mvnGroup") String group,
      @Param("artifact") String artifact,
      @Param("version") String version);

  //	@Query("SELECT DISTINCT app FROM Application AS app JOIN app.dependencies order by
  // app.mvnGroup,app.artifact,app.version")
  ...
  @Query(
      value =
          "(select distinct applicatio0_.id, applicatio0_.artifact, applicatio0_.created_at,  applicatio0_.modified_at, applicatio0_.last_vuln_change, applicatio0_.last_scan, "
              ...
              + " order by applicatio0_.mvn_group, applicatio0_.artifact, applicatio0_.version )",
      nativeQuery = true)
  List<Application> findNonEmptyApps(
      @Param("spaceToken") String spaceToken, @Param("timestamp") long timestamp);
   ...
   */
  @Query(
      "SELECT DISTINCT app FROM Application AS app JOIN FETCH app.dependencies d where d.lib.digest=:digest")
  ...
  /**
   * findJPQLVulnerableDependenciesByGAVAndAffVersion.
   *
   * @param group a {@link java.lang.String} object.
   * @param artifact a {@link java.lang.String} object.
   * @param version a {@link java.lang.String} object.
  ...
  @Query(
      "SELECT"
          ...
          + "   AND cc IS NULL ")
  TreeSet<VulnerableDependency> findJPQLVulnerableDependenciesByGAVAndAffVersion(
      @Param("mvnGroup") String group,
      @Param("artifact") String artifact,
      @Param("version") String version,
      @Param("space") Space space,
      @Param("affected") Boolean affected);

  /**
   * findJPQLVulnerableDependenciesByGAV.
   *
   * @param group a {@link java.lang.String} object.
   * @param artifact a {@link java.lang.String} object.
   * @param version a {@link java.lang.String} object.
  ...
  /**
   * Counts the number of constructs of the given {@link ConstructType}s contained in the
   * application's {@link Dependency}s (excluding those having the provided scopes).
   *
   * @param group a {@link java.lang.String} object.
   * @param artifact a {@link java.lang.String} object.
   * @param version a {@link java.lang.String} object.
  ...
  Integer countDepConstructTypes(
      @Param("mvnGroup") String group,
      @Param("artifact") String artifact,
      @Param("version") String version,
      @Param("space") Space space,
      @Param("types") ConstructType[] types,
      @Param("exclScopes") Scope[] exclScopes);

  /**
   * Counts the number of constructs of the given {@link ConstructType}s contained in the
   * application's {@link Dependency}s.
   *
   * @param group a {@link java.lang.String} object.
   * @param artifact a {@link java.lang.String} object.
   * @param version a {@link java.lang.String} object.
  ...
  TreeSet<ConstructSearchResult> searchDepConstructs(
      @Param("mvnGroup") String group,
      @Param("artifact") String artifact,
      @Param("version") String version,
      @Param("space") Space space,
  ...
  /**
   * Counts the number of {@link Dependency}s of the given {@link Application} (excluding those
   * having the provided scopes).
   *
   * @param group a {@link java.lang.String} object.
   * @param artifact a {@link java.lang.String} object.
   * @param version a {@link java.lang.String} object.
  ...
  /**
   * findJPQLVulnerableDependencies.
   *
   * @return a {@link java.util.TreeSet} object.
   */
  @Query(
  ...
  /**
   * Finds the applications whose dependencies include constructs from the given list. Note: the
   * outer select a was added because the lock does not allow the use of "distinct" and we want to
   * avoid to update the same application multiple times in the subsequent update query
   *
  ...
  @Transactional
  @Query(
      "select a from Application a where a in"
  ...
  /**
   * Finds the applications whose dependencies include {@link LibraryId}s from the given list. Note:
   * the outer select a was added because the lock does not allow the use of "distinct" and we want
   * to avoid to update the same application multiple times in the subsequent update query
   *
   * @return list of {@link Application}
  ...
  @Query(
      value =
          "UPDATE Application app SET last_vuln_change=CURRENT_TIMESTAMP where app in :listOfApp ")
