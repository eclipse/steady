...
public interface ApplicationRepository
    ...
    @Query(
            value =
                    "select distinct a.id, a.artifact, a.created_at, a.modified_at,"
                        + " a.last_vuln_change, a.last_scan,  a.mvn_group, a.space, a.version "
                        + " from app a  inner join space s on a.space=s.id  where s.space_token="
                        + " :spaceToken and (extract(epoch from last_vuln_change) > :timestamp OR"
                        + " extract(epoch from last_scan) > :timestamp)",
            nativeQuery = true)
    ...
    @Query(
            value =
                    "(select distinct applicatio0_.id, applicatio0_.artifact,"
                        + " applicatio0_.created_at,  applicatio0_.modified_at,"
                        + " applicatio0_.last_vuln_change, applicatio0_.last_scan, "
                        + " applicatio0_.mvn_group, applicatio0_.space, applicatio0_.version  from"
                        + " app applicatio0_  inner join space space3_ on"
                        + " applicatio0_.space=space3_.id  where space3_.space_token= :spaceToken"
                        + " and (extract(epoch from applicatio0_.last_vuln_change) > :timestamp OR"
                        + " extract(epoch from applicatio0_.last_scan) > :timestamp) and"
                        + " applicatio0_.id in (select application_id from app_constructs)  order"
                        + " by applicatio0_.mvn_group, applicatio0_.artifact, applicatio0_.version"
                        + " ) UNION  (select distinct applicatio0_.id, applicatio0_.artifact,"
                        + " applicatio0_.created_at, applicatio0_.modified_at,"
                        + " applicatio0_.last_vuln_change, applicatio0_.last_scan, "
                        + " applicatio0_.mvn_group, applicatio0_.space, applicatio0_.version  from"
                        + " app applicatio0_ inner join app_dependency dependenci1_ on"
                        + " applicatio0_.id=dependenci1_.app  inner join space space2_ on"
                        + " applicatio0_.space=space2_.id where space2_.space_token= :spaceToken"
                        + " and (extract(epoch from applicatio0_.last_vuln_change) > :timestamp OR"
                        + " extract(epoch from applicatio0_.last_scan) > :timestamp) order by"
                        + " applicatio0_.mvn_group, applicatio0_.artifact, applicatio0_.version )",
            nativeQuery = true)
     ...
     */
    @Query(
            "SELECT   DISTINCT new com.sap.psr.vulas.backend.model.VulnerableDependency(d,b) FROM	"
                + "  Dependency d    JOIN    d.app a 	  JOIN    d.lib l   JOIN    l.libraryId"
                + " dep_libid,	  Bug b   JOIN    b.affectedVersions av    JOIN    av.libraryId"
                + " av_libid    LEFT OUTER JOIN    b.constructChanges as cc 	  WHERE a.mvnGroup ="
                + " :mvnGroup    AND a.artifact = :artifact    AND a.version = :version    AND"
                + " a.space = :space    AND dep_libid = av_libid    AND cc IS NULL ")
     ...
     */
    @Query(
            "SELECT   DISTINCT new com.sap.psr.vulas.backend.model.ConstructSearchResult(d, lc)"
                + " FROM	    Dependency d      JOIN      d.app a 	    JOIN      d.lib l     JOIN  "
                + "    l.constructs lc 	  WHERE a.mvnGroup = :mvnGroup      AND a.artifact ="
                + " :artifact      AND a.version = :version      AND a.space = :space      AND"
                + " lc.type IN :types      AND lc.qname LIKE :searchString ")
    ...
    @Query(
            "SELECT   DISTINCT new com.sap.psr.vulas.backend.model.VulnerableDependency(d,b) FROM	"
                + "  Dependency d    JOIN    d.app a 	  JOIN    d.lib l   JOIN    l.constructs lc,"
                + "	  Bug b   JOIN    b.constructChanges cc 	  WHERE lc = cc.constructId   AND NOT"
                + " lc.type='PACK'")
    ...
    @Query(
            "select a from Application a where a in"
                    ...
                    + "   OR NOT EXISTS (SELECT 1 FROM ConstructChange cc1 JOIN cc1.constructId c1"
                    + " WHERE c1 IN :listOfConstructs AND NOT c1.type='PACK' AND NOT c1.qname LIKE"
                    + " '%test%' AND NOT c1.qname LIKE '%Test%' and NOT"
                    + " cc1.constructChangeType='ADD') )    AND NOT (lc.type='MODU' AND"
                    + " (lc.qname='setup'"
                    + " OR lc.qname='tests'"
