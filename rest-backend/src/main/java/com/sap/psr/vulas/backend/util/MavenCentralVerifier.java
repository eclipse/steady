            ...
            try {
                ...
                if (sc == HttpStatus.SC_OK && entity != null) {
                    ...
                    MavenVersionsSearch json_response =
                            (MavenVersionsSearch)
                                    JacksonUtil.asObject(mvnResponse, MavenVersionsSearch.class);
                    ...
                    if (num_found >= 1) {
                        ...
                        for (ResponseDoc d : json_response.getResponse().getSortedDocs()) {
                            ...
                            // take timestamp of the artifact corresponding to the provided
                            // libraryId or the
                            // first one as that's the one we will use in case the provided one is
                            // not among
