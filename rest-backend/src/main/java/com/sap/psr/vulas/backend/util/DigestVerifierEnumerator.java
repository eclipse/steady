...
/**
 * Loops over available implementations of {@link DigestVerifier} in order to verify the digest of a
 * given {@link Library}.
 */
public class DigestVerifierEnumerator implements DigestVerifier {
   ...
   * {@inheritDoc}
   *
   * <p>Loops over available implementations of {@link DigestVerifier} in order to verify the digest
   * of a given {@link Library}.
   */
  public Boolean verify(Library _lib) throws VerificationException {
    ...
    // Will only have a true or false value if either one verifier returns true, or all returned
    // false (thus, no exception happened)
