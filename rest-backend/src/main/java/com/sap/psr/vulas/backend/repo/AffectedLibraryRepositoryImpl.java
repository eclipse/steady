...
public class AffectedLibraryRepositoryImpl implements AffectedLibraryRepositoryCustom {
    ...
    public List<AffectedLibrary> customSave(Bug _bug, AffectedLibrary[] _aff_libs)
        ...
        for (AffectedLibrary provided_aff_lib : _aff_libs) {
            ...
            try {
                ...
                // (SP, 10.12.2018: in case the assessment flag is equal to the existing, we skip
                // the saving of the affected app and keep the existing one.
                if (provided_aff_lib.getAffected() != null
                                && managed_aff_lib.getAffected() != null
                                && provided_aff_lib
                                        .getAffected()
                                        .equals(managed_aff_lib.getAffected())
                        || (provided_aff_lib.getAffected() == null
                                && managed_aff_lib.getAffected() == null)) continue;
            ...
            // Update refs to existing lib (lib must have been analyzed before -> should already
            // exist)
            ...
            try {
                ...
                // Update vulnChange timestamp for apps with construct changes among its
                // dependencies' constructs
                // this needs to be done after the bug has been created as we need the construct
                // changes to exist in the database to avoid querying by fields (lang, type, qname)
    ...
    public void computeAffectedLib(VulnerableDependency _vd, Library _lib) {
        ...
        // TODO: the code below should be used to check whether an affectedLIbrary for the outer
        // libs exists using the digest (in case the libid is null)
        ...
        if (avByLib != null) {
            ...
            Boolean avByLibId =
                    this.affLibRepository.isBugLibIdAffected(
                            _vd.getBug().getBugId(), _lib.getLibraryId());
            if (avByLibId != null) {
