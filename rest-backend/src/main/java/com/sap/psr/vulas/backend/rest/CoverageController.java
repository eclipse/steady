...
public class CoverageController {
  ...
  private static final String LANG_CONF_THRESHOLD =
      "vulas.backend.coverageService.langConfidenceThreshold";
  private static final String LICENSE_CONF_THRESHOLD =
      "vulas.backend.coverageService.licenseConfidenceThreshold";
  ...
  public ResponseEntity<CoverageStatusResponse> isCovered(@PathVariable String bugid) {
    ...
    final String browse_issue =
        VulasConfiguration.getGlobal().getConfiguration().getString(BROWSE_ISSUE_URL, null);
    final String create_issue =
        VulasConfiguration.getGlobal().getConfiguration().getString(CREATE_ISSUE_URL, null);

    // Create response and set basic info
    final CoverageStatusResponse response = new CoverageStatusResponse(bugid_uc);
    if (classifier_response != null) response.setDescription(classifier_response.getDescription());
    ...
    try {
      ...
      response.setActionUrl(
          VulasConfiguration.getGlobal()
              .getConfiguration()
              .getString(VulasConfiguration.HOMEPAGE, "n/a"));
      response.setActionText(
          "Learn how to detect vulnerable open-source software in your application");
    } catch (EntityNotFoundException enfe) {
      try {
        // It is not in the database, let's ask Jira whether there's already a ticket
        final JiraSearchResponse jira_search_response =
            ServiceWrapper.getInstance().searchJira(bugid_uc);
        ...
        // Classifier
          ...
          final double license_threshold =
              VulasConfiguration.getGlobal()
                  .getConfiguration()
                  .getDouble(LICENSE_CONF_THRESHOLD, 0.2);
          final double language_threshold =
              VulasConfiguration.getGlobal().getConfiguration().getDouble(LANG_CONF_THRESHOLD, 0.2);
          // In scope but not covered yet
          if (classifier_response.isSupportedLanguage(language_threshold)
              && classifier_response.isSupportedLicense(license_threshold)) {
            ...
            if (create_issue != null && !create_issue.equals("")) {
              ...
              response.setActionText(
                  "Create a Jira ticket if this CVE affects an open-source software component developed in Java or Python.");
            }
          }
          // Out of scope
          else {
            ...
            if (create_issue != null && !create_issue.equals("")) {
              response.setActionUrl(create_issue + bugid_uc);
              response.setActionText(
                  "The ML classifier predicted that this CVE does not concern an open-source software component developed in Java or Python. Create a Jira ticket if the classifier is wrong.");
            }
          }
        }
        // All other cases
  ...
  public static class CveClassifierResponse {
    ...
    /**
     * Returns true if (a) the confidence is above the given threshold and the language equals
     * 'java', or (b) the confidence is below the given threshold.
     *
    ...
    public boolean isSupportedLanguage(double _confidence_threshold) {
      return (this.language_confidence > _confidence_threshold
              && "java".equalsIgnoreCase(this.getLanguage()))
          || this.language_confidence < _confidence_threshold;
    }
    ...
    public float getLanguage_confidence() {
      return language_confidence;
    }

    public void setLanguage_confidence(float language_confidence) {
    ...
    public float getLicense_confidence() {
      return license_confidence;
    }

    @JsonIgnore
    public boolean isSupportedLicense(double _confidence_threshold) {
      return (this.license_confidence > _confidence_threshold
              && "oss".equalsIgnoreCase(this.getLicense()))
          || this.license_confidence < _confidence_threshold;
    }

    public void setLicense_confidence(float license_confidence) {
      this.license_confidence = license_confidence;
    }
  }
