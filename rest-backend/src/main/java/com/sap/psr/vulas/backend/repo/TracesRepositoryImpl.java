    ...
    public List<Trace> customSave(Application _app, Trace[] _traces) throws PersistenceException {
        ...
        for (Trace provided_trace : _traces) {
            ...
            try {
                ...
                if (provided_trace.getLib()
                        != null) // Lib can be null if trace belongs to app construct
                provided_trace.setLib(
                            LibraryRepository.FILTER.findOne(
                                    this.libRepository.findByDigest(
                                            provided_trace.getLib().getDigest())));
            ...
            // Check whether a corresponding dependency has been created already and set traced
            // property
            if (provided_trace.getLib() != null) {
                try {
                    // Get the dependency
                        ...
                        managed_dep =
                                DependencyRepository.FILTER.findOne(
                                        this.depRepository.findByAppAndLib(
                                                _app, provided_trace.getLib().getDigest()));
        ...
        // update the lastScan timestamp of the application (we already have a managed application
        // here)
