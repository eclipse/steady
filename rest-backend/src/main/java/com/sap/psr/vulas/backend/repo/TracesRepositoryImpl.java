		...
		for(Trace provided_trace: _traces) {
			...
			try {
				...
				if(provided_trace.getLib()!=null) // Lib can be null if trace belongs to app construct
					provided_trace.setLib(LibraryRepository.FILTER.findOne(this.libRepository.findByDigest(provided_trace.getLib().getDigest())));
				provided_trace.setConstructId(ConstructIdRepository.FILTER.findOne(this.cidRepository.findConstructId(provided_trace.getConstructId().getLang(), provided_trace.getConstructId().getType(), provided_trace.getConstructId().getQname())));
			
				// Find existing trace (if any)
				if(provided_trace.getLib()!=null)
					managed_trace = TracesRepository.FILTER.findOne(this.traceRepository.findTracesOfLibraryConstruct(provided_trace.getApp(), provided_trace.getLib(), provided_trace.getConstructId()));
				
				else
					// Find existing application trace (if any)
					managed_trace = TracesRepository.FILTER.findOne(this.traceRepository.findTracesOfAppConstruct(provided_trace.getApp(), provided_trace.getConstructId()));
			...
			try {
				managed_cid = ConstructIdRepository.FILTER.findOne(this.cidRepository.findConstructId(provided_trace.getConstructId().getLang(), provided_trace.getConstructId().getType(), provided_trace.getConstructId().getQname()));
			} catch (EntityNotFoundException e1) {
			...
			if(provided_trace.getLib()!=null) {
				try {
					...
					if(deps_cache.containsKey(provided_trace.getLib().getDigest())) {
						...
						managed_dep = DependencyRepository.FILTER.findOne(this.depRepository.findByAppAndLib(_app, provided_trace.getLib().getDigest()));
					...
					managed_dep = new Dependency(_app, provided_trace.getLib(), null, null, provided_trace.getFilename());
					managed_dep.setTraced(true);
