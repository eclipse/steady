...
public class LibraryRepositoryImpl implements LibraryRepositoryCustom {
  ...
  public Library customSave(Library _lib) {
    ...
    if (_lib.getDigestAlgorithm() == null) {
      log.warn(
          "Library uploaded with vulas 2.x (digestAlgoritm field missing), defaulting to SHA1 for backward compatibility");
    ...
    try {
      ...
      //	Re-create wellknownDigest if it is null in our current db (this part should be removed once
      // it is created for all)
      if (managed_lib.getWellknownDigest() == null
          || (managed_lib.getDigestTimestamp() == null && managed_lib.getWellknownDigest())) {
        _lib.verifyDigest();
      } else {
      ...
      LibraryRepositoryImpl.log.info(
