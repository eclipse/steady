    ...
    public Library customSave(Library _lib) {
        ...
        try {
            ...
            if (_lib.getLibraryId() == null) {
                // Recreate libId if null (to be removed once we ensure we added to the existing
                // libs)
        ...
        _lib.setBundledLibraryIds(
                refUpdater.saveNestedBundledLibraryIds(_lib.getBundledLibraryIds()));
        ...
        try {
            ...
            try {
                ...
                temp_file =
                        FileUtil.writeToTmpFile("library", "json", JacksonUtil.asJsonString(_lib));
                log.info(
                        "To-be-saved library serialized to temporary JSON file ["
                                + temp_file
                                + "]");
            } catch (Exception ioe) {
                log.error(
    ...
    public Library saveIncomplete(Library _lib) throws PersistenceException {
        ...
        try {
            ...
            throw new PersistenceException(
                    "Error while saving incomplete lib ["
                            + incomplete.getDigest()
                            + "]: "
                            + e.getMessage());
        }
    }
