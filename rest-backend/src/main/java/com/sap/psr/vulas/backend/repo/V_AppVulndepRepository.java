...
public interface V_AppVulndepRepository
    ...
    @Query(
            value =
                    "SELECT count(*) from (select distinct"
                        + " app_group,app_artifact,app_version,digest,bug FROM v_app_vulndep WHERE"
    ...
    @Query(
            value =
                    " select count(*) from  ( select distinct"
                        ...
                        + " app from ( 			select distinct b.mvn_group,max(created_at) as max from "
                        + "				(select distinct g.id,g.created_at,a.id,a.mvn_group from"
                        + " app_goal_exe g join app a on g.app=a.id ) as b 					group by"
                        + " b.mvn_group ) as c join app_goal_exe as l on c.max=l.created_at) as d"
                        + " on d.app=aa.id) as latest  join app a1 on"
                        + " latest.mvn_group=a1.mvn_group and latest.latest_version=a1.version) as"
                        + " f ",
            nativeQuery = true)
    ...
    @Query(
            value =
                    " select count(*) from  (select distinct"
                        + " vd.app_group,vd.app_artifact,vd.app_version from v_app_vulndep as vd"
                        + " join ( 		select aa.mvn_group,aa.version as latest_version from app aa"
                        + " join 		(select app from ( 			select distinct"
                        + " b.mvn_group,max(created_at) as max from 				(select distinct"
                        + " g.id,g.created_at,a.id,a.mvn_group from app_goal_exe g join app a on"
                        + " g.app=a.id ) as b 					group by b.mvn_group ) as c join app_goal_exe"
                        + " as l on c.max=l.created_at) as d on d.app=aa.id) as latest 					on"
                        + " latest.mvn_group=vd.app_group and latest.latest_version=vd.app_version"
                        + " where affected=true) as latest_vd ",
            nativeQuery = true)
    ...
    @Query(
            value =
                    " select count(*) from  (select distinct"
                        + " vd.app_group,vd.app_artifact,vd.app_version,vd.digest,vd.bug from"
                        + " v_app_vulndep as vd join ( 		select aa.mvn_group,aa.version as"
                        + " latest_version from app aa join 		(select app from ( 			select"
                        + " distinct b.mvn_group,max(created_at) as max from 				(select distinct"
                        + " g.id,g.created_at,a.id,a.mvn_group from app_goal_exe g join app a on"
                        + " g.app=a.id ) as b 					group by b.mvn_group ) as c join app_goal_exe"
                        + " as l on c.max=l.created_at) as d on d.app=aa.id) as latest 					on"
                        + " latest.mvn_group=vd.app_group and latest.latest_version=vd.app_version"
                        + " where affected=true or affected is NULL) as latest_vd ",
            nativeQuery = true)
    ...
    @Query(
            value =
                    " select count(*) from  (select distinct"
                        ...
                        + " as l on c.max=l.created_at) as d on d.app=aa.id) as latest 					on"
                        + " latest.mvn_group=vd.app_group and latest.latest_version=vd.app_version"
                        + " where affected=true and NOT (scope='TEST' or scope='PROVIDED')) as"
                        + " latest_vd ",
            nativeQuery = true)
    ...
    @Query(
            value =
                    " select count(*) from  (select distinct vd.app_group,vd.app_artifact from"
                        + " v_app_vulndep as vd join ( 		select aa.mvn_group,aa.version as"
                        + " latest_version from app aa join 		(select app from ( 			select"
                        + " distinct b.mvn_group,max(created_at) as max from 				(select distinct"
                        + " g.id,g.created_at,a.id,a.mvn_group from app_goal_exe g join app a on"
    ...
    @Query(
            value =
                    " select count(*) from  ( select distinct latest.mvn_group,a1.artifact from  ("
                        + "		select aa.mvn_group,aa.version as latest_version from app aa join 	"
                        + "	(select app from ( 			select distinct b.mvn_group,max(created_at) as"
                        + " max from 				(select distinct g.id,g.created_at,a.id,a.mvn_group from"
                        + " app_goal_exe g join app a on g.app=a.id ) as b 					group by"
                        + " b.mvn_group ) as c join app_goal_exe as l on c.max=l.created_at) as d"
                        + " on d.app=aa.id) as latest  join app a1 on"
                        + " latest.mvn_group=a1.mvn_group and latest.latest_version=a1.version) as"
    ...
    @Query(
            value =
                    " select count(*) from  (select distinct vd.app_group from v_app_vulndep as vd"
                        + " join (                select aa.mvn_group,aa.version as latest_version"
                        + " from app aa join                (select app from (                    "
                        + "    select distinct b.mvn_group,max(created_at) as max from            "
                        + "                    (select distinct g.id,g.created_at,a.id,a.mvn_group"
                        + " from app_goal_exe g join app a on g.app=a.id ) as b                   "
                        + "                     group by b.mvn_group ) as c join app_goal_exe as l"
                        + " on c.max=l.created_at) as d on d.app=aa.id) as latest                 "
                        + "                       on latest.mvn_group=vd.app_group and"
                        + " latest.latest_version=vd.app_version where affected=true and NOT"
                        + " (scope='TEST' or scope='PROVIDED')) as latest_vd ",
            nativeQuery = true)
    ...
    @Query(
            value =
                    " select a.client_version,a.date,a.goal,count(*) as c from  (select"
                        + " client_version,date_trunc('day',created_at) as date, goal,id from"
                        + " app_goal_exe ) as a  group by  a.client_version,a.date,a.goal order by"
                        + " a.date desc, c ",
            nativeQuery = true)
    //	@Query(value=" select a.client_version,a.date,a.hour,a.goal,count(*) as c from " +
    //			" (select client_version,date_trunc('day',created_at) as date, extract(hour from
    // created_at) as hour,goal,id from app_goal_exe ) as a "+
    ...
    @Query(
            value =
                    " select latest_vd.app_group,latest_vd.app_artifact,count(*) from  (select"
                        + " distinct vd.app_group,vd.app_artifact,vd.app_version,vd.digest,vd.bug"
                        ...
                        + " as l on c.max=l.created_at) as d on d.app=aa.id) as latest 					on"
                        + " latest.mvn_group=vd.app_group and latest.latest_version=vd.app_version"
                        + " where affected=true or affected is NULL) as latest_vd group by"
    ...
    //	Boolean isAppVulnerableConfig(@Param("space_token") String space_token, @Param("group")
    // String group,  @Param("artifact") String artifact,@Param("version") String version);
    //
