...
public interface SpaceRepository extends CrudRepository<Space, Long>, SpaceRepositoryCustom {
   ...
   */
  @Query(
      "SELECT s FROM Space AS s LEFT JOIN FETCH s.spaceOwners WHERE s.tenant.tenantToken = :tenant and s.isPublic = :p")
   ...
   */
  @Query(
      "SELECT s FROM Space AS s LEFT JOIN FETCH s.spaceOwners WHERE s.tenant = :tenant and s.spaceToken = :token")
   ...
   */
  @Query(
      "SELECT s FROM Space AS s LEFT JOIN FETCH  s.tenant t WHERE t.tenantToken = :tenant and s.spaceToken = :token")
   ...
   */
  @Query(
      "SELECT s FROM Space AS s LEFT JOIN FETCH s.spaceOwners WHERE s.tenant.tenantToken = :tenant and s.isDefault = true")
