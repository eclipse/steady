...
/** ConstructIdRepository interface. */
...
public interface ConstructIdRepository extends PagingAndSortingRepository<ConstructId, Long> {
   ...
   * {@inheritDoc}
   *
   * <p>This method must not be exposed, as {@link ConstructId}s are only created when other
   * entities are uploaded.
   */
  @Override
   ...
   */
  @Query(
      "SELECT cid FROM ConstructId AS cid WHERE cid.lang = :lang AND cid.type = :type AND cid.qname = :qname")
  ...
  // List<ConstructId> findByQnamePrefix(@Param("lang") String lang, @Param("type") String type,
  // @Param("qname") String qname);

  /**
   * countReachableExecConstructsLibrary.
   *
  ...
  @Query(
      value =
          "select count(*) from "
              ...
              + "  on j.reachable_construct_ids_id=c.id   ",
      nativeQuery = true)
