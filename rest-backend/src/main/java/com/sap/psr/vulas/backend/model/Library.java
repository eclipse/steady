...
@Table(
    ...
    indexes = {@Index(name = "digest_index", columnList = "digest", unique = true)})
public class Library implements Serializable {
  ...
  /**
   * Human-readable library identifier, e.g., a Maven artifact ID consisting of group, artifact and
   * version.
   */
  @ManyToOne(
      optional = true,
  ...
  /**
   * True if the library provider or a trusted software repository confirms the mapping of digest to
   * human-readable ID, false otherwise. TODO: field now set to false even in case the verification
   * failed because the external service returns [500]; should be null instead
   */
  @Column(nullable = true)
  @JsonIgnoreProperties(
      value = {"wellknownDigest"},
      allowGetters = true)
  private Boolean wellknownDigest;

  /**
   * The library's publication timestamp (read from the package repository during digest
   * verification).
   */
  @Column(nullable = true)
  @Temporal(TemporalType.TIMESTAMP)
  @JsonFormat(
      ...
      timezone = "GMT")
  @JsonIgnoreProperties(
      value = {"digestTimestamp"},
      allowGetters = true)
  private java.util.Calendar digestTimestamp;

  /**
   * The URL used to verify the digest (and get the publication timestamp), empty if the digest is
   * not known to any of the available package repositories.
   */
  @Column(nullable = true)
  @JsonIgnoreProperties(
  ...
  @Transient private ConstructIdFilter filter = null;
  ...
  @Transient private String mvnResponse = null;

  @Transient private Set<ProgrammingLanguage> developedIn = null;
  ...
  /**
   * Vulas 2.x clients will only provide the SHA1 digest of the library. It will be used to set the
   * Vulas 3.x fields {@link Library#digest} and {@link Library#digestAlgorithm}.
   *
  ...
  /**
   * Getter for the field <code>digestTimestamp</code>.
   *
   * @return a {@link java.util.Calendar} object.
   */
  public java.util.Calendar getDigestTimestamp() {
    return digestTimestamp;
  }
  /**
   * Setter for the field <code>digestTimestamp</code>.
   *
   * @param digestTimestamp a {@link java.util.Calendar} object.
   */
  public void setDigestTimestamp(java.util.Calendar digestTimestamp) {
    this.digestTimestamp = digestTimestamp;
  }
  ...
  // substituted with fields constructorCounter and methodCounter to avoid queries all constructs in
  // order to have the total in the dependencies summary
  /*@JsonProperty(value = "constructCounter")
  public int countConstructs() { return ( this.getConstructs()==null ? 0 : this.getConstructs().size()); } */

  /**
   * Loops over all constructs in order to find the distinct set of {@link ProgrammingLanguage}s
   * used to develop the library. Note: If slow, it can maybe improved by using a JPQL query.
   *
  ...
  public ConstructIdFilter countConstructTypes() {
    if (this.filter == null) this.filter = new ConstructIdFilter(this.getConstructs());
    return this.filter;
  }
  ...
  public final LibraryId getLibIdFromMaven(String _digest) {
    ...
    try {
      libid =
          new LibraryId(
              (String) JsonPath.read(result, "$.response.docs[0].g"),
              (String) JsonPath.read(result, "$.response.docs[0].a"),
              (String) JsonPath.read(result, "$.response.docs[0].v"));
    }
  ...
  /**
   * Getter for the field <code>directUsageCounter</code>.
   *
  ...
  /**
   * Setter for the field <code>directUsageCounter</code>.
   *
