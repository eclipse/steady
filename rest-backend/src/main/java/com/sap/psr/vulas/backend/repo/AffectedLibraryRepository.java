...
//	List<String> findBugByLibraryId(@Param("group") String group, @Param("artifact") String artifact, @Param("version") String version, @Param("affected")Boolean affected);
	...
	@Query(value= "	select a.affected from "
			...
			+ "	 where al1.library_id is null and (al1.source='CHECK_CODE') "  
			+ "	 and not exists (select 1 from bug_affected_library as al2 where al2.source='MANUAL' and al1.bug_id=al2.bug_id and al1.lib=al2.lib)"	
			+ " UNION "
			+ " select distinct al1.bug_id,al1.lib,al1.affected from bug_affected_library as al1"
			...
			+ " and not exists (select 1 from bug_affected_library as al2 where (al2.source='MANUAL' OR al2.source='CHECK_CODE') and al1.bug_id=al2.bug_id and al1.lib=al2.lib)) as a "
...
//			+ " where a.bug_id=:bug_id and a.library_id=:library_id",nativeQuery=true)
	...
	@Query(value= "	select a.affected from "
			...
			+ " where al1.lib is null and (al1.source='CHECK_CODE') "  
			+ "	 and not exists (select 1 from bug_affected_library as al2 where al2.source='MANUAL' and al1.bug_id=al2.bug_id and al1.library_id=al2.library_id)"
			+ " UNION "
			+ " select distinct al1.bug_id,al1.library_id,al1.affected from bug_affected_library as al1"
			...
			+ " and not exists (select 1 from bug_affected_library as al2 where (al2.source='MANUAL' OR al2.source='CHECK_CODE') and al1.bug_id=al2.bug_id and al1.library_id=al2.library_id)) as a "
			+ " where a.bug_id=:bug_id and a.library_id=:library_id",nativeQuery=true)
