...
public interface AffectedLibraryRepository
    ...
    @Query(
            "SELECT afflib FROM AffectedLibrary AS afflib WHERE afflib.bugId = :bug AND"
                ...
                + " AND afflib.libraryId.version = :version AND afflib.source = :source")
    ...
    @Query(
            "SELECT afflib FROM AffectedLibrary AS afflib WHERE afflib.bugId = :bug AND"
                + " afflib.source = :source AND afflib.libraryId.mvnGroup = :group AND"
    ...
    @Query(
            "SELECT afflib FROM AffectedLibrary AS afflib WHERE afflib.bugId = :bug AND"
                + " afflib.libraryId.mvnGroup = :group AND afflib.libraryId.artifact = :artifact"
                + " AND afflib.libraryId.version = :version")
     ...
     */
    @Query(
            "SELECT distinct afflib.bugId FROM AffectedLibrary AS afflib WHERE"
                + " afflib.libraryId.mvnGroup = :group AND afflib.libraryId.artifact = :artifact"
                + " AND afflib.libraryId.version = :version")
    ...
    //	@Query("SELECT distinct bug FROM v_libraryid_bugs AS afflib WHERE afflib.mvnGroup = :group
    // AND afflib.artifact = :artifact  AND afflib.version = :version AND
    // afflib.affected=:affected", nativeQuery=true)
    ...
    @Query(
            "SELECT distinct afflib.bugId FROM AffectedLibrary AS afflib WHERE"
                + " afflib.libraryId.mvnGroup = :group AND afflib.libraryId.artifact = :artifact"
                + " AND afflib.libraryId.version = :version AND afflib.bugId.bugId IN :bugIds ")
    List<Bug> findBugByLibraryId(
            @Param("group") String group,
            @Param("artifact") String artifact,
            @Param("version") String version,
     ...
     * the following strategy priority : MANUAL > CHECK_CODE >
    ...
    @Query(
            value =
                    "	select a.affected from 	(select distinct bug_id,lib,affected from"
                        + " bug_affected_library where source='MANUAL' and library_id is null "
                        + " UNION 	select distinct al1.bug_id,al1.lib,al1.affected from"
                        + " bug_affected_library as al1 	 where al1.library_id is null and"
                        + " (al1.source='CHECK_CODE') 	 and not exists (select 1 from"
                        + " bug_affected_library as al2 where al2.source='MANUAL' and"
                        ...
                        + " al2.source='CHECK_CODE') and al1.bug_id=al2.bug_id and"
    ...
    @Query(
            value =
                    "	select a.affected from 	(select distinct bug_id,library_id,affected from"
                        + " bug_affected_library where source='MANUAL'  and lib is null  UNION "
                        + " select distinct al1.bug_id,al1.library_id,al1.affected from"
                        + " bug_affected_library as al1 where al1.lib is null and"
                        + " (al1.source='CHECK_CODE') 	 and not exists (select 1 from"
                        + " bug_affected_library as al2 where al2.source='MANUAL' and"
                        + " al1.bug_id=al2.bug_id and al1.library_id=al2.library_id) UNION  select"
                        + " distinct al1.bug_id,al1.library_id,al1.affected from"
                        + " bug_affected_library as al1 where al1.lib is null and"
                        + " (al1.source='AST_EQUALITY' OR al1.source='MINOR_EQUALITY'OR"
                        + " al1.source='MAJOR_EQUALITY' OR al1.source='GREATER_RELEASE' OR"
                        + " al1.source='INTERSECTION' OR al1.source='PROPAGATE_MANUAL')  and not"
                        + " exists (select 1 from bug_affected_library as al2 where"
                        + " (al2.source='MANUAL' OR al2.source='CHECK_CODE') and"
                        + " al1.bug_id=al2.bug_id and al1.library_id=al2.library_id)) as a  where"
