  ...
  @Query(
      value =
          "	select a.affected from 	(select distinct bug_id,lib,affected from bug_affected_library"
              ...
              + " al1.library_id is null and (al1.source='CHECK_CODE') 	 and not exists (select 1"
              + " from bug_affected_library as al2 where al2.source='MANUAL' and"
              ...
              + " al2.source='CHECK_CODE') and al1.bug_id=al2.bug_id and al1.lib=al2.lib)) as a "
  ...
  @Query(
      value =
          "	select a.affected from 	(select distinct bug_id,library_id,affected from"
              ...
              + " where al1.lib is null and (al1.source='CHECK_CODE') 	 and not exists (select 1"
              + " from bug_affected_library as al2 where al2.source='MANUAL' and"
              + " al1.bug_id=al2.bug_id and al1.library_id=al2.library_id) UNION  select distinct"
              + " al1.bug_id,al1.library_id,al1.affected from bug_affected_library as al1 where"
              + " al1.lib is null and (al1.source='AST_EQUALITY' OR al1.source='MINOR_EQUALITY'OR"
              + " al1.source='MAJOR_EQUALITY' OR al1.source='GREATER_RELEASE' OR"
              + " al1.source='INTERSECTION' OR al1.source='PROPAGATE_MANUAL')  and not exists"
              + " (select 1 from bug_affected_library as al2 where (al2.source='MANUAL' OR"
