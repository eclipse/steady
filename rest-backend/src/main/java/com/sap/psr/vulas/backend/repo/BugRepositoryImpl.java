...
public class BugRepositoryImpl implements BugRepositoryCustom {
   ...
   * {@inheritDoc}
   *
   * <p>Saves the given {@link Bug} together with all the nested {@link ConstructId}s. This method
   * has to be used in favor of the save method provided by the {@link CrudRepository}.
   */
  ...
  public boolean updateCachedCveData(Bug _b, boolean _force) {
    ...
    if (_force || this.needsCveData(_b)) {
      try {
        ...
        if (cve_id == null) Cve.extractCveIdentifier(_b.getBugIdAlt());
