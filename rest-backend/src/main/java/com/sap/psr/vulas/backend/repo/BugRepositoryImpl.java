...
public class BugRepositoryImpl implements BugRepositoryCustom {
    ...
    public Bug customSave(Bug _bug, Boolean _considerCC) throws PersistenceException {
        ...
        try {
            ...
            // constructs
            // this needs to be done after the bug has been created as we need the construct changes
            // to exist in the database to avoid querying by fields (lang, type, qname)
            if (managed_bug.getConstructChanges() != null
                    && !managed_bug.getConstructChanges().isEmpty())
            ...
            throw new PersistenceException(
                    "Error while saving bug [" + ext_id + "]: " + e.getMessage());
        }
                ...
                managed_cid =
                        ConstructIdRepository.FILTER.findOne(
                                this.cidRepository.findConstructId(
                                        provided_cid.getLang(),
                                        provided_cid.getType(),
                                        provided_cid.getQname()));
            ...
            try {
                ...
                if (cve != null) {
                    ...
                    if (cve.getSummary() != null
                            && (_b.getDescription() == null
                                    || !(cve.getSummary().equals(_b.getDescription())))) {
                        _b.setDescription(cve.getSummary());
                        to_save = true;
                    }
                    if (cve.getCvssScore() != null
                            && (_b.getCvssScore() == null
                                    || !(cve.getCvssScore().equals(_b.getCvssScore())))) {
