...
public class VulnerableDependency implements Serializable, Comparable<VulnerableDependency> {
    ...
    //		this.setAffectedVersion(1); // when the confirmed flag is 0, the value of affected-version
    // is irrelevant but we set it to 1 so that the UI doesn't filter it out when filtering out
        ...
        if (compare == 0)
            compare =
                    this.getDep()
                            .getLib()
                            .getDigest()
                            .compareTo(_other.getDep().getLib().getDigest());
