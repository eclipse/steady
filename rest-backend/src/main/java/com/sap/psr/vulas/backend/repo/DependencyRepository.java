...
public interface DependencyRepository
    ...
    @Query(
            "SELECT dep FROM Dependency dep JOIN FETCH dep.lib l WHERE (dep.app = :app AND"
                + " dep.lib.digest = :digest AND dep.parent = :parent AND dep.relativePath ="
                + " :relPath) OR (dep.app = :app AND dep.lib.digest = :digest AND dep.parent is"
                + " null AND dep.relativePath = :relPath) OR (dep.app = :app AND dep.lib.digest ="
                + " :digest AND dep.parent is null AND dep.relativePath is null) OR (dep.app ="
                + " :app AND dep.lib.digest = :digest AND dep.parent = :parent AND"
                + " dep.relativePath is null ) ")
     ...
     */
    @Query(
            "    SELECT dep FROM  Dependency dep  JOIN FETCH dep.lib l LEFT OUTER JOIN FETCH"
                + " l.libraryId  JOIN dep.app WHERE dep.app.mvnGroup = :mvnGroup AND"
                + " dep.app.artifact = :artifact AND dep.app.version = :version AND dep.app.space"
                + " = :space")
     ...
     */
    @Query(
            "SELECT   new com.sap.psr.vulas.backend.model.DependencyIntersection(d1, d2,"
                + " COUNT(lc1)) FROM 	    Dependency d1      JOIN      d1.app a1 	    JOIN     "
                + " d1.lib l1     JOIN      l1.constructs lc1, 	    Dependency d2      JOIN     "
                + " d2.app a2 	    JOIN      d2.lib l2     JOIN      l2.constructs lc2 	  WHERE"
                + " a1.mvnGroup = :mvnGroup " // Given application
