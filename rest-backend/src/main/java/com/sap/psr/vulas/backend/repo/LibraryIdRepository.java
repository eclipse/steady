...
public interface LibraryIdRepository extends CrudRepository<LibraryId, Long> {
   ...
   */
  @Query(
      "SELECT libid FROM LibraryId AS libid WHERE libid.mvnGroup = :mvnGroup AND libid.artifact = :artifact AND libid.version = :version")
   ...
   */
  @Query(
      "SELECT distinct libid FROM LibraryId AS libid JOIN FETCH libid.affLibraries  WHERE libid.mvnGroup = :mvnGroup AND libid.artifact = :artifact")
   ...
   */
  @Query(
      "SELECT distinct libid.artifact FROM LibraryId AS libid where not libid.mvnGroup LIKE 'com.sap%' and not libid.artifact LIKE 'com.sap%'")
  ...
  @Query(
      value =
          "select lid.id, bl.bundled_library_ids_id as boundled_lid_id "
              + "   from library_id lid "
              + "   inner join lib l1 on lid.id=l1.library_id_id "
              + "   inner join lib_bundled_library_ids bl on l1.id=bl.library_id "
              + "   where lid.mvn_group = :mvnGroup AND lid.artifact = :artifact and not l1.library_id_id = bl.bundled_library_ids_id  ",
      nativeQuery = true)
