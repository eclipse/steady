    ...
    @Query(
            value =
                    "select distinct d.id as dep_id, bl.bundled_library_ids_id as boundled_lid_id "
                        + "   from app_dependency d    inner join lib l1 on d.lib=l1.digest   "
                        + " inner join lib_bundled_library_ids bl on l1.id=bl.library_id    where"
                        + " d.app=:app and not l1.library_id_id = bl.bundled_library_ids_id  ",
            nativeQuery = true)
    List<Object[]> findBundledLibIdByApp(@Param("app") Application app);

    @Query(
            value =
                    "select lid.id, bl.bundled_library_ids_id as boundled_lid_id    from"
                        + " library_id lid    inner join lib l1 on lid.id=l1.library_id_id   "
                        + " inner join lib_bundled_library_ids bl on l1.id=bl.library_id    where"
                        + " lid.mvn_group = :mvnGroup AND lid.artifact = :artifact and not"
                        + " l1.library_id_id = bl.bundled_library_ids_id  ",
            nativeQuery = true)
