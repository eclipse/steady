...
public class TenantController {
		...
		try {
			try {
				...
				if(def==null)
					...
					return new ResponseEntity<Tenant>(def, HttpStatus.OK);
			}
			catch(EntityNotFoundException enfe) {
	...
	public ResponseEntity<Tenant> getTenant(@PathVariable String token) {
		try {
			// Load existing tenant
			try {
				final Tenant tenant = TenantRepository.FILTER.findOne(this.tenantRepository.findBySecondaryKey(token));
	...
	public ResponseEntity<Tenant> modifyTenant(@PathVariable String token, @RequestBody Tenant new_tenant) {
		try {
			...
			// (SP 27-10-2017) The default tenant can now be modified as the notion that it's the default one is stored in the db
...
//			}
			...
			//check that only 1 default tenant exists
			else if(new_tenant.isDefault() && tenantRepository.findDefault()!=null && !tenantRepository.findDefault().equals(new_tenant)) {
				log.error("A default tenant already exists! Only one default tenant is allowed");
				return new ResponseEntity<Tenant>(HttpStatus.BAD_REQUEST);
			}
			...
			try {
				Tenant old_tenant = TenantRepository.FILTER.findOne(this.tenantRepository.findBySecondaryKey(new_tenant.getTenantToken()));
...
//			}
			...
			try {
				Tenant old_tenant = TenantRepository.FILTER.findOne(this.tenantRepository.findBySecondaryKey(token));
				//TODO (27-10-2017): Once we stop supporting vulas 2.x the following condition can be removed
				if(old_tenant.isDefault()){
					log.error("The default tenant cannot be deleted until we stop supporting vulas 2.x.");
					return new ResponseEntity<Tenant>(HttpStatus.BAD_REQUEST);
				}
