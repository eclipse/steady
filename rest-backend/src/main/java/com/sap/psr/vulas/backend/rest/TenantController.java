...
public class TenantController {
  ...
  public ResponseEntity<Tenant> getDefaultTenant() {
    try {
      try {
        ...
        if (def == null) throw new EntityNotFoundException("No default tenant can be found");
   ...
   * @return 404 {@link HttpStatus#NOT_FOUND} if tenant with given token does not exist, 200 {@link
   *     HttpStatus#OK} if the tenant is found
   */
  ...
  /**
   * Creates a new {@link Tenant} with a new, random token in the database and returns it to the
   * client.
   *
  ...
  public ResponseEntity<Tenant> createTenant(@RequestBody Tenant tenant) {
    final StopWatch sw =
        new StopWatch("Create tenant [" + (tenant == null ? null : tenant.getTenantName()) + "]")
            .start();
    try {
  ...
  public ResponseEntity<Tenant> modifyTenant(
    ...
    try {
      ...
      // (SP 27-10-2017) The default tenant can now be modified as the notion that it's the default
      // one is stored in the db
      ...
      try {
        Tenant old_tenant =
            TenantRepository.FILTER.findOne(
                this.tenantRepository.findBySecondaryKey(new_tenant.getTenantToken()));
  ...
  public ResponseEntity<Tenant> deleteTenant(@PathVariable String token) {
    try {
      ...
      try {
        Tenant old_tenant =
            TenantRepository.FILTER.findOne(this.tenantRepository.findBySecondaryKey(token));
        // TODO (27-10-2017): Once we stop supporting vulas 2.x the following condition can be
        // removed
