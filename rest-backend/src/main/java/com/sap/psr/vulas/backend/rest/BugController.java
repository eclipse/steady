...
public class BugController {
  ...
  /**
   * Returns the {@link Bug} with the given external ID. This ID is provided by the user when
   * creating a bug, e.g., a CVE identifier.
   *
   * @return 404 {@link HttpStatus#NOT_FOUND} if bug with given bug ID does not exist, 200 {@link
   *     HttpStatus#OK} if the bug is found
   ...
   * isBugExisting.
   *
   * @return 404 {@link HttpStatus#NOT_FOUND} if bug with given bugid does not exist, 200 {@link
   *     HttpStatus#OK} if the bug is found
   * @param bugid a {@link java.lang.String} object.
   */
   ...
   *
   * @param bug a {@link com.sap.psr.vulas.backend.model.Bug} object.
   * @return 404 {@link HttpStatus#NOT_FOUND} if bug with given bug ID does not exist, 422 {@link
   *     HttpStatus.UNPROCESSABLE_ENTITY} if the value of path variable (bug ID) does not equal the
   *     corresponding field in the body 200 {@link HttpStatus#OK} if the bug was successfully
   *     re-created
   * @param bugid a {@link java.lang.String} object.
   */
  @RequestMapping(
  ...
  /**
   * Deletes the {@link Bug} with the given external ID. This ID is provided by the user when
   * creating a bug, e.g., a CVE identifier.
   *
   * @return 404 {@link HttpStatus#NOT_FOUND} if bug with given bug ID does not exist, 200 {@link
   *     HttpStatus#OK} if the bug was successfully deleted
   * @param bugid a {@link java.lang.String} object.
   */
  ...
  /**
   * Creates a set of {@link AffectedLibrary}s for the given {@link Bug} and {@link
   * AffectedVersionSource}. Note that {@link AffectedLibrary}s cannot be modified or deleted
   * individually, but always as bulk for a given {@link AffectedVersionSource}. The only exception
   * is the source CHECK_VERSION which allows the PUT method to add new AffectedLibraries
   *
   * @return 409 {@link HttpStatus#CONFLICT} if bug with given bug ID already exists, 201 {@link
   *     HttpStatus#CREATED} if the bug was successfully created, 422 {@link
   *     HttpStatus#UNPROCESSABLE_ENTITY} if the source in the path variable and JSON content are
   *     not consistent or when attempting to save an affected lib for a digest verified against
   *     Maven Central (in such case the affected Library must be defined for the corresponding
   *     LibraryId)
   * @param bugid a {@link java.lang.String} object.
   * @param source a {@link com.sap.psr.vulas.shared.enums.AffectedVersionSource} object.
   * @param affectedLibraries an array of {@link com.sap.psr.vulas.backend.model.AffectedLibrary}
   *     objects.
   */
  @RequestMapping(
  ...
  /**
   * Adds a set of {@link AffectedLibrary}s for the given {@link Bug} and {@link
   * AffectedVersionSource}. This method is only allows for source CHECK_VERSION, AST_EQUALITY,
   * MINOR_EQUALITY MAJOR_EQUALITY GREATER_RELEASE TO_REVIEW
   *
   * @return 409 {@link HttpStatus#CONFLICT} if bug with given bug ID already exists, 201 {@link
   *     HttpStatus#CREATED} if the bug was successfully created
   * @param bugid a {@link java.lang.String} object.
  ...
  /**
   * Creates a set of {@link AffectedLibrary}s for the given {@link Bug} and {@link
   * AffectedVersionSource}. Note that {@link AffectedLibrary}s cannot be created, modified or
   * deleted individually, but always as bulk for a given {@link AffectedVersionSource}.
   *
   * @return 409 {@link HttpStatus#CONFLICT} if bug with given bug ID already exists, 201 {@link
   *     HttpStatus#CREATED} if the bug was successfully created
   * @param bugid a {@link java.lang.String} object.
  ...
  public ResponseEntity<List<AffectedLibrary>> getAllAffectedLibraries(
    ...
    if (source == null)
      return new ResponseEntity<List<AffectedLibrary>>(
          this.afflibRepository.findByBug(bug), HttpStatus.OK);
    else
      return new ResponseEntity<List<AffectedLibrary>>(
          this.afflibRepository.findByBugAndSource(bug, source), HttpStatus.OK);
  }

  /**
   * getAffectedLibraryDetails.
   *
   * @param bugid a {@link java.lang.String} object.
  ...
  /**
   * areBugAffectedLibrariesExisting.
   *
   * @return 404 {@link HttpStatus#NOT_FOUND} if affected libraries for a given bugId and source
   *     does not exist, 200 {@link HttpStatus#OK} if they are found
   * @param bugid a {@link java.lang.String} object.
   * @param source a {@link com.sap.psr.vulas.shared.enums.AffectedVersionSource} object.
   */
  ...
  /**
   * Returns the {@link Bug} with the given internal ID. This ID is created internally upon saving a
   * bug. Returns {@link HttpStatus#NOT_FOUND} if no bug with the given internal ID can be found.
   *
   * @param id
   * @return
   */
