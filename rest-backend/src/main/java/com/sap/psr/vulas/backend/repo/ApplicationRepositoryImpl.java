...
public class ApplicationRepositoryImpl implements ApplicationRepositoryCustom {
        ...
        try {
            managed_app =
                    ApplicationRepository.FILTER.findOne(
                            this.appRepository.findByGAV(
                                    group, artifact, version, _app.getSpace()));
            ...
            // if the application does not exist, we create an empty one so that we can later add
            // the dependencies incrementally
        ...
        for (Dependency provided_dep : _provided_app.getDependencies()) {
            for (Dependency managed_dep : _managed_app.getDependencies()) {
                ...
                if (provided_dep.equalLibParentRelPath(managed_dep)) {
                    ...
                    // as they have the same parent, we have to take also the existing parent to get
                    // its db id
                ...
                // if no dependency  with the same lib parent and relPath exists, we do not look for
                // a library with same lib only but we consider it a new dependency
                // here we check whether it has a parent to see whether it already exists among the
                // managed entities
    ...
    public void updateFlags(VulnerableDependency _vd, Boolean _withChangeList) {
        ...
        // 2) Bugs WITHOUT change list
            ...
            // 2.2) Static analysis
            final Collection<ConstructId> reachable_constructs =
                    _vd.getDep().getReachableConstructIds();
    ...
    public VulnerableDependency getVulnerableDependencyBugDetails(
        ...
        VulnerableDependency vd =
                new VulnerableDependency(
                        DependencyRepository.FILTER.findOne(
                                depRepository.findByAppAndLib(a, digest)),
                        bug);
        ...
        List<ConstructChangeInDependency> constructsList =
                new ArrayList<ConstructChangeInDependency>();
    ...
    public TreeSet<VulnerableDependency> findAppVulnerableDependencies(
        ...
        // to improve performances we use a native query to get the affected version (having moved
        // to SQL the computation of the affected version source having priority.
        // further improvements could be:
        // embedding the native query into the JPQL one to only get the bugs according to the
        // requested flags, e.g., only affected or only historical ones.
        ...
        for (Object[] e : bundledDigests) {
            ...
            // for bundled libraries we do have have traced and reachable information so we skip the
            // query
        ...
        if (_log) {
            sw.lap(
                    "Found ["
                            + vd_list_bundled_cc.size()
                            + "] vulns w/  cc through bundled library ids");
            sw.lap(
                    "Found ["
                            + vd_list_bundled_av.size()
                            + "] vulns w/o cc through bundled library ids");
        }

        // 4) Join over libids to identify vulnerable dependencies based on affectedLibraries that
        // were created based on the GAV
        ...
        // this.affLibRepository.computeAffectedLib(vd_list_libid); this is not required for vulns
        // w/o cc as we select by affectedVersions.affected (in this way we save the additional
        // queries to get the affected flag afterwards)
        ...
        vd_all.addAll(
                vd_list_cc); // this must be done before the add for bundled vulndeps to ensure that
        // we do not consider as bundledcc the cases where the code signature
        // was not altered.
        vd_all.addAll(vd_list_bundled_cc);
        vd_all.addAll(vd_list_bundled_av);
        vd_all.addAll(
                vd_list_libid); // this must be added after the vd_list_bundled_av, to ensure that
        // we get the info that a pair dep,bug comes out of something
        // rebundled, though we overwrite it as FP at the level of the
        // rebundling artifact
        ...
        if (_add_exemption_info) {
            ...
            if (latest != null) {
                final ExemptionSet exempts =
                        ExemptionSet.createFromMap(latest.getConfigurationMap());
                for (VulnerableDependency vd : vd_all) {
                    com.sap.psr.vulas.shared.json.model.VulnerableDependency svd =
                            (com.sap.psr.vulas.shared.json.model.VulnerableDependency)
                                    JacksonUtil.fromTo(
                                            vd,
                                            null,
                                            com.sap.psr.vulas.backend.model.view.Views.class,
                                            com.sap.psr.vulas.shared.json.model.VulnerableDependency
                                                    .class);
    ...
    public HashMap<Long, HashMap<String, Boolean>> findAffectedApps(String[] _bugs) {
        ...
        final List<Object[]> affected_apps_raw =
                this.v_appVulnDepRepository.findAffectedApps(_bugs);
        if (affected_apps_raw != null && affected_apps_raw.size() > 0) {
            for (Object[] o : affected_apps_raw) {
                try {
                    ...
                    log.error(
                            "Cannot cast, too few elements in object array, ["
                                    + o.length
                                    + "] instead of [3]");
                }
            }
