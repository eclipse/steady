...
public class ApplicationRepositoryImpl implements ApplicationRepositoryCustom {
  ...
  public Application customSave(Application _app) {
    ...
    try {
      ...
      // if the applcation does not exist, we create an empty one so that we can later add the
  ...
  private Dependency updateParent(Dependency _dep, Application _app) {
    ...
    if (_dep.getParent() != null) _dep.setParent(updateParent(_dep.getParent(), _app));
    try {
      _dep =
          DependencyRepository.FILTER.findOne(
              depRepository.findByAppAndLibAndParentAndRelPath(
                  _app, _dep.getLib().getDigest(), _dep.getParent(), _dep.getRelativePath()));
    } catch (EntityNotFoundException e) {
      // this should not happen, as we ordered the list, when we came across a parent it should have
      // been already saved
   ...
   * {@inheritDoc}
   *
   * <p>Calls {@link VulnerableDependency#setTraced(int)} and {@link
   * VulnerableDependency#setReachable(int)} depending on the following conditions: If the {@link
   * Bug} has a change list, it is checked whether vulnerable constructs were traced during dynamic
   * analysis or found reachable during static analysis. If it does not have a change list, it is
   * checked whether any of the library's constructs has been traced or found reachable.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * <p>Returns a {@link SortedSet} of all {@link Application}s of the given {@link Space}.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * <p>Deletes all the traces and paths collected during the various analyses. If requested, also
   * deletes the goal history.
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * <p>Finds all @{VulnerableDependency}s for a given {@link Space} and {@link Application}.
   */
  @Override
  public TreeSet<VulnerableDependency> findAppVulnerableDependencies(
    ...
    if (_log) sw.lap("Found [" + vd_list_cc.size() + "] through joining constructs", true);
    ...
    if (_log) {
      sw.lap(
          "Found [" + vd_list_bundled_cc.size() + "] vulns w/  cc through bundled library ids",
          true);
      sw.lap(
          "Found [" + vd_list_bundled_av.size() + "] vulns w/o cc through bundled library ids",
          true);
    }
    ...
    final TreeSet<VulnerableDependency> vd_list_libid =
        this.appRepository.findJPQLVulnerableDependenciesByGAVAndAffVersion(
            _app.getMvnGroup(), _app.getArtifact(), _app.getVersion(), _app.getSpace(), true);
    for (VulnerableDependency vd : vd_list_libid) {
      vd.setAffectedVersion(1);
      vd.setAffectedVersionConfirmed(1);
    }
    final TreeSet<VulnerableDependency> vd_list_libid_false =
        this.appRepository.findJPQLVulnerableDependenciesByGAVAndAffVersion(
            _app.getMvnGroup(), _app.getArtifact(), _app.getVersion(), _app.getSpace(), false);
    for (VulnerableDependency vd : vd_list_libid_false) {
      vd.setAffectedVersion(0);
      vd.setAffectedVersionConfirmed(1);
    }
    vd_list_libid.addAll(vd_list_libid_false);
    if (_log) sw.lap("Found [" + vd_list_libid.size() + "] through joining libids", true);
  ...
  /**
   * Returns a set of {@link Scope}s corresponding to the configuration setting {@link
   * GoalExecutionRepositoryCustom#EXCLUDED_SCOPES} of the most recent {@link GoalExecution} of the
   * given application, or null if no executions or no configuration exist.
   *
   * @param _app
   * @return
   */
  ...
  /**
   * Returns a map of Strings corresponding to the configuration setting {@link
   * GoalExecutionRepositoryCustom#EXCLUDED_BUGS} of the most recent {@link GoalExecution} of the
   * given application, or null if no executions or no configuration exist.
   *
   * @param _app
   * @return
    ...
    if (latest != null) {
      ...
      if (property != null) {
        ...
        for (String v : values) {
          ...
          if (text != null) excluded_bugs.put(v.toLowerCase(), text);
          else
            excluded_bugs.put(
                v.toLowerCase(),
                "No reason provided, please add using configuration parameter [" + setting + "]");
        }
      }
    }
