...
public class MainController extends SpringBootServletInitializer {
  ...
  /**
   * TODO: Autom. switch off in production. Registers the Servlet for the H2 Web console, which
   * allows checking DB content. Connect using localhost:xxxx/backend/console (JDBC URL
   * jdbc:h2:mem:testdb)
   *
   * @return
   */
  ...
  /**
   * Used by custom repository methods to replace nested transient entities by managed ones. See,
   * for instance, {@link BugRepositoryImpl#customSave(com.sap.psr.vulas.backend.model.Bug)}.
   *
   * @return
   */
  @Bean
  ...
  /**
   * Can be used to do some initialization at application startup, but does not do anything right
   * now.
   *
