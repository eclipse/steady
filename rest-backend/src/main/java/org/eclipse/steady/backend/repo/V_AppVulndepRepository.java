...
import org.eclipse.steady.backend.model.V_AppVulndep;
...
public interface V_AppVulndepRepository
  ...
  @Query(
      value =
          "SELECT count(*) from (select distinct app_group,app_artifact,app_version,digest,bug"
              + " FROM v_app_vulndep WHERE affected=true OR affected is NULL) as a",
      nativeQuery = true)
  ...
  @Query(
      value =
          "SELECT count(*) from (select distinct app_group,app_artifact,app_version,digest,bug"
              + " FROM v_app_vulndep WHERE affected=true) as a",
      nativeQuery = true)
  ...
  @Query(
      value =
          "SELECT count(*) from (select distinct app_group FROM v_app_vulndep WHERE affected=true"
              + " OR affected is NULL) as a",
      nativeQuery = true)
  ...
  @Query(
      value =
          "SELECT count(*) from (select distinct app_group FROM v_app_vulndep WHERE affected=true"
              + " and NOT (scope='TEST' or scope='PROVIDED')) as a",
      nativeQuery = true)
  ...
  @Query(
      value =
          "SELECT count(*) from (select distinct app_group,app_artifact FROM v_app_vulndep WHERE"
  ...
  @Query(
      value =
          " select count(*) from  (select distinct vd.app_group from v_app_vulndep as vd join (   "
              + "             select aa.mvn_group,aa.version as latest_version from app aa join   "
              + "             (select app from (                        select distinct"
              + " b.mvn_group,max(created_at) as max from                                (select"
              + " distinct g.id,g.created_at,a.id,a.mvn_group from app_goal_exe g join app a on"
              + " g.app=a.id ) as b                                        group by b.mvn_group )"
              + " as c join app_goal_exe as l on c.max=l.created_at) as d on d.app=aa.id) as"
              + " latest                                        on latest.mvn_group=vd.app_group"
              + " and latest.latest_version=vd.app_version where affected=true and NOT"
  ...
  @Query(
      value =
          " select a.client_version,a.date,a.goal,count(*) as c from  (select"
              + " client_version,date_trunc('day',created_at) as date, goal,id from app_goal_exe )"
              + " as a  group by  a.client_version,a.date,a.goal order by a.date desc, c ",
      nativeQuery = true)
  ...
  @Query(
      value =
          "SELECT DISTINCT app_id, bug, affected FROM v_app_vulndep WHERE bug IN :bugIds AND NOT"
              + " affected='false'",
      nativeQuery = true)
