...
public interface ApplicationRepository
   ...
   */
  @Query(
      "SELECT app FROM Application AS app JOIN FETCH app.space s WHERE app.mvnGroup = :mvnGroup"
          + " AND app.artifact = :artifact AND app.space = :space")
  ...
  @Query(
      "SELECT app FROM Application AS app JOIN FETCH app.space s WHERE app.mvnGroup = :mvnGroup"
          + " AND app.artifact = :artifact AND app.version = :version AND app.space = :space")
  ...
  @Query(
      value =
          "select distinct a.id, a.artifact, a.created_at, a.modified_at, a.last_vuln_change,"
              + " a.last_scan,  a.mvn_group, a.space, a.version  from app a  inner join space s on"
              + " a.space=s.id  where s.space_token= :spaceToken and (extract(epoch from"
              + " last_vuln_change) > :timestamp OR extract(epoch from last_scan) > :timestamp)",
      nativeQuery = true)
   ...
   *
   * @param tenant a {@link org.eclipse.steady.backend.model.Tenant} object.
  ...
  @Query(
      "SELECT app FROM Application AS app JOIN FETCH app.space s WHERE s.tenant = :tenant ORDER BY"
   ...
   */
  @Query(
      "SELECT app FROM Application AS app JOIN FETCH app.space s WHERE s.tenant = :tenant AND"
          + " s.spaceToken = :spaceToken ORDER BY app.mvnGroup, app.artifact, app.version")
   ...
   */
  @Query(
      "SELECT app FROM Application AS app JOIN FETCH app.space s WHERE app.mvnGroup LIKE :mvnGroup"
          + " AND app.artifact LIKE :artifact AND app.version LIKE :version")
  ...
  @Query(
      value =
          "(select distinct applicatio0_.id, applicatio0_.artifact, applicatio0_.created_at, "
              + " applicatio0_.modified_at, applicatio0_.last_vuln_change, applicatio0_.last_scan,"
              + "  applicatio0_.mvn_group, applicatio0_.space, applicatio0_.version  from app"
              + " applicatio0_  inner join space space3_ on applicatio0_.space=space3_.id  where"
              + " space3_.space_token= :spaceToken and (extract(epoch from"
              + " applicatio0_.last_vuln_change) > :timestamp OR extract(epoch from"
              + " applicatio0_.last_scan) > :timestamp) and applicatio0_.id in (select"
              + " application_id from app_constructs)  order by applicatio0_.mvn_group,"
              + " applicatio0_.artifact, applicatio0_.version ) UNION  (select distinct"
              + " applicatio0_.id, applicatio0_.artifact, applicatio0_.created_at,"
              + " applicatio0_.modified_at, applicatio0_.last_vuln_change, applicatio0_.last_scan,"
              + "  applicatio0_.mvn_group, applicatio0_.space, applicatio0_.version  from app"
              + " applicatio0_ inner join app_dependency dependenci1_ on"
              + " applicatio0_.id=dependenci1_.app  inner join space space2_ on"
              + " applicatio0_.space=space2_.id where space2_.space_token= :spaceToken and"
              + " (extract(epoch from applicatio0_.last_vuln_change) > :timestamp OR extract(epoch"
              + " from applicatio0_.last_scan) > :timestamp) order by applicatio0_.mvn_group,"
              + " applicatio0_.artifact, applicatio0_.version )",
      nativeQuery = true)
   ...
   * @param version a {@link java.lang.String} object.
   * @param space a {@link org.eclipse.steady.backend.model.Space} object.
   * @param types an array of {@link org.eclipse.steady.shared.enums.ConstructType} objects.
   * @param exclScopes an array of {@link org.eclipse.steady.shared.enums.Scope} objects.
  ...
  @Query(
      "select a from Application a where a in"
          ...
          + "   AND (NOT lc.type='PACK' " // Java + Python exception
          + "   OR NOT EXISTS (SELECT 1 FROM ConstructChange cc1 JOIN cc1.constructId c1 WHERE c1"
          + " IN :listOfConstructs AND NOT c1.type='PACK' AND NOT c1.qname LIKE '%test%' AND NOT"
   ...
   *
   * @return list of {@link Application}
   * @param affLibId a {@link org.eclipse.steady.backend.model.LibraryId} object.
   */
  @Lock(LockModeType.PESSIMISTIC_WRITE)
  @Transactional
  @Query(
