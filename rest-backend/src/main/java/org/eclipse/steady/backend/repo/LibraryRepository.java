...
public interface LibraryRepository extends CrudRepository<Library, Long>, LibraryRepositoryCustom {
  ...
  @Query(
      value =
          "select t.lib from (SELECT COUNT(distinct app) as c,lib  FROM "
              + "    app_dependency   "
              + "   WHERE  transitive=false group by lib order by c desc) as t limit :num",
      nativeQuery = true)
  ...
  @Query(
      value =
          " SELECT d.lib.digest,COUNT(distinct d.app)  FROM    Dependency d     WHERE "
              + " transitive=false    AND d.scope NOT IN :exclScopes group by d.lib.digest order"
              + " by COUNT(distinct d.app) desc")
  ...
  // "join "+
  // "(select c.id from lib as l join lib_constructs as lc on l.id=lc.lib_id join construct_id as c
  // on lc.constructs_id=c.id where l.digest=:digest and c.type='PACK') as u on v.id=u.id group by
  // v.digest,v.library_id_id) as y , "+
  ...
  @Query(
      value =
          "select distinct s.digest  from  (select candidate.digest, candidate.id,"
              ...
              + " a.digest,a.id,a.mvn_group,a.artifact,a.version   from  (select distinct"
              + " l1.digest,l1.id,lid1.mvn_group,lid1.artifact,lid1.version,c1.id  as cid  from"
              + " lib as l1   join library_id lid1 on l1.library_id_id=lid1.id  join"
              + " lib_constructs as lc1 on l1.id=lc1.library_id   join construct_id as c1 on"
              + " lc1.constructs_id=c1.id where c1.type='PACK' ) as a  join  (select c.id   from"
              + " lib as l  join lib_constructs as lc on l.id=lc.library_id  join construct_id as"
              + " c on lc.constructs_id=c.id   where l.digest=:digest and c.type='PACK') as"
              + " unknown  on a.cid=unknown.id) as candidate  join lib_constructs as lc3 on"
              + " candidate.id=lc3.library_id  join construct_id as c3 on lc3.constructs_id=c3.id "
              + "  where c3.type='PACK' group by"
              ...
              + " lib_constructs as lc2 on l2.id=lc2.library_id  join construct_id as c2 on"
              + " lc2.constructs_id=c2.id   where l2.digest=:digest and c2.type='PACK') as"
              + " unknown_count  where (s.candidate_pack_count >="
  ...
  @Query(
      value =
          "select distinct d.id as dep_id, l2.id as bundled_lib_id   from app_dependency d   "
              + " inner join lib l1 on d.lib=l1.digest inner join library_id lid1 on"
              + " l1.library_id_id =lid1.id    inner join lib_bundled_library_ids bl on"
