...
public class BugControllerTest {
    ...
    public void testGetBug() throws Exception {
        ...
        mockMvc.perform(get("/bugs/" + bug.getBugId()))
    ...
    public void testAffectedLibrary() throws Exception {
        ...
        AffectedLibrary[] afl =
                (AffectedLibrary[])
                        JacksonUtil.asObject(
                                FileUtil.readFile(
                                        Paths.get(
                                                "./src/test/resources/real_examples/affectedLib-propagate.json")),
                                AffectedLibrary[].class);

        AffectedConstructChange acc =
                new AffectedConstructChange(cc, afl[0], true, true, true, null);
        ...
        MockHttpServletRequestBuilder get_builder =
                get("/bugs/CVE-2014-0050/affectedLibIds/bar/bar");
        mockMvc.perform(get_builder)
                .andExpect(status().isOk())
                .andExpect(content().contentType(contentType))
        ...
        get_builder =
                get("/bugs/CVE-2014-0050/affectedLibIds/bar/bar/0.0.1?source=PROPAGATE_MANUAL");
        mockMvc.perform(get_builder)
                .andExpect(status().isOk())
                .andExpect(content().contentType(contentType))
    ...
    public void testGetWellKnownAffectedLibrary() throws Exception {
        ...
        mockMvc.perform(
                        get(
                                "/bugs/"
                                        + bug.getBugId()
                                        + "/affectedLibIds?onlyWellKnown=true&source=MANUAL"))
                .andExpect(status().isOk())
                .andExpect(content().contentType(contentType))
                .andExpect(jsonPath("$.length()", is(1)));
