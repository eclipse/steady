...
//	}
	...
	public void testSpaceCreation() throws Exception {
		...
		new_shared_space.setDefault(true); // This will not be possible, as another default space has been created above
		new_shared_space.setExportConfiguration(ExportConfiguration.OFF);
	...
	public void testSearchSpaces() throws Exception {
		...
		new_shared_space.setSpaceDescription("bar");
		new_shared_space.setOwnerEmails(new HashSet<String>(Arrays.asList(new String[] {"foo@bar.com"})));		
		final List<com.sap.psr.vulas.shared.json.model.Property> props = new ArrayList<com.sap.psr.vulas.shared.json.model.Property>();
		props.add(new com.sap.psr.vulas.shared.json.model.Property(PropertySource.USER, "propName", "propValue"));
		...
		post_builder = post("/spaces")
				...
				.header(Constants.HTTP_TENANT_HEADER, d_tenant.getTenantToken())
				.accept(MediaType.APPLICATION_JSON);
		result = mockMvc.perform(post_builder).andExpect(status().isCreated())
				.andExpect(content().contentType(contentType))
				.andExpect(jsonPath("$.default", is(false)))
				.andExpect(jsonPath("$.spaceName", is(space_name))).andReturn();
