...
public class VulnerableDependency implements Serializable, Comparable<VulnerableDependency> {
    ...
    public void evalAffectedVersion() {
        ...
        this.setAffectedVersion(
                1); // when the confirmed flag is 0, the value of affected-version is irrelevant but
        // we set it to 1 so that the UI doesn't filter it out when filtering out
    ...
    public int getReachableConfirmed() {
        // Need to be calculated differently than in backend model, since the collection of
        // reachable constructs is not serialized
