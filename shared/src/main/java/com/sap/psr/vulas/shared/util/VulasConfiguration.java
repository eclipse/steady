...
public class VulasConfiguration {
    ...
    private Configuration writableConfiguration =
    ...
    private void appendInitialConfigurations() {
        ...
        for (String jar_path : jar_paths) {
            if (!jar_paths_analyzed.contains(jar_path)) {
                // getLog().info("Search configuration info in URL [" + urls[i] + "], JAR [" +
                // jar_path + "]");
    ...
    protected boolean appendConfigurationsFromJarPath(String _jar_path) {
        final Map<String, Configuration> jar_entries =
                this.discoverConfigurationsInJarUri(_jar_path);
    ...
    private Map<String, Configuration> discoverConfigurationsInJarUri(String _jar_path) {
        ...
        try {
            ...
            final Pattern pattern = Pattern.compile(VulasConfiguration.propertiesRegex);
            final Pattern pattern_spring =
                    Pattern.compile(VulasConfiguration.propertiesRegexSpring);
            ...
            getLog().error(
                            "Error searching for configurations in JAR ["
                                    + _jar_path
                                    + "]: "
                                    + e.getMessage(),
                            e);
                ...
                // Process nested JAR
                    ...
                    final Map<String, Configuration> nested_configs =
                            this.discoverConfigurationsInNestedJar(
                                    full_name,
                                    new JarInputStream(
                                            new ByteArrayInputStream(this.readContent(_jis))));
                    if (nested_configs != null && nested_configs.size() > 0) {
                        jar_configs.putAll(nested_configs);
                    }
                }
        ...
        while (iter.hasNext()) {
            ...
            if (key.startsWith("vulas.") && key.endsWith(".format") && !this.isEmpty(key)) {
                ...
                if (!this.isEmpty(key_to_check)) {
                    final String[] values_to_check =
                            this.getStringArray(key_to_check, new String[] {});
    ...
    public void setProperty(
        ...
        if ((_value != null || !_ignore_null)
            ...
            // If the _value contains a comma, the new object will be a String array or
            // ArrayList<String>
            final ArrayList<String> array_list = new ArrayList<String>();

            // Check that setting worked
            if (after != null
                    && (after.getClass().isArray()
                            || after.getClass().equals(array_list.getClass()))) {
            } else if ((_value == null && after != null)
                    || (_value != null && !_value.equals(after))) {
                getLog().error(
                                "New value ["
                                        + _value
                                        + "] for setting ["
    ...
    public void setServiceUrl(Service _service, String _value) throws IllegalArgumentException {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "Invalid URL [" + _value + "], cannot set [" + key + "]");
        }
    }
