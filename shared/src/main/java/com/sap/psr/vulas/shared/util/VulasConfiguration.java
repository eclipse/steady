 ...
 *
 * <p>Vulas configuration is composed of multiple, individual {@link Configuration} items. When
 * reading a given setting, the list of configuration items is searched until a configuration
 * containing the respective setting is found.
 *
 * <p>The list contains items in the following order:
 *
 * <p>Writable map properties (added empty by default): Settings provided through {@link
 * VulasConfiguration#setProperty(String, Object, String, boolean)}.
 *
 * <p>System properties (added by default): Settings specified with the JVM option -D.
 *
 * <p>Map properties (no default, added through {@link
 * VulasConfiguration#addLayerAfterSysProps(String, Map, String, boolean)}. Results in rebuilding
 * the entire composite configuration.
 *
 * <p>Properties files (added by default): Found in the current work directory (.) and its
 * sub-directories.
 *
 * <p>Properties files contained in JAR files (by default, the JAR from which {@link
 * VulasConfiguration} is loaded is considered): Searched at the very end, these configuration items
 * are useful for providing defaults.
 *
...
public class VulasConfiguration {
  ...
  public static final synchronized VulasConfiguration getGlobal() {
    if (global == null) global = new VulasConfiguration();
    return global;
  }

  /** Used for retrieving actual configuration settings. */
  private org.apache.commons.configuration.CompositeConfiguration cfg =
  ...
  /**
   * All the configurations used to build the composite configuration. The single configurations are
   * added by {@link VulasConfiguration#appendInitialConfigurations()}.
   */
  private Map<Configuration, String> individualConfigurations =
      new LinkedHashMap<Configuration, String>();
  ...
  /** Constructor for VulasConfiguration. */
  ...
  /**
   * Regex used to discover configurations in the folder BOOT-INF/classes/ of executable Spring
   * JARs.
   */
  ...
  /**
   * Builds the initial list of configurations that can be extended afterwards using {@link
   * VulasConfiguration#appendConfigurationsFromJar(Class)}. The initial list only covers system
   * properties and properties files found in the file system.
   */
  private void appendInitialConfigurations() {
    ...
    for (String jar_path : jar_paths) {
      if (!jar_paths_analyzed.contains(jar_path)) {
        ...
        // getLog().info("URL [" + urls[i] + "], JAR [" + jar_path + "] already analyzed for
        // configuration info");
      }
    }
  ...
  /**
   * Puts the given Configuration as a new layer at the given position and with the given name. If a
   * layer with the same name already exists at the given position, it will be either deleted or
   * shifted by one position according to the boolean argument. In combination with providing null
   * as new configuration, this boolean flag can be used to remove existing layers.
   *
   * @param _cfg
   * @param _source
   * @param _position
   ...
   */
  private boolean putConfiguration(
      Configuration _cfg, String _source, int _position, boolean _replace_if_existing) {
  ...
  /**
   * Rebuilds the composite configuration from the list of individual configurations. Called after
   * {@link VulasConfiguration#addLayerAfterSysProps(String, Map, String, boolean)}, which adds a
   * configuration in the middle of the list rather than appending it to the end. Rebuilding is
   * necessary, since {@link CompositeConfiguration} only appends to the end.
   */
  private void rebuild() {
    cfg.clear();
    for (Configuration config : individualConfigurations.keySet()) cfg.addConfiguration(config);
    this.log(LOG_PREFIXES, "    ");
  }

  /**
   * Adds a {@link MapConfiguration} right after the {@link SystemConfiguration}, which is the
   * second element, and before all other configurations. As such, contained settings get precedence
   * before file-based configurations in file system or JAR files.
   *
   * @param _map a {@link java.util.Map} object.
   * @param _ignore_value if specified, elements will only be added if the value's string
   *     representation from this argument
  ...
  public void addLayerAfterSysProps(
    ...
    if (_map == null) {
      if (removed_existing)
        msg.append("Removed configuration layer [")
            .append(_layer_name)
            .append("] from 3rd position");
      else msg.append("No change of configuration layers");
    } else {
      if (removed_existing)
        msg.append("Replaced existing configuration layer [")
            .append(_layer_name)
            .append("] by new one with [")
            .append(map.size())
            .append("] settings on 3rd position");
      else
        msg.append("Added new configuration layer [")
            .append(_layer_name)
            .append("] with [")
            .append(map.size())
            .append("] settings on 3rd position");
    }
    msg.append(", old/new number of layers is [")
        .append(no_layers_before)
        .append("/")
        .append(no_layers_after)
        .append("]");
    getLog().info(msg.toString());

    if (_map != null || removed_existing) rebuild();
  }

  /**
   * Returns the {@link Configuration} layer with the given name. If multiple layers with that name
   * exist, the top-most layer will be returned.
   *
   * @param _layer_name a {@link java.lang.String} object.
   * @return a {@link org.apache.commons.configuration.Configuration} object.
   */
  ...
  /**
   * appendConfigurationsFromJarPath.
   *
   ...
   *
   * <p>Afterwards, if such configurations were found, the composite configuration is rebuilt from
   * the updated configuration list.
   *
   * <p>Returns true if configurations were found and added.
   *
  ...
  private Map<String, Configuration> discoverConfigurationsInJarUri(String _jar_path) {
    ...
    try {
      ...
      while (entries_enum.hasMoreElements()) {
        ...
        if (m.matches() || m_spring.matches()) {
          try {
            ...
            getLog()
                .error(
                    "Error loading properties from JAR entry ["
                        + full_name
                        + "]: "
                        + e.getMessage(),
                    e);
          ...
          final Map<String, Configuration> nested_configs =
              this.discoverConfigurationsInNestedJar(
                  full_name, new JarInputStream(jf.getInputStream(entry)));
      ...
      getLog()
          .error(
              "Error searching for configurations in JAR [" + _jar_path + "]: " + e.getMessage(),
              e);
    } finally {
      if (jf != null) {
        try {
  ...
  private Map<String, Configuration> discoverConfigurationsInNestedJar(
      String _name, JarInputStream _jis) {
    final Map<String, Configuration> jar_configs = new HashMap<String, Configuration>();
    try {
      final Pattern pattern = Pattern.compile(VulasConfiguration.propertiesRegex);
      final Pattern pattern_spring = Pattern.compile(VulasConfiguration.propertiesRegexSpring);
      ...
      while ((entry = _jis.getNextJarEntry()) != null) {
        ...
        // Process nested JAR
          ...
          final Map<String, Configuration> nested_configs =
              this.discoverConfigurationsInNestedJar(
                  full_name, new JarInputStream(new ByteArrayInputStream(this.readContent(_jis))));
          if (nested_configs != null && nested_configs.size() > 0) {
            jar_configs.putAll(nested_configs);
          }
        }
      ...
      getLog()
          .error("Error searching for configurations in JAR [" + _name + "]: " + e.getMessage(), e);
    } finally {
      if (_jis != null) {
        try {
          _jis.close();
  ...
  /**
   * Reads the content of the current {@link JarEntry} from the given {@link JarInputStream} into a
   * byte array.
   *
   * @param _jis
   * @return
  ...
  public void checkSettings() throws ConfigurationException {
    ...
    if (!not_specified.isEmpty() || !wrong_format.isEmpty())
      throw new ConfigurationException(
          "The following mandatory settings are not specified: ["
              + StringUtil.join(not_specified, ", ")
              + "], the following settings do not comply with the required format: ["
              + StringUtil.join(wrong_format, ", ")
              + "]");
  }

  /**
   * Deletes all transient (not persisted) configuration settings. Returns true if the transient
   * configuration layer contained values that were deleted, false otherwise.
   *
   * @return a boolean.
   */
  public boolean clearTransientProperties() {
    final boolean contains_etries = !writableConfiguration.isEmpty();
    if (contains_etries) writableConfiguration.clear();
    return contains_etries;
  }

  /**
   * Returns true if the configuration does not contain the given setting or its value is an empty
   * string.
   *
  ...
  /**
   * Reads the setting with the given key (recursively, if the key's value is the name of another
   * setting).
   *
   * @param _key a {@link java.lang.String} object.
  ...
  /**
   * Sets the given setting to the specified value in case it is empty. Note that this setting is
  ...
  public void setPropertyIfEmpty(@NotNull String _key, Object _value) {
    if (isEmpty(_key)) setProperty(_key, _value, null, false);
  }

  /**
   * Sets the given setting to the specified value. Note that this setting is transient (not
   * persisted).
   *
   * @param _key a {@link java.lang.String} object.
   * @param _value a {@link java.lang.Object} object.
  ...
  public void setProperty(
      @NotNull String _key, Object _value, String _ignore_value, boolean _ignore_null) {
    if ((_value != null || !_ignore_null)
        && (_ignore_value == null || !_value.toString().equals(_ignore_value))) {

      final Object before = cfg.getProperty(_key);
      if (_value == null) writableConfiguration.clearProperty(_key);
      else writableConfiguration.setProperty(_key, _value);

      final Object after = cfg.getProperty(_key);

      // Log everything (to be deleted)
      // getLog().info("Setting [" + _key + "] value [before=" + before + ", arg=" + _value + ",
      // after=" + after + "]");
      ...
      if (after != null
          && (after.getClass().isArray() || after.getClass().equals(array_list.getClass()))) {
  ...
  /**
   * Improves the method {@link Configuration#getStringArray(String)} by adding a default value,
   * which is returned if the String array returned for the respective setting is either null or
   * contains a single empty {@link String}.
   *
   * @param _key a {@link java.lang.String} object.
  ...
  /**
   * Returns the configuration setting for the given key as {@link Path}. If no such setting exists,
   * the tmp directory will be returned.
   *
   * @param _key a {@link java.lang.String} object.
  ...
  public Path getDir(String _key) {
    Path p = null;
    if (!this.isEmpty(_key)) p = Paths.get(this.getConfiguration().getString(_key));
    else p = this.getTmpDir();
  ...
  /**
   * Creates if necessary and returns the temporary directory to be used by Vulas. This is either
   * the directory indicated by the configuration setting TMP_DIR (if any) or the OS' temporary
   * directory.
   *
   * @return a {@link java.nio.file.Path} object.
   */
  public Path getTmpDir() {
    Path p = null;
    if (!this.isEmpty(TMP_DIR)) p = Paths.get(cfg.getString(TMP_DIR));
    else p = Paths.get(System.getProperty("java.io.tmpdir"));
   ...
   */
  public String getServiceUrl(Service _service, boolean _throw_exception)
    ...
    if (_throw_exception && value == null)
      throw new ServiceConnectionException(
          "Service URL is not configured (parameter [" + key + "])", null);
    return value;
  }
  ...
  public Path getLocalM2Repository() throws IllegalStateException {
    if (this.m2 == null) {
      ...
      try {
        ...
        if (!this.m2.toFile().exists()) Files.createDirectories(m2);

        // Is writable?
        if (!this.m2.toFile().canWrite()) throw new IllegalStateException("No write permission");
      } catch (Exception e) {
        getLog()
            .info(
                "Error configuring the m2 directory ["
                    + m2_path
                    + "], artifacts will not be cached: "
                    + e.getMessage());
        this.m2 = null;
  ...
  /**
   * Returns a {@link StringList} containing items taken from the given configuration settings. Each
   * configuration settings is expected to contain one or more values (comma-separated), which are
   * trimmed and added to the {@link StringList}.
   *
