...
/**
 * Notes: System.nanoTime is well suited to measure elapsed time, cf.
 * http://stackoverflow.com/questions/510462/is-system-nanotime-completely-useless#4588605 It is not
 * well suited to get the current actual time. For that, we use System.runtimeMillis.
 */
public class StopWatch {
   ...
   * The achievements or progress can be reflected with methods {@link StopWatch#progress()} and
   * {@link StopWatch#progress(double, boolean)}.
   *
   * @param _descr
   * @param _total
   * @param _start
  ...
  /**
   * Returns this stop watch in order to behave similar to a builder: StopWatch sw = new
   * StopWatch("foo").start();
   *
   * @return a {@link com.sap.psr.vulas.shared.util.StopWatch} object.
   */
