...
/**
 * Can be used for white- and blacklisting {@link String}s. Supports different modes for comparing a
 * to-be-checked value with the list items, either the entire string or its beginning/end. The
 * latter is used, e.g., for blacklisting classes that belong to certain packages.
 *
 * <p>List items are trimmed by default, unless otherwise specified using the methods {@link
 * StringList#add(String, boolean)} or {@link StringList#addAll(String[], boolean)}.
 */
public class StringList implements Iterable<String> {
  ...
  public StringList(String[] _itemlist) {
    if (_itemlist != null) this.addAll(_itemlist, true);
  }

  /**
   * First trims and then adds all strings contained in argument _itemlist to this {@link
   * StringList}.
   *
   * @param _itemlist an array of {@link java.lang.String} objects.
  ...
  public StringList addAll(String[] _itemlist, boolean _trim) {
    if (_itemlist != null) for (String item : _itemlist) this.add(item, _trim);
    return this;
  }
  ...
  /**
   * Adds the given string to this {@link StringList}. The strings is trimmed depending on the value
   * of the argument _trim. If the provided string is null or empty (""), it will not be added.
   *
   * @param _item a {@link java.lang.String} object.
   * @param _trim a boolean.
   * @return a {@link com.sap.psr.vulas.shared.util.StringList} object.
  ...
  public StringList add(String _item, boolean _trim) {
    if (_item != null && !_item.equals("")) this.itemlist.add((_trim ? _item.trim() : _item));
    return this;
  }
  ...
  /**
   * Returns true if the argument _value is equal (case sensitive) to any of the items in this
   * {@link StringList}.
   *
  ...
  public boolean contains(String _value) {
    return this.contains(_value, ComparisonMode.EQUALS, CaseSensitivity.CASE_SENSITIVE);
  }

  /**
   * Returns true if the argument _value matches any of the items in the list according to the mode.
   *
   ...
   * @param _case_sensitivity a {@link com.sap.psr.vulas.shared.util.StringList.CaseSensitivity}
   *     object.
   */
  public boolean contains(String _value, ComparisonMode _mode, CaseSensitivity _case_sensitivity) {
    ...
    final String value =
        (_case_sensitivity == CaseSensitivity.CASE_INSENSITIVE ? _value.toLowerCase() : _value);
    String item = null;
    ...
    for (String i : this.itemlist) {
      ...
      switch (_mode) {
        ...
        case PATTERN:
          if (value.matches(item)) return true;
          else break;
      }
    }
    return r;
  ...
  /**
   * If matches are to be kept (argument _keep_matches equals true), the method returns a new {@link
   * HashMap} containing all keys of the given map that are contained in this {@link StringList}. If
   * matches are to be dropped (argument _keep_matches equals false), the method returns a new
   * {@link HashMap} containing all keys of the given map NOT contained in this {@link StringList}.
   *
  ...
  public HashMap<String, String> filter(
    ...
    for (String key : _in.keySet()) {
      if (this.contains(key, _mode, _case_sensitivity)) {
        if (_keep_matches) out.put(key, _in.get(key));
