...
public class VulasConfigurationTest {
	...
	public void testConfigurationChange() {
		...
		boolean added = VulasConfiguration.getGlobal().appendConfigurationsFromJar(VulasConfigurationTest.class);
		assertFalse(added);

		// Should not result in an update, since the JAR from the class was loaded does not contain Vulas configuration files
		...
		assertEquals(cfg_1.getString(VulasConfiguration.TMP_DIR), cfg_2.getString(VulasConfiguration.TMP_DIR));
	...
	public void testNestedJars() {
		...
		assertEquals("123", VulasConfiguration.getGlobal().getConfiguration().getString("vulas.test-key-3"));
		assertEquals("abc", VulasConfiguration.getGlobal().getConfiguration().getString("vulas.test-key-4"));
	}
	
	@Test
	public void testToKeyValue() {
		KeyValue[] values = KeyValue.toKeyValue(VulasConfiguration.getGlobal().getConfiguration().subset("vulas"));
	...
	public void testIndependentConfigChanges() {
		...
		assertTrue(!c1.getConfiguration().getString("foo").equals(c2.getConfiguration().getString("foo")));
	}
	...
	public void testCheckSettings() {
		...
		assertFalse(ce==null);
		assertEquals("The following mandatory settings are not specified: [bar], the following settings do not comply with the required format: []", ce.getMessage());
	}
	...
	public void testCheckSettingsFormatWrong() {
		...
		assertFalse(ce==null);
		assertEquals("The following mandatory settings are not specified: [], the following settings do not comply with the required format: [vulas.bar]", ce.getMessage());
	}
