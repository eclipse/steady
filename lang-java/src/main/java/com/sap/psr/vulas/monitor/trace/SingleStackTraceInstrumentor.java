...
/**
 * Tracks how many times a construct has been invoked. Downloads the list of relevant bugs from the
 * backend and collects the stack trace for all their change list elements. Stack traces collected
 * will be transformed into call paths that are again uploaded to the backend.
 */
public class SingleStackTraceInstrumentor extends AbstractTraceInstrumentor {
  ...
  public SingleStackTraceInstrumentor() {
    final GoalContext gc =
        CoreConfiguration.buildGoalContextFromConfiguration(this.vulasConfiguration);
    this.maxStacktraces =
        this.vulasConfiguration
            .getConfiguration()
            .getInt(CoreConfiguration.INSTR_MAX_STACKTRACES, 10);
    try {
      final Map<String, Set<com.sap.psr.vulas.shared.json.model.ConstructId>> bug_change_lists =
          BackendConnector.getInstance()
              .getAppBugs(gc, CoreConfiguration.getAppContext(this.vulasConfiguration));
  ...
  public void instrument(StringBuffer _code, JavaId _jid, CtBehavior _behavior, ClassVisitor _cv)
    ...
    if (is_stacktrace_requested) _cv.addIntMember(st_count, false);
