...
public abstract class AbstractInstrumentor implements IInstrumentor {
  ...
  /**
   * Adds Java code to determine the URL of the resource from which the given class was loaded, and
   * the class loader which loaded it. This information is available in the Java variables
   * vul_cls_res and vul_cls_ldr.
   *
  ...
  protected synchronized void injectUrlAndLoader(
    ...
    if (Modifier.isStatic(m)) {
      ...
      _buffer.append(
          "if(vul_cls==null) { try { vul_cls=java.lang.invoke.MethodHandles.lookup().lookupClass(); } catch(Exception e) {} }");
      ...
      // For unknown reasons, the getClass method does sometimes not exist (even though every class
      // should inherit from Object).
      // The lack of the method causes a compile exception, which can be avoided by checking for its
      // presence.
      // Example: [pool-2-thread-1] INFO  com.sap.psr.vulas.monitor.ClassVisitor  - Exception while
    ...
    _buffer.append(
        "    vul_cls_res = vul_cls_ldr.getResource(vul_cls.getName().replace('.', '/') + \".class\");");
    _buffer.append("}");
  }

  /**
   * Adds Java code to obtain the stacktrace. This information is available in the Java variable
   * vul_st.
   *
   * @param _buffer a {@link java.lang.StringBuffer} object.
   * @param _jid a {@link com.sap.psr.vulas.java.JavaId} object.
  ...
  protected synchronized void injectStacktrace(
    ...
    if (_buffer.length() == 0 || _buffer.indexOf(decl.toString()) == -1) _buffer.insert(0, decl);
