    ...
    public void execute() throws GoalExecutionException {
                            ...
                            if (fa2 instanceof JarAnalyzer) {
                                ...
                                if (this.useAppPrefixes()) {
                                    final Set<ConstructId> constructs =
                                            JavaId.filter(
                                                    fa2.getConstructs().keySet(), this.appPrefixes);
                                    ...
                                    // No constructs match to the prefixes
                                        ...
                                        log.info(
                                                StringUtil.padLeft(++count, 4)
                                                        ...
                                                        + "]: None of the ["
                                                        + fa2.getConstructs().size()
                                                        + "] constructs matched prefix(es): No"
                                                        + " constructs added to application, file"
                                                        + " added as dependency");
                                        dep_files.put(ja.getPath(), ja);
                                    }
                                }
                                    ...
                                    if (this.appJarNames.contains(
                                        ...
                                        log.info(
                                                StringUtil.padLeft(++count, 4)
                                                        + " ["
                                                        + StringUtil.padLeft(ja.getFileName(), 30)
                                                        + "]: Filename matches pattern(s), all of"
                                                        + " its ["
                                                        + fa2.getConstructs().size()
                                                        + "] constructs added to application");
                                    ...
                                    // Dependency
                                        ...
                                        log.info(
                                                StringUtil.padLeft(++count, 4)
                                                        + " ["
                                                        + StringUtil.padLeft(ja.getFileName(), 30)
                                                        + "]: Filename does not match pattern(s),"
                                                        + " file added as dependency");
                                        dep_files.put(ja.getPath(), ja);
                                    }
                                }
        ...
        final long timeout =
                this.vulasConfiguration
                        .getConfiguration()
                        .getLong(CoreConfiguration.JAR_TIMEOUT, -1);
                        ...
                        if (constructs.size() == ja.getConstructs().size()) {
                            log.info(
                                    "All of the ["
                                            ...
                                            + "] matched to prefix ["
                                            + StringUtil.join(this.appPrefixes, ", ")
                                            + "]: Constructs added to application, file removed"
                                            + " from dependencies");
                        }
                        // Mixed archive: Add as dependency
                        else {
                            log.info(
                                    "["
                                            ...
                                            + "] matched to prefix ["
                                            + StringUtil.join(this.appPrefixes, ", ")
                                            + "]: Constructs added to application, file kept as"
                                            + " dependency");
                    ...
                    // No constructs match to the prefixes
                        ...
                        log.info(
                                "None of the ["
                                        ...
                                        + "] matched to prefix ["
                                        + StringUtil.join(this.appPrefixes, ", ")
                                        + "]: No constructs added to application, file kept as"
                                        + " dependency");
                        app_dependencies.add(ja);
                    }
                }
            ...
            try {
                ...
                if (known_dep != null && known_dep.getParent() != null) {
                    ...
                    for (JarAnalyzer ja2 : app_dependencies) {
                        // TODO: As soon as we add the relative path to the JARAnalyzer, it must
                        // also be used to uniquely identify the parent and set all its fields
        ...
        // Fix parents on dependencies that were removed (this block should be removed once we use
        // the relativePath and we get a dependency tree representing the actual one
        for (Dependency d : a.getDependencies()) {
            if (d.getParent() != null) {
                for (Dependency existing : a.getDependencies()) {
                    // TODO: as soon as we add the relative path to the JARAnalyzer, it must also be
                    // used to uniquely identify the parent
