...
public class JavaBomTask extends AbstractBomTask {
  ...
  /**
   * Returns true if the configuration setting {@link CoreConfiguration#APP_PREFIXES} shall be
      ...
      for (Path p : this.getSearchPath()) {
        ...
        if (this.useAppPrefixes() || this.useAppJarNames()) {
          ...
          if (this.useAppPrefixes()) {
            log.info(
                "Looping over Java archive analyzers to separate application and dependency code using package prefix(es) ["
                    + StringUtil.join(this.appPrefixes, ", ")
                    + "] ...");
          } else if (this.useAppJarNames()) {
            log.info(
                "Looping over Java archive analyzers to separate application and dependency code using filename pattern(s) ["
                    ...
                    if (constructs.size() == fa2.getConstructs().size()) {
                      log.info(
                          StringUtil.padLeft(++count, 4)
                              ...
                              + "] constructs matched prefix(es): Constructs added to application, file NOT added as dependency");
                    }
                    // Mixed archive: Add as dependency
                      ...
                      log.info(
                          StringUtil.padLeft(++count, 4)
                              ...
                              + "/"
                              + fa2.getConstructs().size()
                              + "] constructs matched prefix(es): Constructs added to application, file added as dependency");
                  ...
                  // No constructs match to the prefixes
                    ...
                    log.info(
                        StringUtil.padLeft(++count, 4)
                            ...
                            + "]: None of the ["
                            + fa2.getConstructs().size()
                            + "] constructs matched prefix(es): No constructs added to application, file added as dependency");
                    dep_files.put(ja.getPath(), ja);
                  }
                }
                // Jar name regex
