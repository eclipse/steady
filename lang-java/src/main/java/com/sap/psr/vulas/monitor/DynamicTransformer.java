...
/**
 * Uses the {@link ClassVisitor} to dynamically instrument Java constructors and methods during the
 * class loading process. When executed, the injected code will call the {@link ExecutionMonitor} to
 * collect and upload runtime information to the Vulas backend.
 *
 * <p>The name has been chosen to contrast with static instrumentation, which can be done with help
 * of the Vulas Maven plugin (goal vulas:instr).
 */
public class DynamicTransformer implements ClassFileTransformer {
  ...
  /**
   * Determines whether classes will be instrumented or not. Checked in {@link
   * DynamicTransformer#transform(ClassLoader, String, Class, ProtectionDomain, byte[])}. Will be
   * changed to true only after a successful instantiation, otherwise the transform method will
   * called to early (which results in StackOverflow problems).
   */
    ...
    try {
      if (!CoreConfiguration.existsInBackend(CoreConfiguration.getAppContext()))
        throw new IllegalStateException(
            "Application " + CoreConfiguration.getAppContext() + " does not exist in backend");
  ...
  private final void freezeClasses() {
    try {
      ...
      final JarWriter jw =
          new JarWriter(
              Paths.get(
                  DynamicTransformer.class
                      .getClassLoader()
                      .getResource(DynamicTransformer.class.getName().replace('.', '/') + ".class")
                      .toString()));
  ...
  /**
   * Setter for the field <code>transformationEnabled</code>.
   *
  ...
  /**
   * Getter for the field <code>loaderHierarchy</code>.
   *
  ...
  /**
   * Adds instrumentation code to all constructors and methods of all classes (thereby considering
   * certain blacklists and whitelists read from the configuration file).
   *
   * <p>The method is called by the JRE class loading process and returns the instrumented bytecode
   * for a given class.
   *
  ...
  public byte[] transform(
    ...
    // - loader==null || loader.getParent()==null: The class in question is loaded by the bootstrap
    // loader, which loads class definitions from JRE_HOME/lib/ext.
    // - loader is instance of sun.reflect.DelegatingClassLoader (created for the optimization of
    // java.lang.reflect calls) or javax.management.remote.rmi.NoCallStackClassLoader (?)
    ...
    if (loader.getParent() != null) { // && // && loader!=null
      ...
      // If not we spare us the expensive instrumentation (but assume it happened already before,
      // for all JARs, so that the callback happens nevertheless)
      if (this.isTransformationEnabled()) {

        // Blacklisted class according to the configuration parameters
        // "instr.blacklist.classes.jre/custom"?
        ...
        if (!is_blacklisted_class) {
          try {
            ...
            // Blacklisted JAR according to the configuration parameters
            // "instr.blacklist.jars/dirs"?
  ...
  /**
   * Singleton method: Creates (if necessary) and returns the single instance that can be created
   * for this class.
   *
  ...
  public static synchronized boolean isInstantiated() {
    return DynamicTransformer.instance != null;
  }
