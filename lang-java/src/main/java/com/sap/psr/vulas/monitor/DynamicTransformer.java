...
public class DynamicTransformer implements ClassFileTransformer {
    ...
    public byte[] transform(
        ...
        // - loader==null || loader.getParent()==null: The class in question is loaded by the
        // bootstrap loader, which loads class definitions from JRE_HOME/lib/ext.
        // - loader is instance of sun.reflect.DelegatingClassLoader (created for the optimization
        // of java.lang.reflect calls) or javax.management.remote.rmi.NoCallStackClassLoader (?)
        ...
        if (loader.getParent() != null) { // && // && loader!=null
            ...
            // If not we spare us the expensive instrumentation (but assume it happened already
            // before, for all JARs, so that the callback happens nevertheless)
            if (this.isTransformationEnabled()) {
                ...
                final boolean is_blacklisted_class =
                        this.instrControl.isBlacklistedClass(dot_classname);
                ...
                if (!is_blacklisted_class) {
                    try {
                        ...
                        final boolean is_blacklisted_jar =
                                this.instrControl.isBlacklistedJar(c.getURL());
                        ...
                        if (!is_blacklisted_jar && !c.isInterface()) {
                            ...
                            if (!cv.isInstrumented()) {
                                ...
                                DynamicTransformer.log.debug(
                                        "Class [" + dot_classname + "] now instrumented");
                            } else {
                                this.instrControl.updateInstrumentationStatistics(
                                        cv.getJavaId(), null);
                                DynamicTransformer.log.debug(
                                        "Class [" + dot_classname + "] already instrumented");
                            }
                        }
                    ...
                    // Covers the following problems with Javassist and Java 8:
