...
/**
 * This class is used to deserialize an AST JSON representation of a signature change object into
 * its corresponding java object Note : One needs to take a look at the source code of
 * changedistiller to understand the elements that are employed for the Deserialization
 */

/**
 * /**
 *
 * <p>JSON representation of ASTSignatureChange:
 *
 * <p>public class ASTSignatureChange extends DistillerUtil implements SignatureChange
 *
 * <p>public class ASTConstructBodySignature extends ASTSignature public abstract class ASTSignature
 * extends Node implements Signature
 *
 * <p>SourceCodeChange
 *
 * <p>1. private ChangeType fChangeType = ChangeType.UNCLASSIFIED_CHANGE;
 *
 * <p>//Structure entity in which the change operation happened, e.g., attribute, class, or method.
 * 2. private StructureEntityVersion fRootEntity;
 *
 * <p>3. private SourceCodeEntity fChangedEntity; - fUniqueName : String //same information as
 * fValue above - fType : EntityType //same information ast fLabel above - fModifiers : int // this
 * is always zero (Not sure if i should have it in the json object) - fAssociatedEntities :
 * List&lt;SourceCodeEntity&gt; - SourceRange fRange : SourceRange - end : int - start : int
 *
 * <p>//Source code entity that becomes the parent entity when the change is applied. 4. private
 * SourceCodeEntity fParentEntity;
 *
 * <p>public class ASTSignatureChange{
 *
 * <p>private ChangeType changeType; private StructureEntityVersion StructureEntityVersion; private
 * SourceCodeEntity changedEntity; private SourceCodeEntity r }
 *
 * <p>A Source code change has this JSON object format { "changeType" : " ",
 * "StructureEntityVersion" : { "EntityType" : "", "UniqueName" : "", "Modifiers" : "", // might be
 * discarded, int "Version " : "" }, "changedEntity" : { "UniqueName" : "", "EntityType " : "",
 * "Modifiers" : "", // might be discarded, int "SourceRange" : { "start" : "", "end" : "" } } ,
 * "parentEntity" : { "UniqueName" : "", "EntityType " : "", "Modifiers" : "", // might be
 * discarded, int "SourceRange" : { "start" : "", "end" : "" }
 *
 * <p>} }
 */
public class ASTSignatureChangeDeserializer implements JsonDeserializer<SignatureChange> {
  ...
  // representation of the ASTSignatureChange
  // For instance, if we want to  compress  the json object -  not to store a large amount data in
  // the DB.
  ...
  /** {@inheritDoc} */
  @Override
  public SignatureChange deserialize(
    ...
    EntityType type =
        ASTUtil.getJavaEntityType(
            strEntityJsonObject.get(STRUCTURE_ENTITY_ENTITY_TYPE).getAsString());
    ...
    for (int j = 0; j < srcCodeChanges.length; j++) {
      ...
      // Depending on the "OperationType" of the "changes" element, initialize the appropriate
      // sourceCodeChange element (Insert, Update, Move, Delete)
      ...
      if (operationType.equals("Insert")) {
        ...
        SourceCodeEntity insertedEntity =
            this.getSourceCodeEntityElement(
                jsonChangesChild.get("InsertedEntity").getAsJsonObject());
        SourceCodeEntity parentEntity =
            this.getSourceCodeEntityElement(
                jsonChangesChild.getAsJsonObject("ParentEntity").getAsJsonObject());
        ...
        SourceCodeEntity movedEntity =
            this.getSourceCodeEntityElement(jsonChangesChild.get("MovedEntity").getAsJsonObject());
        SourceCodeEntity newEntity =
            this.getSourceCodeEntityElement(
                jsonChangesChild.getAsJsonObject("NewEntity").getAsJsonObject());
        SourceCodeEntity oldParentEntity =
            this.getSourceCodeEntityElement(
                jsonChangesChild.get("OldParentEntity").getAsJsonObject());
        SourceCodeEntity newParentEntity =
            this.getSourceCodeEntityElement(
                jsonChangesChild.getAsJsonObject("NewParentEntity").getAsJsonObject());

        // srcCodeChange.setRootEntity(rootEntity);  //StructureEntityVersion
        srcCodeChange =
            new Move(
                changeType, rootEntity, movedEntity, newEntity, oldParentEntity, newParentEntity);
      ...
      // case "Update":
        ...
        SourceCodeEntity updatedEntity =
            this.getSourceCodeEntityElement(
                jsonChangesChild.get("UpdatedEntity").getAsJsonObject());
        SourceCodeEntity newEntityUpdate =
            this.getSourceCodeEntityElement(
                jsonChangesChild.getAsJsonObject("NewEntity").getAsJsonObject());
        SourceCodeEntity parentEntityUpdate =
            this.getSourceCodeEntityElement(jsonChangesChild.get("ParentEntity").getAsJsonObject());
      ...
      // case "Delete":
        ...
        SourceCodeEntity deletedEntity =
            this.getSourceCodeEntityElement(
                jsonChangesChild.get("DeletedEntity").getAsJsonObject());
        SourceCodeEntity parentEntityDelete =
            this.getSourceCodeEntityElement(jsonChangesChild.get("ParentEntity").getAsJsonObject());
