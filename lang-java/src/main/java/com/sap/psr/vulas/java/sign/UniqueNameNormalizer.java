...
public class UniqueNameNormalizer implements IUniqueNameNormalizer {
  ...
  /**
   * Sets the class loader used for inlining constants, see {@link
   * UniqueNameNormalizer#findConstants(String, String)}.
   *
   ...
   * In more detail, the string is trimmed, occurrences of "this." are removed and constants are
   * inlined.
   *
   ...
   * {@inheritDoc}
   *
   * <p>Applies changes to the unique name of the given {@link SourceCodeEntity}, potentially
   * dependent on its specific {@link EntityType}.
   */
  @Override
   ...
   *
   * <p>Todo: Until now, the method only distinguishes primitive types and everything else. In both
   * cases, the value is obtained by calling toString. In the latter case, the value is additionally
   * wrapped in quotes. One must probably distinguish further cases, e.g., chars wrapper by single
   * quotes.
   *
   * @param _field
   * @return
  ...
  /**
   * Searches for classes or interfaces of the given name (w/o package qualifier) and which have a
   * constant of the given name.
   *
   * <p>Considers all classes before passed to {@link UniqueNameNormalizer#addStrings(Collection)}
   * or the other two methods. At the same time, the classes must be in the class path so that they
   * can be loaded.
   *
   * @param _class_name
   * @param _field_name
