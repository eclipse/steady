...
public class TouchPointCollector {
        ...
        if (!ExecutionMonitor.isPaused()) {
            ...
            if (caller != null) {
                ...
                if (tp_direction != null) {
                    final java.net.URL caller_url =
                            ((com.sap.psr.vulas.java.JavaId) caller).getJarUrl();

                    // Build from method args
                    final ConstructId callee =
                            ConstructIdUtil.getInstance()
                                    .getConstructidFromQName(_callee_qname, _callee_type);
                    ...
                    // Create and collect the touch point
                        ...
                        final String callee_jar_path =
                                (_callee_url == null
                                        ? null
                                        : FileUtil.getJARFilePath(_callee_url.toString()));
                        JarAnalyzer callee_ja = null;
                        if (callee_jar_path != null)
                            callee_ja = tpi.getJarAnalyzerByPath(callee_jar_path);

                        final String caller_jar_path =
                                (caller_url == null
                                        ? null
                                        : FileUtil.getJARFilePath(caller_url.toString()));
                        JarAnalyzer caller_ja = null;
                        if (caller_jar_path != null)
                            caller_ja = tpi.getJarAnalyzerByPath(caller_jar_path);
    ...
    public void uploadInformation(AbstractGoal _exe, int _batch_size) {
        if (this.touchPoints.isEmpty()) {
            ...
            TouchPointCollector.getLog()
                    .info(
                            "Preparing a total of ["
                                    + this.touchPoints.size()
                                    + "] touch points for upload");
        }
    ...
    private static class TouchPoint {
        ...
        public String toString() {
            return "["
                    + this.caller.getQualifiedName()
                    + " --> "
                    + this.callee.getQualifiedName()
                    + "]";
        }
