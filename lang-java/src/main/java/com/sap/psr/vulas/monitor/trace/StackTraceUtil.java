...
/**
 * Transforms a stacktrace, obtained from {@link Throwable} as an array of {@link
 * StackTraceElement}, into a linked list of corresponding construct identifiers.
 *
 * <p>Analyzes the bytecode in order to find methods in case of method overloading (thereby using
 * the line numbers provided in the stack trace).
 *
...
public class StackTraceUtil {
  ...
  private static final Log getLog() {
    if (StackTraceUtil.log == null) StackTraceUtil.log = LogFactory.getLog(StackTraceUtil.class);
  ...
  /**
   * Setter for the field <code>stopAtJUnit</code>.
   *
   * @param _b a boolean.
   */
  ...
  /**
   * Returns the JUnit test case representing the starting point of a previously computed path. If
   * no JUnit test case is present, the method returns null.
   *
  ...
  /**
   * Transforms the stacktrace (obtained by, for instance, new Throwable().getStackTrace()) into a
   * linked list of construct identifiers.
   *
   * @param _st an array of {@link java.lang.StackTraceElement} objects.
   * @param _change_list_element the construct ID for which the stack trace was collected (if known
   *     by caller, can be null)
  ...
  public List<PathNode> transformStackTrace(
    ...
    while (cont && i < _st.length) {
      ...
      if (i == 0
        ...
        // Other elements: Find and add a construct ID for the next stack trace element to <path>
  ...
  private boolean getConstructId(List<PathNode> _path, StackTraceElement _e) {
    ...
    try {
      ...
      if (_e.getMethodName().equals("<init>")) {
        ...
        if (construct_ids.size() > 1) {
          if (_e.getLineNumber() <= 0)
            StackTraceUtil.getLog()
                .warn("Stack trace element w/o line number info, cannot use Javassist: " + _e);
          else {
            try {
              ...
              if (the_one != null) {
                ...
                StackTraceUtil.getLog()
                    .error(
                        "Could not determine constructor despite line information: ["
                            + _e.toString()
                            + "]");
              }
          ...
          if (_e.getLineNumber() <= 0)
            ...
            try {
              ...
              if (the_one != null) {
                ...
                StackTraceUtil.getLog()
                    .error(
                        "Could not determine method despite line information: ["
                            + _e.toString()
                            + "]");
              }
            } catch (NotFoundException e) {
      ...
      if (construct_ids.size() == 0) {
        ...
        // StackTraceUtil.getLog().info("    Class [" + ste.getClassName() + "] has 1 method with
  ...
  /**
   * Returns true if the given JavaId has the annotation "org.junit.Test" and the stack trace util
   * has been configured to stop at JUnit tests, false otherwise.
   *
  ...
  private boolean continueStacktraceTransformation(JavaId _jid) {
    ...
    if (_jid == null) cont = false;
      ...
      if (this.stopAtJUnit) {
        StackTraceUtil.getLog()
            .debug("Found JUnit test [" + _jid.getQualifiedName() + "]: Stop path construction");
        cont = false;
      } else StackTraceUtil.getLog().debug("Found JUnit test [" + _jid.getQualifiedName() + "]");
    }
    return cont;
  }
  ...
  private ConstructId filterMethods(StackTraceElement _e, List<ConstructId> _methods)
    ...
    for (ConstructId m : _methods) {
      // Loop all methods of the class and find the one closest to the line number of the stack
      // trace element
      for (CtMethod ctm : ctclass.getDeclaredMethods()) {
        if (m.getQualifiedName()
                .equals(ClassVisitor.removeParameterQualification(ctm.getLongName()))
  ...
  private ConstructId filterConstructors(StackTraceElement _e, List<ConstructId> _constructors)
    ...
    final CtClass ctclass =
        cp.get(
            ((JavaConstructorId) _constructors.get(0)).getDefinitionContext().getQualifiedName());

    // Loop all constructors of the class and find the one closest to the line number of the stack
    // trace element
    int shortest_distance = Integer.MAX_VALUE;
    int current_distance = -1;
    ConstructId c = null;
    ...
    if (constructor_found == null && !constructor_line_numbers.isEmpty()) {
      constructor_found =
          constructor_line_numbers.get(constructor_line_numbers.keySet().iterator().next());
      StackTraceUtil.getLog()
          .debug(
    ...
    if (_st.length > 2 && StackTraceUtil.constructIds != null) {
      ...
      // We did not search before, search now
        ...
        // We failed, e.g., because the class could not be found
          ...
          // StackTraceUtil.getLog().error("Could not create construct from stack trace element [" +
          // _st[1] + "], which is the predecessor of [" + _st[0] + "]");
          return null;
        }
      }
    }
