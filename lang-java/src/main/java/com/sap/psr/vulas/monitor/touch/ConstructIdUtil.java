...
public class ConstructIdUtil {
  ...
  private static final Log getLog() {
    if (ConstructIdUtil.log == null) ConstructIdUtil.log = LogFactory.getLog(ConstructIdUtil.class);
  ...
  private ConstructIdUtil() {
    try {
      final Set<com.sap.psr.vulas.shared.json.model.ConstructId> app_constructs =
  ...
  /**
   * Checks whether the given {@link ConstructId} is part of the application under analysis. The
   * check is implemented by looing at the definition context, which should be either class or enum.
   * The reason is that, e.g., static initializers are not considered at the time of the source code
   * analysis, hence, are not part of the collection.
   *
  ...
  /**
   * Returns true if the given {@link JavaId} is an instance of {@link JavaClassInit}, {@link
   * JavaMethodId} or {@link JavaConstructorId}, false otherwise.
   *
   * @param _jid a {@link com.sap.psr.vulas.ConstructId} object.
   * @return a boolean.
   */
  ...
  /**
   * Given a qualified name return the ConstructId that represent it. For now is implemented only on
   * constructors and method (also including &lt;clinit&gt; and &lt;init&gt;). If the type requested
   * is null or is not in the range of teh allowed types the method return null
   *
