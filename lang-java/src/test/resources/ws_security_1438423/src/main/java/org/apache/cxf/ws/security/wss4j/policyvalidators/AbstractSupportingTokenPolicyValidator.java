...
public abstract class AbstractSupportingTokenPolicyValidator extends AbstractTokenPolicyValidator
  ...
  private static final Logger LOG =
      LogUtils.getL7dLogger(AbstractSupportingTokenPolicyValidator.class);
  ...
  public void setUsernameTokenResults(
      List<WSSecurityEngineResult> utResultsList, boolean valUsernameToken) {
  ...
  protected boolean processUsernameTokens() {
    ...
    if ((endorsed && !checkEndorsed(tokenResults))
        || !validateSignedEncryptedPolicies(tokenResults)) {
      return false;
    }
  ...
  protected boolean processKeyValueTokens() {
    ...
    for (WSSecurityEngineResult wser : signedResults) {
      PublicKey publicKey = (PublicKey) wser.get(WSSecurityEngineResult.TAG_PUBLIC_KEY);
      if (publicKey != null) {
        tokenResults.add(wser);
      }
    }
  ...
  private boolean validateSignedEncryptedPolicies(List<WSSecurityEngineResult> tokenResults) {
    ...
    if (!validateSignedEncryptedElements(
        encryptedElements, false, encryptedResults, tokenResults)) {
      return false;
    }
  ...
  /**
   * Find an EncryptedKey element that has a cert that matches the cert of the signature, then find
   * a DerivedKey element that matches that EncryptedKey element.
   */
  ...
  /**
   * Get a security result representing a Derived Key that has a secret key that matches the
   * parameter.
   */
  ...
  /**
   * Check the endorsing supporting token policy. If we're using the Transport Binding then check
   * that the Timestamp is signed. Otherwise, check that the signature is signed.
   *
  ...
  private boolean checkSignatureOrEncryptionResult(
      WSSecurityEngineResult result, List<WSSecurityEngineResult> tokenResult) {
