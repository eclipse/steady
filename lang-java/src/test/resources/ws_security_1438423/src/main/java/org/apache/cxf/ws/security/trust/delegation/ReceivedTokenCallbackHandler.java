 ...
 * DelegationCallback object, and obtains a received token (SAML/UsernameToken/BinarySecurityToken)
 * from it to be used as the delegation token.
 */
  ...
  public void handle(Callback[] callbacks) throws IOException, UnsupportedCallbackException {
    for (int i = 0; i < callbacks.length; i++) {
      if (callbacks[i] instanceof DelegationCallback) {
        ...
        if (message != null && message.get(PhaseInterceptorChain.PREVIOUS_MESSAGE) != null) {
          WeakReference<SoapMessage> wr =
              (WeakReference<SoapMessage>) message.get(PhaseInterceptorChain.PREVIOUS_MESSAGE);
  ...
  private Element findToken(List<WSSecurityEngineResult> wsSecEngineResults) {
    for (WSSecurityEngineResult wser : wsSecEngineResults) {
      ...
      if (actInt.intValue() == WSConstants.ST_SIGNED
        ...
        UsernameToken token = (UsernameToken) wser.get(WSSecurityEngineResult.TAG_USERNAME_TOKEN);
