...
public final class SAMLUtils {
  ...
  //
  private static List<String> parseRolesInAssertion(
      org.opensaml.saml1.core.Assertion assertion, String roleAttributeName) {
  ...
  private static List<String> parseRolesInAssertion(
      org.opensaml.saml2.core.Assertion assertion, String roleAttributeName) {
  ...
  public static void validateSAMLResults(
      List<WSSecurityEngineResult> results, Message message, Element body)
  ...
  /**
   * Check the holder-of-key requirements against the received assertion. The subject credential of
   * the SAML Assertion must have been used to sign some portion of the message, thus showing
   * proof-of-possession of the private/secret key. Alternatively, the subject credential of the
   * SAML Assertion must match a client certificate credential when 2-way TLS is used.
   *
  ...
  /**
   * Compare the credentials of the assertion to the credentials used in 2-way TLS or those used to
   * verify signatures. Return true on a match
   *
  ...
  public static boolean compareCredentials(
    ...
    for (WSSecurityEngineResult signedResult : signedResults) {
      ...
      PublicKey publicKey = (PublicKey) signedResult.get(WSSecurityEngineResult.TAG_PUBLIC_KEY);
      byte[] secretKey = (byte[]) signedResult.get(WSSecurityEngineResult.TAG_SECRET);
      if (certs != null
          && certs.length > 0
          && subjectCerts != null
          && subjectCerts.length > 0
  ...
  private static boolean checkSecretKey(
      byte[] secretKey, byte[] subjectSecretKey, WSSecurityEngineResult signedResult) {
    if (secretKey != null && subjectSecretKey != null) {
      if (Arrays.equals(secretKey, subjectSecretKey)) {
        return true;
      } else {
        Principal principal = (Principal) signedResult.get(WSSecurityEngineResult.TAG_PRINCIPAL);
  ...
  /**
   * Check the sender-vouches requirements against the received assertion. The SAML Assertion and
   * the SOAP Body must be signed by the same signature.
   */
  ...
  private static boolean checkAssertionAndBodyAreSigned(
      AssertionWrapper assertionWrapper, Element body, List<WSSecurityEngineResult> signed) {
