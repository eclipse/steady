...
public class Trust13Builder implements AssertionBuilder<Element> {
  ...
  public Assertion build(Element element, AssertionBuilderFactory factory) {
    ...
    if (DOMUtils.getFirstChildWithName(element, SP12Constants.MUST_SUPPORT_CLIENT_CHALLENGE)
        != null) {
      trust13.setMustSupportClientChallenge(true);
    }

    if (DOMUtils.getFirstChildWithName(element, SP12Constants.MUST_SUPPORT_SERVER_CHALLENGE)
        != null) {
    ...
    if (DOMUtils.getFirstChildWithName(
            element, SP12Constants.REQUIRE_REQUEST_SECURITY_TOKEN_COLLECTION)
        != null) {
