 ...
 * close()} method as soon as possible, to force <tt>BZip2CompressorOutputStream</tt> to release the
 * allocated memory.
 *
 ...
 * </table>
 *
 * <p>For decompression <tt>BZip2CompressorInputStream</tt> allocates less memory if the bzipped
...
public class BZip2CompressorOutputStream extends CompressorOutputStream implements BZip2Constants {
   ...
   *     inclusive. For a negative <tt>inputLength</tt> this method returns <tt>MAX_BLOCKSIZE</tt>
   *     always.
  ...
  /**
   * Writes magic bytes like BZ on the first position of the stream and bytes indiciating the
   ...
   *
   * <p>This is the version using unrolled loops. Normally I never use such ones in Java code. The
   * unrolling has shown a noticable performance improvement on JRE 1.4.2 (Linux i586 / HotSpot
   * Client). Of course it depends on the JIT compiler of the vm.
   */
  ...
  private void mainQSort3(final Data dataShadow, final int loSt, final int hiSt, final int dSt) {
    ...
    for (int sp = 1; --sp >= 0; ) {
      ...
      if ((hi - lo < SMALL_THRESH) || (d > DEPTH_THRESH)) {
        ...
        final int d1 = d + 1;
        final int med =
            med3(block[fmap[lo] + d1], block[fmap[hi] + d1], block[fmap[(lo + hi) >>> 1] + d1])
                & 0xff;
  ...
  private static final class Data extends Object {
    ...
    /**
     * Array instance identical to sfmap, both are used only temporarily and indepently, so we do
     * not need to allocate additional memory.
     */
    final char[] quadrant;
