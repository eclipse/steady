...
public abstract class AbstractWSS4JInterceptor extends WSHandler
  ...
  protected void translateProperties(SoapMessage msg) {
    ...
    String futureTTL = (String) msg.getContextualProperty(SecurityConstants.TIMESTAMP_FUTURE_TTL);
    if (futureTTL != null) {
      msg.setContextualProperty(WSHandlerConstants.TTL_FUTURE_TIMESTAMP, futureTTL);
    }
    String ttl = (String) msg.getContextualProperty(SecurityConstants.TIMESTAMP_TTL);
    if (ttl != null) {
    ...
    try {
      try {
        ...
        if (url != null) {
          ...
          props.load(in);
          in.close();
          return CryptoFactory.getInstance(props, this.getClassLoader(reqData.getMsgContext()));
        }
      } catch (Exception e) {
        // ignore
      }
