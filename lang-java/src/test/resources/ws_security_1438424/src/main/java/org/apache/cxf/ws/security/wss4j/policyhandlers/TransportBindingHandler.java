...
public class TransportBindingHandler extends AbstractBindingBuilder {
  ...
  private void handleEndorsingToken(Token token, SupportingToken wrapper) throws Exception {
  ...
  private byte[] doX509TokenSignature(Token token, SupportingToken wrapper) throws Exception {
  ...
  private byte[] doIssuedTokenSignature(Token token, SupportingToken wrapper) throws Exception {
  ...
  private byte[] doDerivedKeySignature(
      boolean tokenIncluded, SecurityToken secTok, Token token, List<WSEncryptionPart> sigParts)
      throws Exception {
  ...
  private byte[] doSignature(
    ...
    if (ref != null) {
      ...
      int type =
          tokenIncluded ? WSConstants.CUSTOM_SYMM_SIGNING : WSConstants.CUSTOM_SYMM_SIGNING_DIRECT;
  ...
  private List<WSEncryptionPart> signPartsAndElements(
      SignedEncryptedParts signedParts, SignedEncryptedElements signedElements)
    ...
    if (signedElements != null) {
      ...
      try {
        result.addAll(
            this.getElements(
                "Element",
                signedElements.getXPathExpressions(),
                signedElements.getDeclaredNamespaces(),
                found,
                true));
