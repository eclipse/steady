...
/**
 * Configuration tags used to configure the WS-SecurityPolicy layer. Some of them are also used by
 * the non WS-SecurityPolicy approach in the WSS4J(Out|In)Interceptors.
 */
public final class SecurityConstants {
  ...
  /**
   * The user's name. It is used differently by each of the WS-Security functions: a) It is used as
   * the name in the UsernameToken b) It is used as the alias name in the keystore to get the user's
   * cert and private key for signature if {@link SIGNATURE_USERNAME} is not set. c) It is used as
   * the alias name in the keystore to get the user's public key for encryption if {@link
   * ENCRYPT_USERNAME} is not set.
   */
  public static final String USERNAME = "ws-security.username";

  /**
   * The user's password when a {@link CALLBACK_HANDLER} is not defined. It is currently only used
   * for the case of adding a password to a UsernameToken.
   */
   ...
   * cert and private key for signature. If this is not defined, then {@link USERNAME} is used
   * instead. If that is also not specified, it uses the the default alias set in the properties
   * file referenced by {@link SIGNATURE_PROPERTIES}. If that's also not set, and the keystore only
   * contains a single key, that key will be used.
   */
  public static final String SIGNATURE_USERNAME = "ws-security.signature.username";

  /**
   * The user's name for encryption. It is used as the alias name in the keystore to get the user's
   * public key for encryption. If this is not defined, then {@link USERNAME} is used instead. If
   * that is also not specified, it uses the the default alias set in the properties file referenced
   * by {@link ENCRYPT_PROPERTIES}. If that's also not set, and the keystore only contains a single
   * key, that key will be used.
   *
   * <p>For the web service provider, the "useReqSigCert" keyword can be used to accept (encrypt to)
   * any client whose public key is in the service's truststore (defined in {@link
   * ENCRYPT_PROPERTIES}).
   */
  ...
  /**
   * The CallbackHandler implementation class used to obtain passwords, for both outbound and
   * inbound requests. The value of this tag must be either: a) The class name of a {@link
   * javax.security.auth.callback.CallbackHandler} instance, which must be accessible via the
   * classpath. b) A {@link javax.security.auth.callback.CallbackHandler} instance.
   */
  public static final String CALLBACK_HANDLER = "ws-security.callback-handler";

  /**
   * The SAML CallbackHandler implementation class used to construct SAML Assertions. The value of
   * this tag must be either: a) The class name of a {@link
   * javax.security.auth.callback.CallbackHandler} instance, which must be accessible via the
   * classpath. b) A {@link javax.security.auth.callback.CallbackHandler} instance.
   */
  ...
  /**
   * The Crypto property configuration to use for signature, if {@link SIGNATURE_CRYPTO} is not set
   * instead. The value of this tag must be either: a) A Java Properties object that contains the
   * Crypto configuration. b) The path of the Crypto property file that contains the Crypto
   * configuration. c) A URL that points to the Crypto property file that contains the Crypto
   * configuration.
   */
  public static final String SIGNATURE_PROPERTIES = "ws-security.signature.properties";

  /**
   * The Crypto property configuration to use for encryption, if {@link ENCRYPT_CRYPTO} is not set
   * instead. The value of this tag must be either: a) A Java Properties object that contains the
   * Crypto configuration. b) The path of the Crypto property file that contains the Crypto
   * configuration. c) A URL that points to the Crypto property file that contains the Crypto
   * configuration.
  ...
  /**
   * A Crypto object to be used for signature. If this is not defined then the {@link
   * SIGNATURE_PROPERTIES} is used instead.
   */
  public static final String SIGNATURE_CRYPTO = "ws-security.signature.crypto";

  /**
   * A Crypto object to be used for encryption. If this is not defined then the {@link
   * ENCRYPT_PROPERTIES} is used instead.
   */
  ...
  /**
   * Whether to enable Certificate Revocation List (CRL) checking or not when verifying trust in a
  ...
  /**
   * Whether to always encrypt UsernameTokens that are defined as a SupportingToken. The default is
   * true. This should not be set to false in a production environment, as it exposes the password
   * (or the digest of the password) on the wire.
   */
  ...
  /**
   * Whether to self-sign a SAML Assertion or not. If this is set to true, then an enveloped
   * signature will be generated when the SAML Assertion is constructed. The default is false.
   */
   ...
   * Signature).The default value is "true" for message recipients, and "false" for message
   * initiators. Set it to true to cache for both cases. Set this to "false" to not cache Timestamp
   * Created Strings.
   */
  ...
  /**
   * The time in seconds in the future within which the Created time of an incoming Timestamp is
   * valid. The default value is "60", to avoid problems where clocks are slightly askew. To reject
   * all future-created Timestamps, set this value to "0".
   */
  public static final String TIMESTAMP_FUTURE_TTL = "ws-security.timestamp.futureTimeToLive";

  /**
   * The attribute URI of the SAML AttributeStatement where the role information is stored. The
   * default is "http://schemas.xmlsoap.org/ws/2005/05/identity/claims/role".
   */
  ...
  /**
   * The SpnegoClientAction implementation to use for SPNEGO. This allows the user to plug in a
   * different implementation to obtain a service ticket.
   */
  ...
  /**
   * A comma separated String of regular expressions which will be applied to the subject DN of the
   * certificate used for signature validation, after trust verification of the certificate chain
   * associated with the certificate. These constraints are not used when the certificate is
  ...
  /**
   * The WSS4J Validator instance to use to validate trust in credentials used in Signature
   * verification. The default value is the SignatureTrustValidator.
   */
  public static final String SIGNATURE_TOKEN_VALIDATOR = "ws-security.signature.validator";

  /**
   * The WSS4J Validator instance to use to validate BinarySecurityTokens. The default value is the
   * NoOpValidator.
   */
  public static final String BST_TOKEN_VALIDATOR = "ws-security.bst.validator";

  /**
   * The WSS4J Validator instance to use to validate SecurityContextTokens. The default value is the
   * NoOpValidator.
   */
  ...
  /**
   * The "AppliesTo" address to send to the STS. The default is the endpoint address of the service
   * provider.
   */
  public static final String STS_APPLIES_TO = "ws-security.sts.applies-to";

  /**
   * Whether to write out an X509Certificate structure in UseKey/KeyInfo, or whether to write out a
   * KeyValue structure. The default value is "false".
   */
   ...
   * IssuedTokenInterceptorProvider. This should be done if a token is being retrieved from an STS
   * in an intermediary. The default value is "true".
   */
  public static final String CACHE_ISSUED_TOKEN_IN_ENDPOINT =
      "ws-security.cache.issued.token.in.endpoint";

  /**
   * Whether to avoid STS client trying send WS-MetadataExchange call using STS EPR WSA address when
   * the endpoint contract contains no WS-MetadataExchange info. The default value is "false".
   */
  ...
  public static final String STS_CLIENT_SOAP12_BINDING = "ws-security.sts.client-soap12-binding";

  /**
   * A Crypto object to be used for the STS. If this is not defined then the {@link
   * STS_TOKEN_PROPERTIES} is used instead.
   *
   * <p>WCF's trust server sometimes will encrypt the token in the response IN ADDITION TO the full
   * security on the message. These properties control the way the STS client will decrypt the
   * EncryptedData elements in the response.
   *
   * <p>These are also used by the STSClient to send/process any RSA/DSAKeyValue tokens used if the
   * KeyType is "PublicKey"
   */
  public static final String STS_TOKEN_CRYPTO = "ws-security.sts.token.crypto";

  /**
   * The Crypto property configuration to use for the STS, if {@link STS_TOKEN_CRYPTO} is not set
   * instead. The value of this tag must be either: a) A Java Properties object that contains the
   * Crypto configuration. b) The path of the Crypto property file that contains the Crypto
   * configuration. c) A URL that points to the Crypto property file that contains the Crypto
   * configuration.
  ...
  /**
   * The token to be sent to the STS in an "ActAs" field. It can be either: a) A String b) A DOM
   * Element c) A CallbackHandler object to use to obtain the token
   */
  public static final String STS_TOKEN_ACT_AS = "ws-security.sts.token.act-as";

  /**
   * The token to be sent to the STS in an "OnBehalfOf" field. It can be either: a) A String b) A
   * DOM Element c) A CallbackHandler object to use to obtain the token
   */
