...
public class SecurityToken implements Serializable {
  ...
  /**
   * The RequestedAttachedReference element NOTE : The oasis-200401-wss-soap-message-security-1.0
   * spec allows an extensibility mechanism for wsse:SecurityTokenReference and wsse:Reference.
   * Hence we cannot limit to the wsse:SecurityTokenReference\wsse:Reference case and only hold the
   * URI and the ValueType values.
   */
  private transient Element attachedReference;

  /**
   * The RequestedUnattachedReference element NOTE : The oasis-200401-wss-soap-message-security-1.0
   * spec allows an extensibility mechanism for wsse:SecurityTokenReference and wsse:Reference.
   * Hence we cannot limit to the wsse:SecurityTokenReference\wsse:Reference case and only hold the
   * URI and the ValueType values.
   */
  ...
  /**
   * This holds the identifier of another SecurityToken which represents a transformed version of
   * this token.
   */
  ...
  private void processLifeTime(Element lifetimeElem) {
    try {
      Element createdElem =
          DOMUtils.getFirstChildWithName(lifetimeElem, WSConstants.WSU_NS, WSConstants.CREATED_LN);
      DateFormat zulu = new XmlSchemaDateFormat();

      this.created = zulu.parse(DOMUtils.getContent(createdElem));

      Element expiresElem =
          DOMUtils.getFirstChildWithName(lifetimeElem, WSConstants.WSU_NS, WSConstants.EXPIRES_LN);
