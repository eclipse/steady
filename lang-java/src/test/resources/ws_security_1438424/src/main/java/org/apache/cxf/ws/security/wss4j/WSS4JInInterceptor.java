...
public class WSS4JInInterceptor extends AbstractWSS4JInterceptor {
  ...
  /**
   * This configuration tag specifies the default attribute name where the roles are present The
  ...
  public WSS4JInInterceptor(Map<String, Object> properties) {
    ...
    final Map<QName, Object> processorMap =
        CastUtils.cast((Map<?, ?>) properties.get(PROCESSOR_MAP));
    final Map<QName, Object> validatorMap =
        CastUtils.cast((Map<?, ?>) properties.get(VALIDATOR_MAP));
  ...
  public void handleMessage(SoapMessage msg) throws Fault {
    ...
    try {
      ...
      ReplayCache timestampCache =
          getReplayCache(
              msg,
              SecurityConstants.ENABLE_TIMESTAMP_CACHE,
      ...
      boolean enableRevocation =
          reqData.isRevocationEnabled()
              || MessageUtils.isTrue(
                  msg.getContextualProperty(SecurityConstants.ENABLE_REVOCATION));
      ...
      if (wsResult != null && !wsResult.isEmpty()) { // security header found
        ...
        if (doc.getSOAPPart().getEnvelope().getBody().hasFault()) {
          ...
          doResults(msg, actor, SAAJUtils.getHeader(doc), SAAJUtils.getBody(doc), wsResult);
        } else {
          checkActions(msg, reqData, wsResult, actions, SAAJUtils.getBody(doc));
          doResults(msg, actor, SAAJUtils.getHeader(doc), SAAJUtils.getBody(doc), wsResult);
        }
  ...
  private void checkActions(
    ...
    String signatureParts = (String) getProperty(msg, WSHandlerConstants.SIGNATURE_PARTS);
    if (signatureParts != null) {
      String warning =
          "To enforce that particular elements were signed you must either "
  ...
  /**
   * Do whatever is necessary to determine the action for the incoming message and do whatever other
   * setup work is necessary.
   *
   * @param msg
   * @param reqData
   */
  ...
  /**
   * Set a WSS4J AlgorithmSuite object on the RequestData context, to restrict the algorithms that
   * are allowed for encryption, signature, etc.
   */
  protected void setAlgorithmSuites(SoapMessage message, RequestData data)
      throws WSSecurityException {
  ...
  /**
   * Checks if a given WSS4J Principal can be represented as a user principal inside
   * SecurityContext. Example, UsernameToken or PublicKey principals can be used to facilitate
   * checking the user roles, etc.
   */
  ...
  /**
   * @return the WSSecurityEngine in use by this interceptor. This engine is defined to be the
   *     secEngineOverride instance, if defined in this class (and supplied through construction);
   *     otherwise, it is taken to be the default WSSecEngine instance (currently defined in the
   *     WSHandler base class).
   */
  ...
  /**
   * @return a freshly minted WSSecurityEngine instance, using the (non-null) processor map, to be
   *     used to initialize the WSSecurityEngine instance.
   */
  ...
  /**
   * Get a ReplayCache instance. It first checks to see whether caching has been explicitly enabled
   * or disabled via the booleanKey argument. If it has been set to false then no replay caching is
   * done (for this booleanKey). If it has not been specified, then caching is enabled only if we
   * are not the initiator of the exchange. If it has been specified, then caching is enabled.
   *
   * <p>It tries to get an instance of ReplayCache via the instanceKey argument from a contextual
   * property, and failing that the message exchange. If it can't find any, then it defaults to
   * using an EH-Cache instance and stores that on the message exchange.
   */
  ...
  /**
   * Create a SoapFault from a WSSecurityException, following the SOAP Message Security 1.1
   * specification, chapter 12 "Error Handling".
   *
   * <p>When the Soap version is 1.1 then set the Fault/Code/Value from the fault code specified in
   * the WSSecurityException (if it exists).
   *
   * <p>Otherwise set the Fault/Code/Value to env:Sender and the Fault/Code/Subcode/Value as the
   * fault code from the WSSecurityException.
   */
      ...
      if (key != null) {
        ...
        try {
          if (o instanceof Validator) {
            ...
            return (Validator)
                ClassLoaderUtils.loadClass(o.toString(), WSS4JInInterceptor.class).newInstance();
          }
