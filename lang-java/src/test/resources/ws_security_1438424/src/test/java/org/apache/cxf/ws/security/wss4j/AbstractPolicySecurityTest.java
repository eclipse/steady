...
public abstract class AbstractPolicySecurityTest extends AbstractSecurityTest {
  ...
  protected Bus createBus() throws BusException {
    ...
    this.policyBuilder = b.getExtension(PolicyBuilder.class);
    return b;
  }
  ...
  protected void runAndValidate(
      ...
      List<QName> notAssertedOutAssertions,
      List<QName> assertedInAssertions,
  ...
  protected void runAndValidate(
      String document,
      String outPolicyDocument,
      String inPolicyDocument,
    ...
    final Document inDoc =
        this.runOutInterceptorAndValidate(
            originalDoc,
            outPolicy,
            outAssertions.getAssertedAssertions(),
    ...
    */
    ...
    this.runInInterceptorAndValidate(
        inDoc,
        inPolicy,
        inAssertions.getAssertedAssertions(),
        inAssertions.getNotAssertedAssertions(),
        types);
  }
  ...
  protected void runInInterceptorAndValidate(
    ...
    this.runInInterceptorAndValidate(
        document,
        policyDocument,
        assertedInAssertion == null ? null : Arrays.asList(assertedInAssertion),
        notAssertedInAssertion == null ? null : Arrays.asList(notAssertedInAssertion),
        Arrays.asList(type));
  }
  ...
  protected void runInInterceptorAndValidate(
    ...
    this.runInInterceptorAndValidate(
        doc, policy, assertedInAssertions, notAssertedInAssertions, types);
  }
  ...
  protected void runInInterceptorAndValidateWss(
      Document document, AssertionInfoMap aim, List<CoverageType> types) throws Exception {
  ...
  protected Document runOutInterceptorAndValidate(
      Document document,
      Policy policy,
      List<QName> assertedOutAssertions,
      List<QName> notAssertedOutAssertions)
    ...
    return this.runOutInterceptorAndValidate(
        msg, policy, aim, assertedOutAssertions, notAssertedOutAssertions);
  }
  ...
  protected void runOutInterceptorAndValidateSamlTokenAttached(String policyDoc) throws Exception {
    ...
    msg.getExchange().get(Endpoint.class).put(SecurityConstants.TOKEN_ID, issuedToken.getId());
  ...
  /**
   * Gets a SoapMessage, but with the needed SecurityConstants in the context properties so that it
   * can be passed to PolicyBasedWSS4JOutInterceptor.
   *
  ...
  protected void verifyWss4jEncResults(SoapMessage inmsg) {
    ...
    WSSecurityUtil.fetchAllActionResults(
        handlerResults.get(0).getResults(), WSConstants.ENCR, protectionResults);
  ...
  protected void verifySignatureAlgorithms(Document signedDoc, AssertionInfoMap aim)
      throws Exception {
    ...
    final XPathExpression digestAlgoExpr =
        xpath.compile(
  ...
  protected void verifySignatureCoversAssertion(Document signedDoc, String assertionId)
    ...
    final XPathExpression strExpr =
        xpath.compile(
            "/s:Envelope/s:Header/wsse:Security/wsse:SecurityTokenReference/wsse:KeyIdentifier");

    final NodeList strKeyIdNodes = (NodeList) strExpr.evaluate(signedDoc, XPathConstants.NODESET);
    ...
    for (int i = 0; i < strKeyIdNodes.getLength(); i++) {
      ...
      if (strKey.equals(assertionId)) {
        ...
        strId =
            strNode
                .getAttributes()
                .getNamedItemNS(nsContext.getNamespaceURI("wsu"), "Id")
                .getNodeValue();
        break;
      }
    }
    assertNotNull(
        "SecurityTokenReference for " + assertionId + " not found in security header.", strId);
  ...
  protected static final class MockEndpoint extends AbstractAttributedInterceptorProvider
      implements Endpoint {
  ...
  /** A simple container used to reduce argument numbers to satisfy project code conventions. */
  protected static final class AssertionsHolder {
    ...
    public AssertionsHolder(List<QName> assertedAssertions, List<QName> notAssertedAssertions) {
      super();
