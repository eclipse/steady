...
public class ClassPoolUpdaterTest {
    ...
    public void testClassPoolUpdater() {
        try {
            // Relative path: Should not be appended, since the package structure does not exist in
            // the file path
            ...
            final File f2 =
                    new File(
                            "./src/test/resources/com/sap/psr/vulas/java/test/OuterClass$InnerClass.class");
    ...
    public void checkForClint() {
        try {
            ...
            final CtClass basicCtclass =
                    ClassPool.getDefault().makeClass(new FileInputStream(basic));
