...
public class ClassFileAnalyzerTest {
    ...
    public void classFileAnalyzerTest() {
        try {
            ...
            ClassFileAnalyzer cfa =
                    (ClassFileAnalyzer)
                            FileAnalyzerFactory.buildFileAnalyzer(
                                    new File("./src/test/resources/Callgraph.class"));
            final Map<ConstructId, Construct> c = cfa.getConstructs();

            // Check that certain constructs have been found
            JavaConstructorId cid =
                    JavaId.parseConstructorQName("com.sap.psr.vulas.cg.Callgraph(Graph)");
            assertEquals(true, cfa.containsConstruct(cid));

            JavaMethodId mid =
                    JavaId.parseMethodQName("com.sap.psr.vulas.cg.Callgraph.getAllEdges(String)");
            assertEquals(true, cfa.containsConstruct(cid));

            JavaMethodId not_existing_method =
                    JavaId.parseMethodQName("com.sap.psr.vulas.cg.Callgraph.1234(String)");
