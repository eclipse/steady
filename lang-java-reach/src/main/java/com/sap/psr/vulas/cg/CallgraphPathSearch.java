...
public class CallgraphPathSearch implements Runnable {
    ...
    private ArrayList<List<com.sap.psr.vulas.shared.json.model.ConstructId>> paths =
            new ArrayList<List<com.sap.psr.vulas.shared.json.model.ConstructId>>();
    ...
    public CallgraphPathSearch setEntrypoints(
            Set<com.sap.psr.vulas.shared.json.model.ConstructId> entrypoints) {
    ...
    public CallgraphPathSearch setTargetpoints(
            Set<com.sap.psr.vulas.shared.json.model.ConstructId> targetpoints) {
    ...
    public void run() {
        ...
        final HashSet<com.sap.psr.vulas.shared.json.model.ConstructId> reachable_target_points =
        ...
        for (com.sap.psr.vulas.shared.json.model.ConstructId cid : this.targetpoints) {
            ...
            if (this.graph.existsInCallgraph(cid)) {
                ...
                final Map<com.sap.psr.vulas.shared.json.model.ConstructId, Integer> distance =
                        this.graph.getDist(cid);
                for (com.sap.psr.vulas.shared.json.model.ConstructId ep : this.entrypoints) {
                    // get the distance from entry points to changed construct; if it's not -1, then
                    // this changed construct is reachable from entrypoints
                    if (this.graph.existsInCallgraph(ep)) {
                        if (distance.get(ep) != Integer.MAX_VALUE) {
                            reachable_target_points.add(cid);
                            // At this point in time, we should be able to exit the loop over all
                            // entry points, added break
                            break;
                        }
                    }
                }
                ...
                if (reachable_target_points.contains(cid)) {
                    ...
                    for (com.sap.psr.vulas.shared.json.model.ConstructId ep : this.entrypoints) {
                        if (this.graph.existsInCallgraph(ep)) {
                            ...
                            // Linked list of integers, each one being the unique ID given to
                            // constructs in the callgraph
                            ...
                            if (spath != null) {
                                ...
                                // Create a new path (linked list of constructs), thereby resolving
                                // the unique integer IDs used in the callgraph class
                                final LinkedList<com.sap.psr.vulas.shared.json.model.ConstructId>
                                        newpath =
                                                new LinkedList<
                                                        com.sap.psr.vulas.shared.json.model
                                                                .ConstructId>();
                                newpath.add(ep);
