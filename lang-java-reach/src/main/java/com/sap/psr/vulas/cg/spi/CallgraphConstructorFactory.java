...
public class CallgraphConstructorFactory {
  ...
  /**
   * Build a call graph constructor, searching the service registry for a service implementation
   * that registers itself using the given string
   *
  ...
  public static ICallgraphConstructor buildCallgraphConstructor(
    ...
    if (cgConstructor != null) {
      ...
      log.error(
          "No Callgraph Constructor found for requested framework  [" + analysisFramework + "]");
    }
  ...
  private static ClassLoader searchInPluginFolder() {
    ...
    try (DirectoryStream<Path> directoryStream = Files.newDirectoryStream(loc)) {
      ...
      log.warn(
          "Cannot load plugin JARs (with additional call graph constructors) from directory ["
              + loc.toAbsolutePath()
              + "]");
    }
