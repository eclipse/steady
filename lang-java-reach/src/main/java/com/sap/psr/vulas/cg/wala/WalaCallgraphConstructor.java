...
public class WalaCallgraphConstructor implements ICallgraphConstructor {
	...
	public static String getFramework() { return WalaCallgraphConstructor.FRAMEWORK; }
	...
	private Set<com.sap.psr.vulas.shared.json.model.ConstructId> filteredEP = new HashSet<com.sap.psr.vulas.shared.json.model.ConstructId>();
	...
	public WalaCallgraphConstructor (Application _ctx) {
		this.appContext = _ctx;
		//this.appJar = _appjar.toString();
	}
	...
	public void buildCallgraph (boolean _policy) throws CallgraphConstructException {
		...
		try{
			...
			WalaCallgraphConstructor.log.error("Error building callgraph: " + e.getMessage());
			throw new CallgraphConstructException("Error building callgraph", e);
		}		
	}
	...
	private void checkEntrypoints (boolean _policy) throws CallgraphConstructException {
		...
		com.sap.psr.vulas.shared.json.model.ConstructId cid = null;
		for(com.sap.psr.vulas.shared.json.model.ConstructId c  : this.filteredEP) 
			ep_diff.add(c);
		...
		if( (!_policy) && (!ep_diff.isEmpty()) ) {
			...
			WalaCallgraphConstructor.log.info("All [" + this.filteredEP.size() + "] entrypoints exist in the callgraph");
		}
	}
		...
		if(this.callgraph != null){
			...
			while (nodes.hasNext()) {
				...
				while(succNodes.hasNext()){
					...
					if ( !graph.hasEdge(src_cid, tgt_cid)) { graph.addEdge(src_cid, tgt_cid); edges_no++; }
				}
			}
