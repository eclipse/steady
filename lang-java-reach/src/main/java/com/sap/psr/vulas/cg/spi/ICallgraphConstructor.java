...
/**
 * A call graph constructor builds a call graph starting from a given set of entry points. Entry
 * points can be, for instance, all {@link Construct}s of an application, or all {@link Construct}s
 * that have been traced during tests.
 */
public interface ICallgraphConstructor {
  ...
  /**
   * Sets the class path with directories and/or JAR files for the application. Individual entries
   * must be separated by the system-specific path separator.
   *
  ...
  /**
   * Sets the class path with directories and/or JAR files for the dependencies. Individual entries
   * must be separated by the system-specific path separator.
   *
  ...
  /**
   * Sets the entry points for call graph construction. These are the starting point of the call
   * graph construction.
   *
  ...
  /**
   * Excludes certain packages from the analysis, i.e., potential invocations happening in {@link
   * Construct}s of those packages will be ignored.
   *
  ...
  /**
   * Returns the effectively used entry points. {@link Construct}s of type class or package, for
   * instance, will be filtered. Also, it may happen that some of the specified entry points cannot
   * be found, e.g., due to an incomplete class path.
   *
  ...
  /**
   * Builds the call graph. If the argument is set to true, the construction will be aborted if not
   * all of the specified entry points could be used.
   *
   * @throws com.sap.psr.vulas.cg.CallgraphConstructException
