...
public class PathSimilarity {
    ...
    private static final Logger log =
            org.apache.logging.log4j.LogManager.getLogger(PathSimilarity.class);

    // ArrayList: every path has a unique index
    ArrayList<LinkedList<String>> paths = null;

    // group all paths based on the node joint, the map key is the qname of the node joint, the
    // value are all paths id in this group, id is read from ArrayList paths
    ...
    private HashSet<List<String>> overlapPath(
            LinkedList<String> _path1, LinkedList<String> _path2) {
    ...
    public void pathSimilarity(LinkedList<String> _path1, LinkedList<String> _path2) {
        ...
        PathSimilarity.log.info(
                "The lengths of two paths are "
                        + _path1.size()
                        + " and "
                        + _path2.size()
