...
public class Callgraph {
    ...
    public Callgraph(Graph<com.sap.psr.vulas.shared.json.model.ConstructId> _g) {
        if (_g != null) {
            ...
            while (iter.hasNext()) {
                ...
                while (succNodes.hasNext()) {
                    ...
                    if (tgt_id == null) {
                        ...
                        this.nodeInfoMap.put(
                                tgt_id, this.createNodeMetaInformation(tgt_node, tgt_id));
            ...
            Callgraph.log.info(
                    "Built Graph<Integer> of " + this.idgraph.getNumberOfNodes() + " nodes");
        }
    }
    ...
    private Map<Integer, Integer> computeDist(Integer _tgt, Map<Integer, Integer> _dist) {
        ...
        while (pred_nodes.hasNext()) {
            ...
            // HP, 29.11: Changed from -1 for infinite to Integer.MAX_VALUE, which safes one
            // comparison
     ...
     */
    public Map<com.sap.psr.vulas.shared.json.model.ConstructId, LinkedList<Integer>>
            getShortestPath(
                    com.sap.psr.vulas.shared.json.model.ConstructId _tgt,
                    final Set<com.sap.psr.vulas.shared.json.model.ConstructId>
                            _stop_if_path_found) {
        ...
        // the path from target to target is empty (size of linked list = 0), from all other nodes
        // to target is null (= no path)
    ...
    private Map<Integer, LinkedList<Integer>> computeShortestPath(
            Integer _tgt,
            Map<Integer, LinkedList<Integer>> _paths,
            final Set<Integer> _stop_nodes) {
        ...
        while (pred_nodes.hasNext()) {
            ...
            if (_paths.get(prednode) == null || (_paths.get(prednode).size() > new_path_length)) {
                ...
                // Only continue searching if there are no stop nodes, or none of them has been
                // reached yet
    ...
    private com.sap.psr.vulas.shared.json.model.ConstructId parseNonStaticInnerClassConstruct(
        ...
        // For nested classes, get the declaring (outer) class: It is used to skip the first
        // argument in non-static inner classes
    ...
    private synchronized String getShaFromCachedJarAnalyzer(URL _jar_url) {
        ...
        if (!this.jarAnalyzersCache.containsKey(_jar_url)) {
            ...
            try {
                ...
                log.error(
                        "File system not found for [" + _jar_url + "]: " + fsnfe.getMessage(),
                        fsnfe);
