...
/**
 * A general call graph representation for both wala and soot framework. Usage: Graph path
 * computation; rendering/visualization
 */
public class Callgraph {
  ...
  /**
   * Maps all callgraph constructs to integers (useful when wanting to have the unique integer ID
   * for a given construct). The IDs are equivalant to the ones in nodeId.
   *
   * @see Callgraph#nodeId
   */
  private final HashMap<com.sap.psr.vulas.shared.json.model.ConstructId, Integer> nodeMap =
      new HashMap<com.sap.psr.vulas.shared.json.model.ConstructId, Integer>();

  /**
   * Maps all callgraph metaInformation to integers (useful when wanting to have the unique integer
   * ID for a given construct). The IDs are equivalant to the ones in nodeId.
   *
   * @see Callgraph#nodeId
   */
  ...
  /**
   * For each URL contains a JarAnalyzer Object. This allows us to get every information about the
   * JAR of each node without the need to create an instance of JarAnalyzer for all of them. Every
   * node is a method so most of them share the JAR with others methods
   */
  ...
  private final Set<com.sap.psr.vulas.shared.json.model.ConstructId> constructsWithoutJarUrl =
      new HashSet<com.sap.psr.vulas.shared.json.model.ConstructId>();

  /**
   * Returns the unique integer ID of the given construct in the context of this callgraph, or -1 if
  ...
  /**
   * Contains all callgraph constructs (useful when wanting a construct for a given integer ID). The
   * IDs are equivalant to the ones in nodeMap.
   *
   * @see Callgraph#nodeMap
   */
  private final ArrayList<com.sap.psr.vulas.shared.json.model.ConstructId> nodeId =
  ...
  /**
   * Creates a callgraph and populates an internal map of constructs and integer indices (which
   * allow faster processing and more compact representations).
   *
  ...
  /**
   * Given a {@link ConstructId} and its ID (look {@link Callgraph#nodeId}) return the
   * NodeMetaInformation object that represent this node.
   *
   * @param target
   * @param target_id
   * @return
  ...
  private NodeMetaInformation createNodeMetaInformation(
      com.sap.psr.vulas.shared.json.model.ConstructId target, Integer target_id) {
  ...
  public NodeMetaInformation getInformationForConstructId(
      com.sap.psr.vulas.shared.json.model.ConstructId target) {
    return this.nodeInfoMap.get(this.getIdForConstruct(target));
  }

  /**
   * Getter for the field <code>constructsWithoutJarUrl</code>.
   *
   ...
   */
  public Map<com.sap.psr.vulas.shared.json.model.ConstructId, Integer> getDist(
    ...
    final Map<com.sap.psr.vulas.shared.json.model.ConstructId, Integer> result =
        new HashMap<com.sap.psr.vulas.shared.json.model.ConstructId, Integer>();
  ...
  /**
   * Given a target construct (e.g., a change list element of a security patch), the method computes
   * the shortest path from all callgraph nodes to this target (if any). The method internally
   * performs a back search, starting from the target node.
   *
   * @param _tgt a {@link com.sap.psr.vulas.shared.json.model.ConstructId} object.
   ...
   */
  public Map<com.sap.psr.vulas.shared.json.model.ConstructId, LinkedList<Integer>> getShortestPath(
      com.sap.psr.vulas.shared.json.model.ConstructId _tgt,
      final Set<com.sap.psr.vulas.shared.json.model.ConstructId> _stop_if_path_found) {
    ...
    // path map used above
    final Map<com.sap.psr.vulas.shared.json.model.ConstructId, LinkedList<Integer>> result =
        new HashMap<com.sap.psr.vulas.shared.json.model.ConstructId, LinkedList<Integer>>();
  ...
  private synchronized URL collectArchiveInformation(
      com.sap.psr.vulas.shared.json.model.ConstructId tgt_node) {
    URL url = null;
  ...
  /**
   * This method parse a construct, if is the constructor of a NON static inner class that take as
   * first argument a reference to the outer class it will be deleted from the argument list (is
   * added by the compiler).
   *
   * <p>If the ConstructId is not a constructor OR if is not of an inner class OR if there are no
   * reason to modify this constructor the method return null
   *
   ...
   */
  private com.sap.psr.vulas.shared.json.model.ConstructId parseNonStaticInnerClassConstruct(
      com.sap.psr.vulas.shared.json.model.ConstructId target) {
      ...
      try {
        ...
        final String param_to_skip =
            (declaringClass != null && !Modifier.isStatic(clazz.getModifiers())
                ...
                : null);
        com.sap.psr.vulas.shared.json.model.ConstructId result =
            JavaId.toSharedType(
