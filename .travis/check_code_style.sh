...
if [[ $1 == "format" ]]; then
    ...
    printf "Formatting... "
    java -jar google-java-format-1.8-all-deps.jar -r -a --skip-sorting-imports --skip-javadoc-formatting --set-exit-if-changed @java-files.txt
    status=$?
    ...
    printf "Checking... "
    java -jar google-java-format-1.8-all-deps.jar -n -a --skip-sorting-imports --skip-javadoc-formatting --set-exit-if-changed @java-files.txt > non-compliant-files.txt
    status=$?
